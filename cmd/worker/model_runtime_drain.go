...
func newWorkerModelDrain(
	...
	if nilLifecycleDependency(dispatcher) || resumeContext == nil || runningJobCount == nil {
		return nil, errors.New("worker model drain dependencies are unavailable")
	}
	drain := &workerModelDrain{
		dispatcher: dispatcher, resumeContext: resumeContext,
...
func (drain *workerModelDrain) Begin(ctx context.Context) error {
	...
	if drain.dispatcher.Started() {
		return errors.New("worker model dispatcher remained active after drain")
	}
	return nil
}
