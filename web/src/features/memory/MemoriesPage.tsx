...
import { MemoryApiError, parseMemoryJsonObject, type CreateMemoryCandidateInput, type EditMemoryInput, type MemoryRecord, type MemoryStatus, type MemoryType, type TransitionMemoryInput } from "../../api/memory";
...
const MemoryForm = ({ item, onSave, pending }: { item?: MemoryRecord; onSave: (input: Omit<CreateMemoryCandidateInput, "workspaceId" | "idempotencyKey">) => void; pending: boolean }) => {
  ...
  return <form className="artifact-form" onSubmit={(event) => {
    ...
    <label>类型<select value={type} disabled={item !== undefined} onChange={(event) => setType(event.target.value as MemoryType)}>{memoryTypes.map((value) => <option key={value}>{value}</option>)}</select></label>
    ...
    <label className="artifact-form__wide">Memory 内容（JSON 对象）<textarea value={content} onChange={(event) => setContent(event.target.value)} rows={7} /></label>
    <div className="artifact-form__wide button-row"><Button type="submit" disabled={invalid || pending}>{item === undefined ? <Plus size={15} /> : <Pencil size={15} />}{pending ? "正在保存…" : item === undefined ? "创建 Candidate" : "保存修改"}</Button><span className="sidebar-note">Candidate 在确认前不会进入 Agent 上下文；身份由当前认证会话决定。</span></div>
  </form>;
};
...
export const MemoriesPage = () => {
  ...
  const title = useMemo(() => editing === undefined ? "创建 Candidate" : `编辑 ${editing.id.slice(0, 8)}…`, [editing]);

  if (workspaceId === "") return <div className="page-stack"><UnavailableState title="请选择 Workspace" description="Memory 只属于当前 Workspace，浏览器不能指定或伪造 owner。" /></div>;
  return <div className="page-stack">
    <div className="page-intro page-intro--split"><div><p className="eyebrow">Learning / Memory</p><h2>把长期上下文留在你的确认之后。</h2><p>Memory 是用户控制的偏好与上下文，不是正式知识、Citation 或 Evidence。</p></div><div className="folio-mark"><strong>{String(memories.data?.items.length ?? 0)}</strong><span>条 Memory</span></div></div>
    <Card><CardHeader eyebrow="Candidate" title={title} description="仅明确 Confirm 才会进入 ACTIVE；编辑、暂停、删除均保留服务端审计历史。" {...(editing === undefined ? {} : { action: <Button variant="ghost" onClick={() => setEditing(undefined)}>取消编辑</Button> })} /><MemoryForm key={editing?.id ?? "new"} {...(editing === undefined ? {} : { item: editing })} pending={create.isPending || edit.isPending} onSave={(input) => editing === undefined ? createCandidate(input) : saveEdit(editing, input)} />{commandError !== null ? <div className="ui-state ui-state--error" role="alert"><strong>Memory 操作未完成</strong><p>{errorText(commandError)}</p>{retryable(commandError) && commandVariables !== undefined ? <Button variant="secondary" onClick={retry}><RotateCcw size={15} />重试原请求</Button> : null}</div> : null}</Card>
    <Card><CardHeader eyebrow="当前用户范围列表" title="Memory 生命周期" description="筛选只影响当前 Workspace 的服务端读取；暂停、到期和删除记录不会进入上下文。" action={<Button variant="ghost" aria-label="刷新 Memory 列表" onClick={() => void memories.refetch()} disabled={memories.isFetching}><RefreshCw size={16} /></Button>} />
      <div className="filter-bar"><fieldset><legend>类型</legend>{memoryTypes.map((value) => <label key={value}><input type="checkbox" checked={types.includes(value)} onChange={() => { setCursor(undefined); toggle(value, types, setTypes); }} />{value}</label>)}</fieldset><fieldset><legend>状态</legend>{memoryStatuses.map((value) => <label key={value}><input type="checkbox" checked={statuses.includes(value)} onChange={() => { setCursor(undefined); toggle(value, statuses, setStatuses); }} />{value}</label>)}</fieldset></div>
      {memories.isPending ? <div className="ui-state" role="status"><strong>正在读取 Memory</strong><p>正在读取当前认证用户在此 Workspace 中的生命周期记录。</p></div> : null}
      {memories.isError ? <ErrorState title="Memory 列表不可用" description={errorText(memories.error)} onRetry={() => void memories.refetch()} /> : null}
      {!memories.isPending && !memories.isError && memories.data.items.length === 0 ? <EmptyState title="没有匹配的 Memory" description="创建 Candidate 后，确认前不会用于任何 Agent 上下文。" /> : null}
      {!memories.isPending && !memories.isError && memories.data.items.length > 0 ? <div className="collection-definition-list">{memories.data.items.map((item) => <article className="collection-definition-row" key={item.id}><div><strong>{item.type}</strong><small>来源 {item.source.type} · {item.source.ref} · v{item.version}{item.expiresAt === undefined ? "" : ` · 到期 ${new Date(item.expiresAt).toLocaleString("zh-CN")}`}</small><pre className="json-preview">{contentText(item.content)}</pre></div><div><Badge tone={statusTone(item.status)}>{item.status}</Badge><div className="button-row">{item.status === "CANDIDATE" ? <Button size="sm" onClick={() => transition(item, "confirm")} disabled={busy}><CheckCircle2 size={15} />确认</Button> : null}{item.status === "ACTIVE" ? <Button size="sm" variant="secondary" onClick={() => transition(item, "pause")} disabled={busy}><CirclePause size={15} />暂停</Button> : null}{item.status === "PAUSED" ? <Button size="sm" variant="secondary" onClick={() => transition(item, "resume")} disabled={busy}><CirclePlay size={15} />恢复</Button> : null}{(item.status === "CANDIDATE" || item.status === "ACTIVE" || item.status === "PAUSED") ? <Button size="sm" variant="ghost" onClick={() => setEditing(item)} disabled={busy}><Pencil size={15} />编辑</Button> : null}{item.status !== "DELETED" && item.status !== "EXPIRED" ? <Button size="sm" variant="ghost" onClick={() => transition(item, "delete")} disabled={busy}><Trash2 size={15} />删除</Button> : null}</div></div></article>)}</div> : null}
