...
export const HealthPage = () => {
  ...
  return <div className="page-stack"><div className="page-intro page-intro--split"><div><h1>知识健康</h1><p>发现并处理知识质量问题。</p></div><div className="folio-mark"><HeartPulse size={20} /><strong>Health</strong><span>knowledge</span></div></div><HealthOverview state={state} onChange={onChange} /><ScanRecovery scanId={state.scanId} /><IssueFilters state={state} onChange={onChange} /><IssueList state={state} onChange={onChange} returnFocus={returnFocus} /><EvidenceDialog issueId={state.issueId} onClose={() => onChange({ ...state, issueId: "" })} returnFocus={returnFocus} /></div>;
};
