...
const fieldMetadata: Record<(typeof collectionQueryFieldRegistry)[number], { label: string; operators: CollectionOperator[]; kind: "text" | "number" | "date" | "enum"; options?: readonly string[] }> = {
  object_type: { label: "对象类型", operators: [...collectionFieldOperators.object_type], kind: "enum", options: collectionEnumValueRegistry.object_type }, topic_id: { label: "主题 ID", operators: [...collectionFieldOperators.topic_id], kind: "text" }, status: { label: "状态", operators: [...collectionFieldOperators.status], kind: "enum", options: collectionEnumValueRegistry.status },
  ...
  relation_type: { label: "关系类型", operators: [...collectionFieldOperators.relation_type], kind: "enum", options: collectionEnumValueRegistry.relation_type }, health_issue_type: { label: "健康问题类型", operators: [...collectionFieldOperators.health_issue_type], kind: "enum", options: collectionEnumValueRegistry.health_issue_type }, source_type: { label: "来源类型", operators: [...collectionFieldOperators.source_type], kind: "enum", options: collectionEnumValueRegistry.source_type },
...
type CollectionQueryField = (typeof collectionQueryFieldRegistry)[number];
const objectTypeLabels: Record<string, string> = { TOPIC: "主题", CLAIM: "知识点" };
const collectionStatusLabels: Record<CollectionStatus, string> = { ACTIVE: "已启用", ARCHIVED: "已归档" };
const itemStatusLabels: Record<(typeof collectionEnumValueRegistry.status)[number], string> = { ACTIVE: "已启用", MERGED: "已合并", DEPRECATED: "已弃用", SUGGESTED: "待确认", CONFIRMED: "已确认", DISPUTED: "有争议", SUPERSEDED: "已替代", INVALID: "无效" };
const relationTypeLabels: Record<string, string> = { BELONGS_TO: "属于", SUPPORTS: "支持", CITES: "引用", DERIVED_FROM: "派生自", COMPLEMENTS: "补充", DUPLICATES: "重复", CONFLICTS_WITH: "冲突", PREREQUISITE_OF: "前置条件", VERSION_OF: "版本关系", IMPACTS: "影响" };
const sourceTypeLabels: Record<string, string> = { markdown: "Markdown", text: "文本", pdf: "PDF", html: "HTML", file: "文件" };
const healthIssueTypeLabels: Record<string, string> = { ORPHAN: "孤立", DUPLICATE: "重复", CONFLICT: "冲突", STALE: "已过期", MISSING_SOURCE: "缺少来源", LOW_CONFIDENCE: "置信度低", BROKEN_REFERENCE: "引用损坏", INDEX_ERROR: "索引错误", SUPERSEDED_USAGE: "已替代使用", REVIEW_INVALIDATED: "复习失效" };
const healthSeverityLabels: Record<string, string> = { CRITICAL: "严重", HIGH: "高", MEDIUM: "中", LOW: "低", INFO: "提示" };
const viewLabels: Record<CollectionViewType, string> = { LIST: "列表", TABLE: "表格", COMPACT_CARD: "紧凑卡片" };
const densityLabels = { COMFORTABLE: "舒适", COMPACT: "紧凑" } as const;
const columnLabels: Record<(typeof collectionViewColumnRegistry)[number], string> = { object_type: "对象类型", title: "标题", summary: "摘要", status: "状态", topic: "主题", source: "来源", relation_summary: "关系", health_summary: "健康问题", confidence: "置信度", created_at: "创建时间", updated_at: "更新时间" };
const objectTypeLabel = (type: string): string => objectTypeLabels[type] ?? type;
const itemStatusLabel = (status: string): string => collectionEnumValueRegistry.status.includes(status as (typeof collectionEnumValueRegistry.status)[number]) ? itemStatusLabels[status as (typeof collectionEnumValueRegistry.status)[number]] : status;
const relationTypeLabel = (type: string): string => relationTypeLabels[type] ?? type;
const sourceTypeLabel = (type: string): string => sourceTypeLabels[type.toLowerCase()] ?? type;
const healthIssueTypeLabel = (type: string): string => healthIssueTypeLabels[type] ?? type;
const healthSeverityLabel = (severity: string): string => healthSeverityLabels[severity] ?? severity;
const queryEnumLabel = (field: CollectionQueryField, value: string): string => {
  if (field === "object_type") return objectTypeLabel(value);
  if (field === "status") return itemStatusLabel(value);
  if (field === "relation_type") return relationTypeLabel(value);
  if (field === "health_issue_type") return healthIssueTypeLabel(value);
  if (field === "source_type") return sourceTypeLabel(value);
  return value;
};
...
const ItemMeta = ({ item }: { item: CollectionItem }) => <div className="collection-item__meta"><Badge tone={item.objectType === "TOPIC" ? "info" : "neutral"}>{objectTypeLabel(item.objectType)}</Badge><Badge tone={itemTone(item)}>{itemStatusLabel(item.status)}</Badge>{item.healthSummary ? <Badge tone="danger">{healthSeverityLabel(item.healthSummary.maxSeverity)} · {item.healthSummary.count}</Badge> : null}<time>{formatDate(item.updatedAt)}</time></div>;
...
const ListView = ({ items, selected, onSelect }: { items: CollectionItem[]; selected: string; onSelect: (ref: string) => void }) => <div className="collection-items" aria-label="集合列表视图">{items.map((item) => { const ref = `${item.objectType}:${item.id}`; return <article className="collection-item" key={ref}><div><h3>{textOrDash(item.title)}</h3><p>{textOrDash(item.summary)}</p><ItemMeta item={item} /></div><SelectItem selected={selected === ref} onSelect={() => onSelect(selected === ref ? "" : ref)} /></article>; })}</div>;
const columnValue = (item: CollectionItem, column: string): ReactNode => ({ object_type: objectTypeLabel(item.objectType), title: textOrDash(item.title), summary: textOrDash(item.summary), status: itemStatusLabel(item.status), topic: item.topicId?.slice(0, 8) ?? "—", source: textOrDash(item.sourceSummaries.map((source) => `${sourceTypeLabel(source.sourceType)}:${source.filePath}`).join("；")), relation_summary: textOrDash(item.relationTypes.map(relationTypeLabel).join("、")), health_summary: item.healthSummary === null ? "—" : `${healthSeverityLabel(item.healthSummary.maxSeverity)} · ${String(item.healthSummary.count)} · ${textOrDash(item.healthSummary.summary)}`, confidence: item.confidence?.toFixed(2) ?? "—", created_at: formatDate(item.createdAt), updated_at: formatDate(item.updatedAt) } as Record<string, ReactNode>)[column] ?? "—";
const groupValue = (item: CollectionItem, group: CollectionUrlState["group"]): string => group === "" ? "" : textOrDash(({ object_type: objectTypeLabel(item.objectType), status: itemStatusLabel(item.status), topic_id: item.topicId?.slice(0, 8) ?? "", relation_type: relationTypeLabel(item.relationType ?? item.relationTypes[0] ?? ""), health_issue_type: healthIssueTypeLabel(item.healthIssueType ?? item.healthSummary?.issueTypes[0] ?? ""), source_type: sourceTypeLabel(item.sourceType ?? item.sourceSummaries[0]?.sourceType ?? "") } as Record<Exclude<CollectionUrlState["group"], "">, string>)[group]);
...
const QueryBuilder = ({ state, onChange, onRefresh, refreshing }: { state: CollectionUrlState; onChange: (state: CollectionUrlState) => void; onRefresh: () => void; refreshing: boolean }) => {
  ...
  const valueControl = (clause: CollectionPredicateDraft, index: number, metadata: (typeof fieldMetadata)[(typeof collectionQueryFieldRegistry)[number]]) => {
    ...
    if (metadata.kind === "enum" && metadata.options !== undefined) {
      if (clause.operator === "IN") return <select aria-label={`条件 ${String(index + 1)} 值`} multiple value={clause.value.split(",").filter(Boolean)} onChange={(event) => updateValue(Array.from(event.currentTarget.selectedOptions, (option) => option.value).join(","))}>{metadata.options.map((option) => <option key={option} value={option}>{queryEnumLabel(clause.field, option)}</option>)}</select>;
      return <select aria-label={`条件 ${String(index + 1)} 值`} value={clause.value} onChange={(event) => updateValue(event.target.value)}>{metadata.options.map((option) => <option key={option} value={option}>{queryEnumLabel(clause.field, option)}</option>)}</select>;
    }
  ...
  return <Card className="collection-builder"><CardHeader eyebrow="查询" title="条件构建器" description="选择字段、条件和值，实时查看匹配数量。" action={<Filter size={18} />} />
    <div className="filter-bar" aria-label="集合查询条件"><label>组合<select value={state.logic} onChange={(event) => onChange({ ...state, logic: event.target.value === "OR" ? "OR" : "AND" })}><option value="AND">满足全部条件</option><option value="OR">满足任一条件</option></select></label><label>排序字段<select value={state.sortField} onChange={(event) => onChange({ ...state, sortField: event.target.value as CollectionUrlState["sortField"] })}>{collectionSortableFieldRegistry.map((field) => <option key={field} value={field}>{fieldMetadata[field].label}</option>)}</select></label><label>方向<select value={state.sortDirection} onChange={(event) => onChange({ ...state, sortDirection: event.target.value === "ASC" ? "ASC" : "DESC" })}><option value="DESC">降序</option><option value="ASC">升序</option></select></label></div>
    <div className="collection-clause-list">{state.clauses.map((clause, index) => {
      const metadata = fieldMetadata[clause.field];
      return <div className="filter-bar collection-clause-row" key={`${String(index)}:${clause.field}:${clause.operator}`} aria-label={`集合条件 ${String(index + 1)}`}><label>字段<select value={clause.field} onChange={(event) => updateClause(index, { field: event.target.value as CollectionPredicateDraft["field"] })}>{collectionQueryFieldRegistry.map((field) => <option key={field} value={field}>{fieldMetadata[field].label}</option>)}</select></label><label>条件<select value={clause.operator} onChange={(event) => updateClause(index, { operator: event.target.value as CollectionOperator })}>{metadata.operators.map((operator) => <option key={operator}>{operator}</option>)}</select></label>{clause.operator === "IS_NULL" ? null : <label className="filter-field--wide">值{valueControl(clause, index, metadata)}</label>}<Button variant="ghost" disabled={state.clauses.length <= 1} onClick={() => removeClause(index)}>删除</Button></div>;
    })}</div>
...
const ViewControls = ({ state, fixedColumns, onChange }: { state: CollectionUrlState; fixedColumns: string[]; onChange: (state: CollectionUrlState) => void }) => <Card><CardHeader eyebrow="视图" title="视图、列与分组" description="列表、表格和卡片保持相同对象顺序。" /><div className="collection-view-controls"><div className="view-switcher" aria-label="结果视图">{(["LIST", "TABLE", "COMPACT_CARD"] as CollectionViewType[]).map((view) => <Button key={view} size="sm" variant={state.view === view ? "primary" : "ghost"} onClick={() => onChange({ ...state, view })}>{view === "LIST" ? <List size={15} /> : view === "TABLE" ? <Table2 size={15} /> : <LayoutGrid size={15} />}{viewLabels[view]}</Button>)}</div><label>分组<select value={state.group} onChange={(event) => onChange({ ...state, group: event.target.value as CollectionUrlState["group"] })}><option value="">不分组</option>{collectionGroupFieldRegistry.map((field) => <option key={field} value={field}>{fieldMetadata[field].label}</option>)}</select></label><label>密度<select value={state.density} onChange={(event) => onChange({ ...state, density: event.target.value === "COMPACT" ? "COMPACT" : "COMFORTABLE" })}><option value="COMFORTABLE">{densityLabels.COMFORTABLE}</option><option value="COMPACT">{densityLabels.COMPACT}</option></select></label><fieldset><legend>表格列</legend>{columnOptions.map((column) => { const fixed = fixedColumns.includes(column); return <label key={column} title={fixed ? "固定列不可取消" : undefined}><input type="checkbox" checked={state.columns.includes(column)} disabled={fixed} onChange={() => { if (fixed) return; const columns = state.columns.includes(column) ? state.columns.filter((item) => item !== column) : [...state.columns, column]; if (columns.length > 0) onChange({ ...state, columns }); }} />{columnLabels[column]}</label>; })}</fieldset></div></Card>;

const SelectedProjection = ({ item }: { item: CollectionItem | undefined }) => {
  if (item === undefined) return null;
  const healthText = item.healthSummary === null ? "无待处理问题摘要" : textOrDash(item.healthSummary.summary) === "—" ? textOrDash(item.healthSummary.issueTypes.map(healthIssueTypeLabel).join("、")) : item.healthSummary.summary;
  return <Card className="collection-selected"><CardHeader eyebrow="当前页对象" title={textOrDash(item.title) === "—" ? `${objectTypeLabel(item.objectType)} ${item.id.slice(0, 8)}` : item.title} description="展开对象摘要、来源、关系与健康状态。" /><dl className="stacked-meta"><div><dt>别名</dt><dd>{textOrDash(item.aliases.join("、"))}</dd></div><div><dt>适用范围</dt><dd>{item.applicabilitySchemaVersion ?? "—"} {item.applicabilityHash?.slice(0, 12) ?? ""}</dd></div><div><dt>来源</dt><dd>{textOrDash(item.sourceSummaries.map((source) => `${sourceTypeLabel(source.sourceType)}:${textOrDash(source.filePath)}`).join("；"))}</dd></div><div><dt>关系</dt><dd>{textOrDash(item.relationTypes.map(relationTypeLabel).join("、"))}</dd></div><div><dt>健康状态</dt><dd>{healthText}</dd></div></dl></Card>;
};
...
export const CollectionsPage = () => {
  ...
  if (!workspaceId) return <div className="page-stack"><UnavailableState title="先连接 Workspace" description="集合查询必须绑定真实 Workspace；这里不会用本地样例填充。" /><Link className="ui-button ui-button--primary" to="/settings">前往设置</Link></div>;
...
export const CollectionDetailPage = () => {
  ...
  if (!workspaceId) return <div className="page-stack"><UnavailableState title="未连接 Workspace" description="请先在设置中选择 Workspace，再打开集合。" /></div>;
  if (collection.isPending) return <div className="page-stack"><StateMessage title="正在读取集合" description="读取保存的查询条件与视图配置。" /></div>;
  if (collection.isError) return <div className="page-stack"><ErrorState title="集合不可用" description={collection.error instanceof Error ? collection.error.message : "找不到该集合。"} onRetry={() => void collection.refetch()} /></div>;
  ...
  const scanStatus = result.isError ? "结果读取失败，修复后才能启动健康扫描。" : result.isPending || result.isFetching ? "结果加载完成后才能启动健康扫描。" : !resultMatchesCollection ? "结果与当前集合版本不一致，刷新后再扫描。" : exceedsHealthScanCapacity ? `当前集合有 ${String(resultBinding.exactCount)} 个对象，单次健康扫描最多处理 5000 个；请收窄集合条件后重试。` : `将扫描 ${String(resultBinding.exactCount)} 个对象。`;
  const scan = () => {
  ...
  return <div className="page-stack"><div className="page-intro page-intro--split"><div><Link className="back-link" to="/collections">← 返回集合</Link><p className="eyebrow">已保存查询 / v{item.queryVersion}</p><h1>{item.name}</h1><p>{item.description || "没有描述"}</p></div><div className="hash-card"><span>范围指纹</span><code>{item.queryHash.slice(0, 16)}…</code><Badge tone={item.status === "ACTIVE" ? "success" : "neutral"}>{collectionStatusLabels[item.status]}</Badge></div></div><Card><CardHeader eyebrow="受控操作" title="下游入口" description="健康扫描绑定当前集合版本与结果事实；关系分析由服务端冻结集合范围。" /><div className="button-row"><Button onClick={scan} disabled={scanDisabled}><HeartPulse size={15} />{startScan.isPending ? "正在接受扫描…" : "启动健康扫描"}</Button><Button variant="secondary" onClick={startRelationScan} disabled={item.status !== "ACTIVE" || startSemanticScan.isPending}>{startSemanticScan.isPending ? "正在启动…" : "启动关系分析"}</Button><Button variant="ghost" disabled title="复习功能尚未接入">创建复习卡组（不可用）</Button></div><p className="sidebar-note">{scanStatus}</p>{startScan.isError ? <p className="form-error" role="alert">{startScan.error.message}</p> : null}{startSemanticScan.isError ? <p className="form-error" role="alert">{startSemanticScan.error.message}</p> : null}</Card><CollectionExportPanel collection={item} /><CollectionResults result={result} cursor={cursor} setCursor={setCursor} state={state} fixedColumns={item.viewConfig.fixedColumns} onStateChange={updateState} /><Card><CardHeader eyebrow="查询定义" title="保存的条件" /><pre className="json-preview">{JSON.stringify(item.query, null, 2)}</pre><p className="sidebar-note">归档不会删除正式知识；事实变化会让旧分页明确失效。</p></Card></div>;
};
