...
import { getSourceVersion, listSourceVersions, type SourceIndexStatus, type SourceIngestionStatus, type SourceSecurityStatus, type WorkflowStatus } from "../../api/business";
...
const sourceSecurityLabels: Record<SourceSecurityStatus, string> = { pending: "待检查", passed: "通过", quarantined: "已隔离" };
const sourceIngestionLabels: Record<SourceIngestionStatus, string> = { validating: "安全校验", parsing: "解析中", parsed: "已解析", chunking: "分块中", chunked: "已分块", parse_failed: "解析失败", cancelled: "已取消" };
const workflowStatusLabels: Record<WorkflowStatus, string> = { pending: "等待", running: "运行中", waiting_for_human: "等待人工", retry_wait: "等待重试", paused: "已暂停", succeeded: "成功", failed: "失败", cancelled: "已取消" };
const sourceIndexLabel = (status: SourceIndexStatus | undefined) => status === "included" ? "已纳入" : status === "excluded" ? "已排除" : "未选择";
...
export const DocumentsPage = () => {
  ...
  return <div className="page-stack">
    ...
    {query.isError ? <UnavailableState title="资料详情不可用" description={query.error.message} /> : query.isPending ? <Card><p>正在查询资料版本…</p></Card> : query.data ? <Card>
      ...
      <dl className="detail-grid"><div><dt>资料版本</dt><dd className="mono">{query.data.id}</dd></div><div><dt>内容哈希</dt><dd className="mono">{query.data.contentHash}</dd></div><div><dt>Mime / 大小</dt><dd>{query.data.mimeType} · {formatBytes(query.data.byteSize)}</dd></div><div><dt>安全状态</dt><dd><Badge tone={query.data.securityStatus === "passed" ? "success" : "warning"}>{sourceSecurityLabels[query.data.securityStatus]}</Badge></dd></div><div><dt>解析状态</dt><dd>{query.data.ingestionStatus ? <Badge tone={ingestionTone(query.data.ingestionStatus)}>{sourceIngestionLabels[query.data.ingestionStatus]}</Badge> : <span className="muted">未开始</span>}</dd></div><div><dt>Workflow 状态</dt><dd>{query.data.workflowStatus ? <Badge tone={workflowTone(query.data.workflowStatus)}>{workflowStatusLabels[query.data.workflowStatus]}</Badge> : <span className="muted">未绑定</span>}</dd></div><div><dt>索引状态</dt><dd>{query.data.indexStatus ? <Badge tone={query.data.indexStatus === "included" ? "success" : "warning"}>{sourceIndexLabel(query.data.indexStatus)}</Badge> : <span className="muted">未选择</span>}</dd></div></dl>
      <div className="document-actions"><Button asChild variant="secondary"><Link to={`/search?source_version_id=${encodeURIComponent(query.data.id)}&scope_workspace=${encodeURIComponent(workspaceId)}`}><Search size={15} />在搜索中限定此版本</Link></Button><Button asChild variant="secondary"><Link to="/chat"><MessageSquare size={15} />进入对话</Link></Button><Button asChild variant="secondary"><Link to="/graph"><GitBranch size={15} />进入图谱</Link></Button></div>
