...
const DeckActions = ({
  ...
  <Button size="sm" onClick={() => onStart(deck)} disabled={deck.status !== "ACTIVE" || pending} title={deck.status === "ACTIVE" ? undefined : "只有活动 Deck 可以开始复习"}>
  ...
  <Button size="sm" variant="ghost" onClick={() => onSchedule(deck, "reset")} disabled={deck.status === "ARCHIVED" || pending} title={deck.status === "ARCHIVED" ? "归档 Deck 不可重置调度" : undefined}><RotateCcw size={15} />重置调度</Button>
</div>;
...
export const ReviewPage = () => {
  ...
  if (workspaceId === "") return <div className="page-stack"><UnavailableState title="请选择 Workspace" description="Review Deck 始终在单一 Workspace 内创建和复习。" /></div>;
  ...
  return <div className="page-stack">
    ...
    <Card>
      <CardHeader eyebrow="New Deck" title="创建 Review Deck" description="每日上限、暂停、恢复和重置均由服务端调度事实决定。" />
      <form className="artifact-form" onSubmit={(event) => { event.preventDefault(); submitCreate(); }}>
        <label>Deck 名称<input value={name} maxLength={256} onChange={(event) => setName(event.target.value)} placeholder="例如：Go 并发基础" /></label>
        <label>每日上限<input type="number" min="1" max="1000" value={dailyLimit} onChange={(event) => setDailyLimit(event.target.value)} /></label>
        <div className="artifact-form__wide button-row"><Button type="submit" disabled={createInvalid || create.isPending}><Plus size={15} />{create.isPending ? "正在创建…" : "创建 Deck"}</Button><span className="sidebar-note">范围固定为当前 Workspace；Card 仍须经过服务端证据校验与审批后才会进入复习队列。</span></div>
      </form>
      {create.isError ? <div className="ui-state ui-state--error" role="alert"><strong>Deck 创建失败</strong><p>{errorText(create.error)}</p>{isRetryableReviewError(create.error) ? <Button variant="secondary" onClick={retryCreate}><RotateCcw size={15} />重试原请求</Button> : null}</div> : null}
    </Card>

    <Card>
      <CardHeader eyebrow="Workspace Decks" title="Deck 列表" description="今天的待复习数由 Deck 上限、Card 状态和当前调度共同计算。" action={<Button variant="ghost" onClick={() => void decks.refetch()} disabled={decks.isFetching} aria-label="刷新 Deck 列表"><RefreshCw size={16} /></Button>} />
      {decks.isPending ? <div className="ui-state" role="status"><strong>正在读取 Deck</strong><p>正在加载当前 Workspace 的复习配置。</p></div> : null}
      {decks.isError ? <ErrorState title={isVersionConflict(decks.error) ? "Deck 版本已变化" : "Deck 列表不可用"} description={errorText(decks.error)} onRetry={() => void decks.refetch()} /> : null}
      {!decks.isPending && !decks.isError && decks.data.items.length === 0 ? <EmptyState title="还没有 Review Deck" description="先创建一个 Deck，再由服务端验证并审批 Card。" /> : null}
      {!decks.isPending && !decks.isError && decks.data.items.length > 0 ? <div className="collection-definition-list">{decks.data.items.map((deck) => <article className="collection-definition-row" key={deck.id}>
        ...
        <div><Badge tone={deckTone(deck.status)}>{deck.status}</Badge><DeckActions deck={deck} onStart={startSession} onSchedule={runSchedule} pending={schedulePending || (start.isPending && activeDeckId === deck.id)} /></div>
      </article>)}</div> : null}
      {scheduleError !== null ? <div className="ui-state ui-state--error" role="alert"><strong>{isVersionConflict(scheduleError) ? "Deck 已在其他位置更新" : "Deck 调度操作失败"}</strong><p>{errorText(scheduleError)}</p><div className="button-row"><Button variant="secondary" onClick={() => { void decks.refetch(); }}><RefreshCw size={15} />刷新 Deck</Button>{isRetryableReviewError(scheduleError) ? <Button variant="secondary" onClick={retrySchedule}><RotateCcw size={15} />重试原请求</Button> : null}</div></div> : null}
