...
import { workflowStatusOptions } from "../business/url-state";
...
const artifactStatusLabels: Record<ArtifactStatus, string> = { PLANNING: "规划中", OUTLINE_REVIEW: "大纲待审", GENERATING: "生成中", DRAFT: "草稿", APPROVED: "已审批", EXPORTED: "已导出", PUBLISH_PROPOSED: "待发布审批", PUBLISHED: "已发布", ARCHIVED: "已归档" };
const coverageLabels: Record<ArtifactCoverage["status"], string> = { COVERED: "已覆盖", PARTIAL: "部分覆盖", GAP: "知识缺口" };
const generationStatusLabels: Record<ArtifactSectionGenerationStatus, string> = { PENDING: "等待生成", COMPLETED: "已完成", FAILED: "生成失败", CANCELLED: "已取消", RECOVERY_REQUIRED: "需要恢复" };
const workflowStatusLabel = (status: WorkflowStatus): string => workflowStatusOptions.find(([value]) => value === status)?.[1] ?? status;
...
const ArtifactSectionView = ({ artifact, outlineSection, section, serverGeneration, refetchGenerations, refetchArtifact }: { artifact: Artifact; outlineSection: ArtifactOutlineSection; section: ArtifactSection | undefined; serverGeneration: ArtifactSectionGeneration | undefined; refetchGenerations: RefetchSectionGenerations; refetchArtifact: RefetchArtifact }) => {
  ...
  const generationPanel = artifact.status === "GENERATING" ? <div className={`artifact-generation${status === "RECOVERY_REQUIRED" ? " artifact-generation--recovery" : ""}`}>
    ...
    {generation.isPending ? <p className="artifact-generation__status" role="status"><LoaderCircle className="is-spinning" size={15} />{status === "PENDING" && workflowTerminal ? "正在同步生成终态…" : "正在提交生成请求…"}</p> : null}
    ...
    {currentGeneration !== undefined ? <>
      <div className="artifact-generation__headline"><Badge tone={generationTone(currentGeneration.status)}>{generationStatusLabels[currentGeneration.status]}</Badge>{replayed ? <span>已从原请求恢复</span> : <span>生成记录 {currentGeneration.generationId.slice(0, 8)}…</span>}</div>
    {currentGeneration.status === "PENDING" ? <p className="artifact-generation__status">{workflow.isError ? "Workflow 状态读取失败。" : workflowTerminal ? receiptPollExhausted ? "生成终态仍未可见，请手动刷新。" : "Workflow 已结束，正在核对生成终态。" : workflowStatus === undefined ? "正在读取 Workflow 状态…" : `Workflow · ${workflowStatusLabel(workflowStatus)}`}</p> : null}
      {currentGeneration.status === "COMPLETED" ? <p className="artifact-generation__status">章节生成完成，正在刷新产物修订版本。</p> : null}
      ...
      <div className="artifact-generation__actions">
        ...
        {workflowTerminal && currentGeneration.status === "PENDING" ? <Button variant="secondary" onClick={refreshReceipt}><RefreshCw size={15} />刷新生成状态</Button> : null}
  ...
  return <article className="artifact-section">
    <header><div><small>{outlineSection.key}</small><h3>{outlineSection.title}</h3></div>{section === undefined ? <Badge tone={status === undefined ? "neutral" : generationTone(status)}>{generation.isPending && status === undefined ? "等待生成" : status === undefined ? "未开始" : generationStatusLabels[status]}</Badge> : <Badge tone={coverageTone(section.coverage)}>{coverageLabels[section.coverage.status]}</Badge>}</header>
    {section === undefined ? <>
      <p className="muted">此章节尚未记录。</p>
      {generationPanel}
    </> : <><p className="artifact-section__content">{section.content || "此章节没有正文，缺口已显式记录。"}</p><Coverage coverage={section.coverage} />{section.citations.length > 0 ? <div className="artifact-source-group"><strong>正式证据</strong><ul className="artifact-citations">{section.citations.map((citation) => <li key={citation.sourceSpanId}><code>{citation.sourceVersionId.slice(0, 8)}…/{citation.sourceSpanId.slice(0, 8)}…</code><span>{citation.excerpt}</span></li>)}</ul></div> : null}{section.documentSources.length > 0 ? <div className="artifact-source-group"><strong>来源文档</strong><ul className="artifact-citations" aria-label="来源文档">{section.documentSources.map((source) => <li key={`${source.documentId}:${source.articleRevisionId}`}><code>{source.documentId.slice(0, 8)}…/{source.articleRevisionId.slice(0, 8)}…</code><span>修订版本 {String(source.revisionNo)} · {source.verifiedContentHash.slice(0, 16)}…</span></li>)}</ul></div> : null}{generationPanel}</>}
  </article>;
};
...
export const ArtifactsPage = () => {
  ...
  if (workspaceId === "") return <div className="page-stack"><UnavailableState title="先连接 Workspace" description="产物只能读取和写入当前 Workspace；不会使用本地样例替代。" /><Link className="ui-button ui-button--primary" to="/workspace">连接 Workspace</Link></div>;
  return <div className="page-stack"><div className="page-intro page-intro--split"><div><h1>产物</h1><p>创建、审阅和导出可追溯产物。</p></div><div className="folio-mark"><FileText size={20} /><strong>{artifacts.data?.items.length ?? "—"}</strong><span>份产物</span></div></div>
    <Card><CardHeader eyebrow="新建隔离产物" title="目标与范围" description="创建后先提交并审批大纲，章节及其来源覆盖保留在不可变修订版本中。" /><form className="artifact-form" onSubmit={(event) => { event.preventDefault(); submit(); }}><label>类型<input value={type} maxLength={128} onChange={(event) => setType(event.target.value)} /></label><label>标题<input value={title} maxLength={512} onChange={(event) => setTitle(event.target.value)} placeholder="例如：检索链路复盘" required /></label><label className="artifact-form__wide">范围<textarea value={scope} maxLength={16_384} onChange={(event) => setScope(event.target.value)} placeholder="目标、边界和预期读者" required /></label><div className="button-row"><Button type="submit" disabled={plan.isPending}><Plus size={15} />{plan.isPending ? "正在创建…" : "创建产物"}</Button>{plan.isError ? <span className="form-error" role="alert">{errorText(plan.error)}</span> : null}</div></form></Card>
    <Card><CardHeader eyebrow="Workspace 产物" title="已创建产物" action={<Button variant="ghost" onClick={() => void artifacts.refetch()} aria-label="刷新产物列表"><RefreshCw size={16} /></Button>} />{artifacts.isError ? <ErrorState title="产物列表不可用" description={errorText(artifacts.error)} onRetry={() => void artifacts.refetch()} /> : artifacts.isPending ? <div className="ui-state"><strong>正在读取产物</strong><p>当前 Workspace 内的隔离版本正在加载。</p></div> : artifacts.data.items.length === 0 ? <EmptyState title="还没有产物" description="先创建目标和范围，再提交可审阅的大纲。" /> : <div className="artifact-list">{artifacts.data.items.map((artifact) => <Link className="artifact-row" key={artifact.id} to={`/artifacts/${artifact.id}`}><div><strong>{artifact.title}</strong><small>{artifact.type} · 版本 {String(artifact.revision.revisionNo)} · v{String(artifact.version)}</small></div><div><Badge tone={statusTone(artifact.status)}>{artifactStatusLabels[artifact.status]}</Badge><ArrowRight size={16} /></div></Link>)}</div>}<div className="pagination-row"><span className="sidebar-note">当前页 {String(artifacts.data?.items.length ?? 0)} 个产物</span><div className="button-row">{cursor !== undefined ? <Button variant="ghost" onClick={() => setCursor(undefined)}>回到首屏</Button> : null}{artifacts.data?.nextCursor ? <Button variant="secondary" onClick={() => setCursor(artifacts.data.nextCursor)}>下一页<ArrowRight size={15} /></Button> : null}</div></div></Card>
  </div>;
};
...
export const ArtifactDetailPage = () => {
  ...
  if (workspaceId === "") return <div className="page-stack"><UnavailableState title="未连接 Workspace" description="无法读取未绑定 Workspace 的产物。" /></div>;
  if (artifact.isPending) return <div className="page-stack"><div className="ui-state"><strong>正在读取产物</strong><p>正在核对当前修订版本、状态和来源覆盖。</p></div></div>;
  if (artifact.isError) return <div className="page-stack"><ErrorState title="产物不可用" description={errorText(artifact.error)} onRetry={() => void artifact.refetch()} /></div>;
  if (generations.isPending) return <div className="page-stack"><div className="ui-state"><strong>正在恢复章节生成状态</strong><p>正在从服务端核对每章的权威生成记录。</p></div></div>;
...
const ArtifactWorkspace = ({ artifact, generations, refetchGenerations, refetchArtifact }: { artifact: Artifact; generations: ArtifactSectionGenerationPage; refetchGenerations: RefetchSectionGenerations; refetchArtifact: RefetchArtifact }) => {
  ...
  return <div className="page-stack"><div className="page-intro page-intro--split"><div><Link className="back-link" to="/artifacts">← 返回产物</Link><p className="eyebrow">{artifact.type} / 版本 {String(artifact.revision.revisionNo)}</p><h1>{artifact.title}</h1><p>{artifact.scopeDefinition}</p></div><div className="hash-card"><span>当前内容哈希</span><code>{artifact.revision.contentHash.slice(0, 16)}…</code><Badge tone={statusTone(artifact.status)}>{artifactStatusLabels[artifact.status]}</Badge></div></div>
    <Card><CardHeader eyebrow="受控操作" title="产物生命周期" description="每次操作带当前版本和稳定 Idempotency-Key；409 表示状态或版本已变化，刷新后再继续。" /><div className="button-row">{artifact.status === "OUTLINE_REVIEW" ? <Button onClick={() => revisionCommand("artifact-approve-outline", (input) => approveOutline.mutate(input))} disabled={busy || versionConflict || refreshingRevision}><ShieldCheck size={15} />审批大纲</Button> : null}{artifact.status === "DRAFT" ? <Button onClick={() => revisionCommand("artifact-approve-draft", (input) => approveDraft.mutate(input))} disabled={busy || versionConflict || refreshingRevision}><ShieldCheck size={15} />审批草稿</Button> : null}{artifact.status === "DRAFT" || artifact.status === "APPROVED" || artifact.status === "EXPORTED" ? <Button variant="secondary" onClick={() => revisionCommand("artifact-start-revision", (input) => startRevision.mutate(input))} disabled={busy || versionConflict || refreshingRevision}><RotateCcw size={15} />开始修订</Button> : null}{artifact.status === "APPROVED" ? <Button onClick={() => revisionCommand("artifact-export", (input) => exportMarkdown.mutate(input))} disabled={busy || versionConflict || refreshingRevision}><Download size={15} />导出 Markdown</Button> : null}{artifact.status === "APPROVED" || artifact.status === "EXPORTED" ? <Button variant="secondary" onClick={() => revisionCommand("artifact-publish", (input) => publish.mutate(input))} disabled={busy || versionConflict || refreshingRevision}><Send size={15} />创建发布提案</Button> : null}{artifact.status === "PUBLISH_PROPOSED" ? <span className="artifact-note"><TriangleAlert size={15} />已创建提案；正式知识写入仍须在提案页审批与执行。</span> : null}</div>{versionConflict ? <section className="artifact-generation__error" role="alert"><strong>当前版本已更新</strong><p>其他操作已改变产物，不能继续使用旧版本提交。</p><Button variant="secondary" onClick={refreshCurrentRevision} disabled={refreshingRevision}><RefreshCw size={15} />{refreshingRevision ? "正在刷新当前版本…" : "刷新当前版本"}</Button></section> : mutationError !== undefined ? <p className="form-error" role="alert">{errorText(mutationError)}</p> : null}{exportMarkdown.data?.export ? <p className="artifact-success" role="status">Markdown 已导出并绑定版本 {String(exportMarkdown.data.export.revisionNo)}，输出哈希 {exportMarkdown.data.export.outputHash.slice(0, 16)}…</p> : null}{publish.data?.publication ? <p className="artifact-success" role="status">发布提案已创建：<Link to={`/proposals/${publish.data.publication.proposalId}`}>{publish.data.publication.proposalId}</Link>。产物尚未成为正式知识。</p> : null}</Card>
    {artifact.status === "PLANNING" ? <Card><CardHeader eyebrow="第一步" title="提交大纲" description="每行使用 key: 标题。提交会冻结新的不可变修订版本，随后进入人工审批。" /><label className="artifact-outline-editor">大纲<textarea value={outlineText} onChange={(event) => setOutlineText(event.target.value)} placeholder="intro: 背景\nevidence: 已批准知识" /></label><div className="button-row"><Button onClick={outline} disabled={submitOutline.isPending || parseOutline() === undefined}>提交大纲</Button>{parseOutline() === undefined ? <span className="form-error" role="alert">每行必须为唯一的 key: 标题。</span> : null}</div></Card> : null}
    {artifact.status === "GENERATING" ? <Card><CardHeader eyebrow="章节记录" title="显式记录知识缺口" description="此入口只记录没有正文、正式证据或来源文档的知识缺口章节，也可替换当前修订版本中的已有章节。带来源的已覆盖或部分覆盖章节必须由服务端验证后写入。" /><div className="artifact-form"><label>修订章节<select value={sectionKey} onChange={(event) => setSectionKey(event.target.value)}><option value="">选择章节</option>{artifact.revision.outline.map((section) => <option key={section.key} value={section.key}>{section.title}</option>)}</select></label><label>缺口代码<input value={gapCode} maxLength={128} onChange={(event) => setGapCode(event.target.value)} /></label><label className="artifact-form__wide">缺口说明<textarea value={gapDescription} maxLength={4096} onChange={(event) => setGapDescription(event.target.value)} required /></label><Button onClick={record} disabled={recordGap.isPending || selectedOutline === undefined || gapDescription.trim() === ""}>{selectedSectionExists ? "替换为知识缺口章节" : "记录知识缺口章节"}</Button></div></Card> : null}
    <Card><CardHeader eyebrow="版本证据" title="章节、来源与覆盖情况" description="正式证据与来源文档都由服务端验证并绑定不可变版本，页面不会自行判定可信性。" />{artifact.revision.outline.length === 0 ? <EmptyState title="尚未提交大纲" description="产物仍在规划阶段。" /> : <div className="artifact-sections">{artifact.revision.outline.map((outlineSection) => <ArtifactSectionView key={outlineSection.key} artifact={artifact} outlineSection={outlineSection} section={artifact.revision.sections.find((item) => item.key === outlineSection.key)} serverGeneration={generations.items.find((item) => item.sectionKey === outlineSection.key)} refetchGenerations={refetchGenerations} refetchArtifact={refetchArtifact} />)}</div>}</Card>
    <Card><CardHeader eyebrow="不可变绑定" title="当前快照" /><dl className="detail-grid"><div><dt>产物版本</dt><dd>{String(artifact.version)}</dd></div><div><dt>修订版本</dt><dd>{String(artifact.revision.revisionNo)}</dd></div><div><dt>修订版本 ID</dt><dd className="mono">{artifact.revision.id}</dd></div><div><dt>内容哈希</dt><dd className="mono">{artifact.revision.contentHash}</dd></div></dl></Card>
  </div>;
};
