...
const ArtifactSectionView = ({ artifact, outlineSection, section, serverGeneration, refetchGenerations, refetchArtifact }: { artifact: Artifact; outlineSection: ArtifactOutlineSection; section: ArtifactSection | undefined; serverGeneration: ArtifactSectionGeneration | undefined; refetchGenerations: RefetchSectionGenerations; refetchArtifact: RefetchArtifact }) => {
  ...
  return <article className="artifact-section">
    ...
    {section === undefined ? <>
      ...
      {generationPanel}
    </> : <><p className="artifact-section__content">{section.content || "此章节没有正文，缺口已显式记录。"}</p><Coverage coverage={section.coverage} />{section.citations.length > 0 ? <div className="artifact-source-group"><strong>正式证据</strong><ul className="artifact-citations">{section.citations.map((citation) => <li key={citation.sourceSpanId}><code>{citation.sourceVersionId.slice(0, 8)}…/{citation.sourceSpanId.slice(0, 8)}…</code><span>{citation.excerpt}</span></li>)}</ul></div> : null}{section.documentSources.length > 0 ? <div className="artifact-source-group"><strong>来源文档</strong><ul className="artifact-citations" aria-label="来源文档">{section.documentSources.map((source) => <li key={`${source.documentId}:${source.articleRevisionId}`}><code>{source.documentId.slice(0, 8)}…/{source.articleRevisionId.slice(0, 8)}…</code><span>Revision {String(source.revisionNo)} · {source.verifiedContentHash.slice(0, 16)}…</span></li>)}</ul></div> : null}{generationPanel}</>}
  </article>;
};
...
const ArtifactWorkspace = ({ artifact, generations, refetchGenerations, refetchArtifact }: { artifact: Artifact; generations: ArtifactSectionGenerationPage; refetchGenerations: RefetchSectionGenerations; refetchArtifact: RefetchArtifact }) => {
  ...
  return <div className="page-stack"><div className="page-intro page-intro--split"><div><Link className="back-link" to="/artifacts">← 返回 Artifact</Link><p className="eyebrow">{artifact.type} / Revision {String(artifact.revision.revisionNo)}</p><h1>{artifact.title}</h1><p>{artifact.scopeDefinition}</p></div><div className="hash-card"><span>当前内容哈希</span><code>{artifact.revision.contentHash.slice(0, 16)}…</code><Badge tone={statusTone(artifact.status)}>{artifact.status}</Badge></div></div>
