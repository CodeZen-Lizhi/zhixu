...
export const Dialog = ({ open, onOpenChange, title, description, children, restoreFocusRef, contentClassName }: { open: boolean; onOpenChange: (open: boolean) => void; title: string; description?: string; children: ReactNode; restoreFocusRef?: RefObject<HTMLElement | null>; contentClassName?: string }) => <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}><DialogPrimitive.Portal><DialogPrimitive.Overlay className="ui-dialog__overlay" /><DialogPrimitive.Content className={cn("ui-dialog__content", contentClassName)} onCloseAutoFocus={(event) => { if (restoreFocusRef?.current) { event.preventDefault(); restoreFocusRef.current.focus(); } }}><DialogPrimitive.Title>{title}</DialogPrimitive.Title>{description ? <DialogPrimitive.Description>{description}</DialogPrimitive.Description> : null}{children}<DialogPrimitive.Close asChild><button className="ui-dialog__close" aria-label="关闭"><X size={18} /></button></DialogPrimitive.Close></DialogPrimitive.Content></DialogPrimitive.Portal></DialogPrimitive.Root>;
