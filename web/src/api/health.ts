...
const params = (workspaceId: string): URLSearchParams => new URLSearchParams({ workspace_id: requireWorkspace(workspaceId) });
...
const encodeScope = (scope: HealthScanScope): Record<string, unknown> => {
  ...
  const base = { type, ref: requireUuid(scope.ref, "scope.ref"), version: requireVersion(scope.version), schema_version: scope.schemaVersion };
  if (type === "SMART_COLLECTION") {
    ...
    return { ...base, hash: scope.hash, read_model_revision: scope.readModelRevision, exact_count: scope.exactCount };
  }
  return base;
};
export const startHealthScan = (input: StartHealthScanInput, signal?: AbortSignal): Promise<HealthScanAcceptance> => { const expectedWorkspaceId = requireWorkspace(input.workspaceId); const maxItems = input.maxItems ?? 5000; if (!Number.isSafeInteger(maxItems) || maxItems < 1 || maxItems > 50_000) throw invalidRequest("maxItems"); return request("/api/v1/health/scans", { method: "POST", headers: { "Idempotency-Key": requireIdempotencyKey(input.idempotencyKey) }, body: JSON.stringify({ workspace_id: expectedWorkspaceId, scope: encodeScope(input.scope), max_items: maxItems, prevent_scope_concurrency: input.preventScopeConcurrency ?? true }), ...signalInit(signal) }).then((value) => { if (!isRecord(value)) throw invalidResponse("scan_acceptance"); exact(value, ["scan", "health_scan_id", "workflow_run_id", "status_url", "replayed"], "scan_acceptance"); const scan = decodeHealthScan(value.scan); assertWorkspace(scan, expectedWorkspaceId, "scan_acceptance.scan.workspace_id"); const healthScanId = uuid(value.health_scan_id, "scan_acceptance.health_scan_id"); const workflowRunId = uuid(value.workflow_run_id, "scan_acceptance.workflow_run_id"); const statusUrl = stringValue(value.status_url, "scan_acceptance.status_url"); if (scan.id !== healthScanId || scan.workflowRunId !== workflowRunId || scan.statusUrl !== statusUrl) throw invalidResponse("scan_acceptance.binding"); return { scan, healthScanId, workflowRunId, statusUrl, replayed: booleanValue(value.replayed, "scan_acceptance.replayed") }; }); };
export const decideHealthIssue = (input: DecideHealthIssueInput, signal?: AbortSignal): Promise<HealthIssue> => { const expectedWorkspaceId = requireWorkspace(input.workspaceId); const expectedIssueId = requireUuid(input.issueId, "issueId"); const reason = input.reason?.trim(); if (input.action === "DEFER" && (input.deferredUntil === undefined || reason === undefined || reason === "" || !isValidTimestamp(input.deferredUntil) || Date.parse(input.deferredUntil) <= Date.now())) throw invalidRequest("deferredUntil"); if ((input.action === "IGNORE" || input.action === "FALSE_POSITIVE") && (reason ?? "") === "") throw invalidRequest("reason"); return request(`/api/v1/health/issues/${encodeURIComponent(expectedIssueId)}/decisions?${params(expectedWorkspaceId)}`, { method: "POST", headers: { "Idempotency-Key": requireIdempotencyKey(input.idempotencyKey) }, body: JSON.stringify({ workspace_id: expectedWorkspaceId, expected_version: requireVersion(input.expectedVersion), action: input.action, ...(reason === undefined ? {} : { reason }), ...(input.deferredUntil === undefined ? {} : { deferred_until: input.deferredUntil }) }), ...signalInit(signal) }).then(decodeIssue).then((issue) => assertIssue(issue, expectedWorkspaceId, expectedIssueId)); };
export const createHealthRepairProposal = (input: CreateHealthRepairProposalInput, signal?: AbortSignal): Promise<never> => { const expectedWorkspaceId = requireWorkspace(input.workspaceId); const expectedIssueId = requireUuid(input.issueId, "issueId"); if (input.repairOptionCode.trim() === "" || !Number.isSafeInteger(input.expectedVersion) || input.expectedVersion < 1) throw invalidRequest("repairOption"); return request(`/api/v1/health/issues/${encodeURIComponent(expectedIssueId)}/repair-proposals?${params(expectedWorkspaceId)}`, { method: "POST", headers: { "Idempotency-Key": requireIdempotencyKey(input.idempotencyKey) }, body: JSON.stringify({ workspace_id: expectedWorkspaceId, expected_version: input.expectedVersion, repair_option_code: input.repairOptionCode }), ...signalInit(signal) }).then(() => { throw invalidResponse("repair_proposal"); }); };
