...
import { HealthApiError, createHealthRepairProposal, decodeHealthIssueDetail, decodeHealthScan, decodeHealthSummary, decideHealthIssue, getHealthIssue, getHealthSummary, listHealthIssues, startHealthScan } from "./health";
...
describe("Health API boundary", () => {
  ...
  it("lazy-decodes evidence detail and repair options", () => {
    const detail = decodeHealthIssueDetail({ issue, latest_observation: null, observations: [], decisions: [] });
    ...
    expect(() => decodeHealthIssueDetail({ issue: { ...issue, future: true }, observations: [], decisions: [] })).toThrow(HealthApiError);
  });
  ...
  it("binds issue list/detail to Workspace and requested issue", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn<typeof fetch>().mockResolvedValue(jsonResponse({ issue: { ...issue, id: "12000000-0000-4000-8000-000000000009" }, latest_observation: null, observations: [], decisions: [] })));
