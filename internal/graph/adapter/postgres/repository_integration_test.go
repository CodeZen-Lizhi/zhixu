...
func seedGraphProvenance(t *testing.T, ctx context.Context, tx graphSeedDB, workspaceID foundation.ID, now time.Time) graphProvenance {
	...
	statements := []struct {
		...
		{`INSERT INTO core.source_version(id,source_id,workspace_id,content_artifact_id,content_hash,byte_size,mime_type,original_content_location,security_status,captured_at) VALUES($1,$2,$3,$4,$5,4,'text/plain','graph.txt','pending',$6)`, []any{string(sourceVersionID), string(sourceID), string(workspaceID), string(artifactID), contentHash, now}},
