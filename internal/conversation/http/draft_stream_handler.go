...
func (handler *DraftStreamHandler) Routes(router gin.IRouter) {
	router.GET("/answers/:answer_id/stream", httpapi.GinHandler(handler.handle))
}
...
func parseDraftStreamRequest(request *http.Request) (agentapplication.DraftStreamReadQuery, *agentapplication.DraftStreamReadCursor, error) {
	...
	answerID, err := parseCanonicalDraftID(request.PathValue("answer_id"))
