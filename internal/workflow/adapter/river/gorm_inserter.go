...
func NewScopedTypedJobInserter[T riverlib.JobArgs](database *platformpostgres.Pool, client *Client, validate func(T) error, fence ScopedEnqueueFence) (*ScopedTypedJobInserter[T], error) {
	...
	if client == nil || isNilScopedRiverDependency(client.insert) || client.Queue() == "" || client.Schema() == "" {
	...
	if isNilScopedRiverDependency(fence) {
	...
	inner, err := riverlib.NewClient(driver, &riverlib.Config{Schema: client.Schema()})
	if err != nil {
...
func (i *ScopedTypedJobInserter[T]) InsertTx(ctx context.Context, scope foundation.TransactionScope, args T, options InsertOptions) (JobReceipt, error) {
	...
	if isNilScopedRiverDependency(i.fence) {
	...
	opts, err := buildScopedInsertOptions(ctx, i.queue, options)
	if err != nil {
		return JobReceipt{}, err
	}
...
func buildScopedInsertOptions(ctx context.Context, queue string, options InsertOptions) (*riverlib.InsertOpts, error) {
...
func isNilScopedRiverDependency(value any) bool {
	if value == nil {
		return true
	}
