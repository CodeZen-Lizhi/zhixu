...
func stageIncrementalSources(ctx context.Context, tx pgx.Tx, command domain.WorkspaceSnapshotCommand, activeID foundation.ID) error {
	...
	for {
		rows, err := tx.Query(ctx, `SELECT source_manifest.source_id::text,source_manifest.workspace_id::text,
				source_manifest.source_version_id::text,source_manifest.parse_projection_id::text,
				source_manifest.selection_status,source_manifest.exclusion_code,source_manifest.created_at
