...
func seedDeliveryRuntimeFixture(t *testing.T, ctx context.Context, database *pgxpool.Pool, prefix string) (foundation.ID, foundation.ID) {
	...
	batch.Queue(`INSERT INTO core.source_version(id,source_id,workspace_id,content_hash,byte_size,mime_type,original_content_location,security_status,captured_at,content_artifact_id)
		VALUES($1,$2,$3,$4,1,'text/markdown','runtime.md','passed',CURRENT_TIMESTAMP,$5)`, string(sourceVersionID), string(sourceID), string(workspaceID), strings.Repeat("a", 64), string(artifactID))
