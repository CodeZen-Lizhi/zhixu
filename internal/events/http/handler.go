...
func (handler *Handler) handleEvents(writer http.ResponseWriter, request *http.Request) {
	...
	workspaceID, cursorText, hasCursor, err := parseReplayRequest(request)
	if err != nil {
		writeError(writer, err)
		return
	}
	prepared, err := handler.prepare(request.Context(), workspaceID, cursorText, hasCursor)
	if err != nil {
		writeError(writer, err)
		return
	}
	...
	flusher.Flush()
	handler.stream(request.Context(), writer, flusher, workspaceID, prepared.afterSeq, prepared.continueDrain)
}
...
func (handler *Handler) prepare(ctx context.Context, workspaceID foundation.ID, cursorText string, hasCursor bool) (preparedStream, error) {
	...
	page, lastSequence, err := encodePage(events, workspaceID, afterSeq)
	if err != nil {
		return preparedStream{}, err
	}
...
func (handler *Handler) stream(ctx context.Context, writer http.ResponseWriter, flusher http.Flusher, workspaceID foundation.ID, afterSeq int64, continueDrain bool) {
	var err error
	if continueDrain {
		afterSeq, err = handler.drain(ctx, writer, flusher, workspaceID, afterSeq)
		if err != nil {
			return
		}
	}
...
func (handler *Handler) drain(ctx context.Context, writer http.ResponseWriter, flusher http.Flusher, workspaceID foundation.ID, afterSeq int64) (int64, error) {
	for {
...
func encodePage(events []domain.ServerEvent, workspaceID foundation.ID, afterSeq int64) ([]byte, int64, error) {
...
func parseReplayRequest(request *http.Request) (foundation.ID, string, bool, error) {
	query, err := url.ParseQuery(request.URL.RawQuery)
	if err != nil || len(query) != 1 {
		return "", "", false, workspaceInvalid()
	}
	...
	if len(values) != 1 || values[0] == "" {
		return "", "", false, foundation.NewError(foundation.ErrorInvalidInput, domain.ErrorCodeCursorInvalid, false, errors.New("Last-Event-ID must be unique and non-empty"))
	}
	return workspaceID, values[0], true, nil
}
