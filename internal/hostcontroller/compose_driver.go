...
// ComposeDriverOptions fixes every Docker scope before HTTP input is accepted.
...
func (driver *ComposeDriver) ApplyGrant(ctx context.Context, grant Grant) (_ *url.URL, returnErr error) {
	...
	if _, err := driver.compose(ctx, "up", "--detach", "--no-deps", "--force-recreate", "--wait", "app", "worker"); err != nil {
		return nil, runtimeFault("WORKSPACE_RUNTIME_START_FAILED")
	}
	...
	if _, err := driver.compose(ctx, "run", "--rm", "--no-deps", "-T", "firewall"); err != nil {
		return nil, runtimeFault("WORKSPACE_RUNTIME_FIREWALL_FAILED")
	}
	if _, err := driver.compose(ctx, "up", "--detach", "--no-deps", "--wait", "proxy"); err != nil {
		return nil, runtimeFault("WORKSPACE_RUNTIME_INGRESS_FAILED")
	}
	...
	if err != nil || current.Fingerprint.Digest() != grant.RootFingerprint || current.Fingerprint.BindingVersion != grant.BindingVersion {
		return nil, &ValidationError{Code: "WORKSPACE_PATH_IDENTITY_CHANGED"}
	}
	if err := driver.InspectGrant(ctx, grant); err != nil {
		return nil, err
	}
	backend, err := driver.CurrentBackend(ctx)
	if err != nil {
		return nil, err
	}
	return backend, nil
...
// one-off unprivileged candidate container. The Host Controller itself never
...
func candidateProbeFault(role string, err error) error {
	...
	case candidateprobe.ExitPathIdentity:
		return &Fault{Code: "WORKSPACE_PATH_IDENTITY_CHANGED", Message: "所选宿主机目录在验证期间发生了变化", Status: 409}
	case candidateprobe.ExitPathUnavailable:
		return &Fault{Code: "WORKSPACE_RUNTIME_ROOT_UNAVAILABLE", Message: "运行容器无法访问所选宿主机目录", Status: 409}
	case candidateprobe.ExitPermission:
		return &Fault{Code: "WORKSPACE_RUNTIME_ROOT_PERMISSION_DENIED", Message: "运行容器无权访问所选目录，请检查 Docker 文件共享和目录权限", Status: 409}
	case candidateprobe.ExitGitRequired:
		return &Fault{Code: "WORKSPACE_GIT_REQUIRED", Message: "所选目录不是 Git 仓库；如需创建，请勾选初始化 Git", Status: 409}
	case candidateprobe.ExitGitInvalid:
		return &Fault{Code: "WORKSPACE_GIT_INVALID", Message: "所选目录的 Git 仓库无法安全验证", Status: 409}
	case candidateprobe.ExitGitMetadataOutsideRoot:
		return &Fault{Code: "WORKSPACE_GIT_METADATA_OUTSIDE_ROOT", Message: "Git 元数据位于所选目录之外，无法按精确目录授权", Status: 409}
	case candidateprobe.ExitGitInitialize:
		return &Fault{Code: "WORKSPACE_GIT_INIT_FAILED", Message: "无法在所选目录中初始化 Git 仓库", Status: 409}
	case candidateprobe.ExitProbe:
		return &Fault{Code: "WORKSPACE_RUNTIME_ROOT_PROBE_FAILED", Message: "运行容器无法完成所选目录的读写验证", Status: 409}
	case candidateprobe.ExitDatabaseUnavailable:
		return &Fault{Code: "WORKSPACE_CANDIDATE_DATABASE_UNAVAILABLE", Message: "Workspace 候选运行时暂时无法连接数据库", Retryable: true, Status: 503}
	case candidateprobe.ExitRuntimeRegistration:
		return &Fault{Code: "WORKSPACE_CANDIDATE_REGISTRATION_FAILED", Message: "Workspace 候选运行时无法登记准备状态", Retryable: true, Status: 503}
	case candidateprobe.ExitConfiguration:
		return &Fault{Code: "WORKSPACE_CANDIDATE_CONFIGURATION_INVALID", Message: "Workspace 候选运行时配置无效", Status: 503}
	}
	if role == "worker" {
		return &Fault{Code: "WORKSPACE_WORKER_CANDIDATE_FAILED", Message: "Worker 无法验证所选宿主机目录", Status: 409}
	}
	return &Fault{Code: "WORKSPACE_API_CANDIDATE_FAILED", Message: "API 无法验证所选宿主机目录", Status: 409}
}
...
func (driver *ComposeDriver) prepareGrantModel(ctx context.Context, grant Grant) error {
	...
	if err != nil {
		_ = driver.removeGrantOverride()
		return &Fault{Code: "COMPOSE_MODEL_UNAVAILABLE", Message: "无法解析 Workspace 运行配置", Retryable: true, Status: 503}
	}
...
// CurrentBackend resolves only the fixed API namespace port and accepts only loopback output.
func (driver *ComposeDriver) CurrentBackend(ctx context.Context) (*url.URL, error) {
	output, err := driver.compose(ctx, "port", "--index", "1", "app", "8080")
	if err != nil {
	...
	if err != nil || host != "127.0.0.1" {
		return nil, runtimeFault("RUNTIME_BACKEND_INVALID")
	}
	port, err := strconv.ParseUint(rawPort, 10, 16)
	if err != nil || port == 0 {
		return nil, runtimeFault("RUNTIME_BACKEND_INVALID")
	}
	return &url.URL{Scheme: "http", Host: net.JoinHostPort(host, rawPort)}, nil
}
...
func runtimeFault(code string) error {
	return &Fault{Code: code, Message: "Workspace 运行时操作失败", Retryable: true, Status: 503}
}
