...
func TestRepositoryPostgreSQLSessionRotationIsAtomicAndConcurrentAuthenticationUpdatesLastSeen(t *testing.T) {
	runAuthIntegrationRepositoryCases(t, func(t *testing.T, pool *pgxpool.Pool, repository authIntegrationRepository) {
		...
		if _, err := repository.RotateSession(ctx, rotated.ID, sessionIssue(duplicate), time.Hour); errorCode(err) != "AUTH_CREDENTIAL_CONFLICT" {
			t.Fatalf("duplicate token hash rotation code=%s err=%v", errorCode(err), err)
		}
...
func runAuthIntegrationRepositoryCases(t *testing.T, test func(*testing.T, *pgxpool.Pool, authIntegrationRepository)) {
	...
	gormRepository, err := NewGORMRepository(gormDatabase)
	if err != nil {
		t.Fatal(err)
	}
...
func authIntegrationPool(t *testing.T) *platformpostgres.Pool {
	t.Helper()
