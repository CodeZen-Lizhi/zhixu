...
func (h *synthesisHistoricalRepublish) Resume(ctx context.Context, c app.ResumeSynthesisHistoricalRepublish) (app.SynthesisHistoricalRepublishReview, error) {
	var out app.SynthesisHistoricalRepublishReview
	if !validID(c.WorkspaceID) || !validID(c.NoteID) || !validID(c.AttemptID) {
		return out, manuscriptStoreInvalid("invalid historical recovery")
	}
	...
	err := h.within(ctx, func(ctx context.Context, scope foundation.TransactionScope, tx *gorm.DB) error {
		...
		if !reflect.DeepEqual(r, applied.Revision) || r.DocumentID != note.DocumentID || r.HistoricalRepublish == nil || r.HistoricalRepublish.AttemptID != c.AttemptID || applied.Command.WorkspaceID != c.WorkspaceID || applied.Command.NoteID != c.NoteID || applied.Command.AttemptID != c.AttemptID || p.NoteID != c.NoteID || p.RevisionID != r.ID || p.DocumentID != r.DocumentID || p.ArticleRevisionID != r.ArticleRevisionID || p.ContentHash != r.ContentHash || p.IdempotencyKey != "synthesis-publish:"+string(r.ID) {
			return synthesisConsistency("historical recovery result differs")
		}
		hash, err := authoringdomain.ComputePublishRequestHash(c.WorkspaceID, p.DocumentID, p.ArticleRevisionID)
		if err != nil {
			return err
		}
		base := attempt.Capture.ContentHash
		if !attempt.Capture.Exists {
			base = attempt.Capture.AbsenceToken
		}
		var exact bool
		err = tx.Raw(`SELECT EXISTS(SELECT 1 FROM authoring.document_publication_reservation WHERE id=? AND workspace_id=? AND document_id=? AND article_revision_id=? AND content_hash=? AND idempotency_key=? AND request_hash=? AND merge_capture_id=? AND historical_republish_id=? AND merge_receipt_id IS NULL AND base_version=? AND merge_published_revision_id IS NOT DISTINCT FROM NULLIF(?,'')::uuid AND merge_published_content_hash IS NOT DISTINCT FROM NULLIF(?,''))`, string(applied.ReservationID), string(c.WorkspaceID), string(p.DocumentID), string(p.ArticleRevisionID), p.ContentHash, p.IdempotencyKey, hash, string(attempt.Capture.ID), string(applied.ApplyID), base, string(attempt.Command.ExpectedPublishedRevisionID), attempt.PublishedContentHash).Scan(&exact).Error
		if err != nil {
			return err
		}
		if !exact {
			return synthesisConsistency("historical recovery reservation differs")
		}
	...
	if b.WorkspaceID != c.WorkspaceID || b.DocumentID != p.DocumentID || b.ArticleRevisionID != p.ArticleRevisionID || b.ContentHash != p.ContentHash {
		return out, synthesisConsistency("historical publication result differs")
	}
