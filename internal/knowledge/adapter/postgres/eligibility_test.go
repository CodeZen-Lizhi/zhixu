...
func TestEvidenceEligibilitySQLScopesConflictsToRequestedDisputedClaims(t *testing.T) {
	...
	normalized := strings.Join(strings.Fields(gormEvidenceEligibilitySQL), " ")
	requiredFragments := []string{
		...
		"FROM requested_disputed_claims requested_claim JOIN core.conflict_member member ON member.workspace_id=? AND member.claim_id=requested_claim.claim_id",
