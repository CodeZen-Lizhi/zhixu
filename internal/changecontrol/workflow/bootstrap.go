...
func validateBootstrapExecutionBinding(execution workflowapplication.ExecutionContext, input BootstrapInput, proposal domain.Proposal, durable domain.WritebackExecution, expectedKey string) error {
	if durable.ID == "" || durable.WorkspaceID != execution.WorkspaceID || durable.WorkflowRunID != execution.RunID || durable.NodeRunID != execution.NodeRunID || durable.ProposalID != input.ProposalID || durable.RevisionID != input.RevisionID || durable.ApprovalID != proposal.Approval.ID || durable.WriteAuthorizationID == "" || durable.GitAuthorizationID == "" || durable.WriteAuthorizationID == durable.GitAuthorizationID || durable.TargetPath != proposal.TargetPath || !strings.EqualFold(durable.BaseHash, proposal.Revision.BaseHash) || !strings.EqualFold(durable.ResultHash, domain.ComputeWritebackResultHash([]byte(proposal.Revision.Content))) || !strings.EqualFold(durable.ApprovedChangeHash, input.ApprovedChangeHash) || !strings.EqualFold(durable.ApprovedGitHead, *proposal.Approval.ApprovedGitHead) || durable.IdempotencyKey != expectedKey {
		return bootstrapBindingConflict(domain.ErrWritebackIdentityConflict)
	}
	return nil
}
