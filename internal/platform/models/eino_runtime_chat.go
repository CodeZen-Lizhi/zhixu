...
func NewEinoRuntimeChatModel(options OpenAIChatOptions) (*EinoRuntimeChatModel, error) {
	...
	if config.client.Timeout <= 0 || options.Timeout < config.client.Timeout {
		config.client.Timeout = options.Timeout
	}
	config.client.Transport = &einoRuntimeRoundTripper{
		base: config.client.Transport, model: config.contract.Model.ModelVersion,
