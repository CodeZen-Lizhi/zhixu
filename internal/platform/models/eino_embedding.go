...
func NewEinoOpenAICompatibleEmbedder(options OpenAIEmbeddingOptions) (*EinoEmbedder, error) {
	...
	config.client.client.Transport = &einoEmbeddingRoundTripper{
...
func NewEinoOllamaEmbedder(options OllamaEmbeddingOptions) (*EinoEmbedder, error) {
	...
	moveEinoEmbeddingClientTimeoutToCallContext(&config)
	config.client.client.Transport = &einoEmbeddingRoundTripper{
		base: config.client.client.Transport, provider: ollamaProvider, maxResponseBytes: config.maxResponseBytes,
	}
	truncate := false
	backend, err := einoollama.NewEmbedder(context.Background(), &einoollama.EmbeddingConfig{
		HTTPClient: config.client.client, BaseURL: strings.TrimSuffix(config.endpointURL, "/api/embed"),
...
func (embedder *EinoEmbedder) closeModelResource() error {
	if embedder == nil {
		return nil
	}
...
func moveEinoEmbeddingClientTimeoutToCallContext(config *embeddingHTTPConfig) {
	if config == nil || config.client == nil || config.client.client == nil {
		return
	}
	if clientTimeout := config.client.client.Timeout; clientTimeout > 0 && clientTimeout < config.timeout {
		config.timeout = clientTimeout
	}
	config.client.client.Timeout = 0
}
...
func (transport *einoEmbeddingRoundTripper) RoundTrip(request *http.Request) (*http.Response, error) {
	...
	if response.StatusCode < http.StatusOK || response.StatusCode >= http.StatusMultipleChoices {
		diagnostic := providerResponseDiagnostic(response, request.Header.Get("Authorization"), request.URL.String())
		_ = response.Body.Close()
		state.failure = classifyEmbeddingStatus(response.StatusCode, diagnostic)
	...
	if err != nil {
		return nil, failEinoEmbeddingState(state, classifyEmbeddingTransportErrorWithStage(request.Context(), err, ConnectionStageResponseRead))
	}
