...
func newRAGSmokeChatRelay(chatBaseURL string, suppliedClient *http.Client) (*RAGSmokeChatRelay, error) {
	...
	endpoint, err := url.Parse(appendChatPath(baseURL, chatCompletionsPath))
	if err != nil {
		return nil, errChatConfigInvalid
	}
...
// Close releases the idle connections owned by the relay transport.
func (relay *RAGSmokeChatRelay) Close() error {
