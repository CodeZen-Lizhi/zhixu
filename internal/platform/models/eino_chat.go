...
func NewEinoOpenAIChatModel(options OpenAIChatOptions, telemetry ...ModelTelemetry) (*EinoOpenAIChatModel, error) {
	...
	config.client.client.Transport = &einoChatRoundTripper{
		base:             config.client.client.Transport,
...
func (model *EinoOpenAIChatModel) closeModelResource() error {
	if model == nil {
		return nil
	}
...
// ProbeConnection verifies endpoint, credential, model, and strict JSON-schema
// support through the same Eino Chat path used by production structured calls.
func (model *EinoOpenAIChatModel) ProbeConnection(ctx context.Context) error {
	if model == nil {
	...
	_, err := model.Chat(ctx, agentapplication.ChatRequest{
		...
		PromptRef:  agentdomain.PromptRef{ID: "connection-probe", Version: "v1"},
		SchemaRef:  agentdomain.SchemaRef{ID: "connection-probe", Version: "v1"},
		Model:      model.http.contract.Model,
...
func (model *EinoOpenAIChatModel) String() string {
	...
	return "eino-backed " + safeChatAdapterString(model.http.contract)
}
...
func (transport *einoChatRoundTripper) RoundTrip(request *http.Request) (*http.Response, error) {
	...
	if response.StatusCode < http.StatusOK || response.StatusCode >= http.StatusMultipleChoices {
		diagnostic := providerResponseDiagnosticBounded(
			response, request.Header.Get("Authorization"), request.URL.String(), maxEinoChatErrorBodyBytes,
		)
		_ = response.Body.Close()
		return nil, &einoChatStatusError{statusCode: response.StatusCode, diagnostic: diagnostic}
	}
...
func classifyEinoChatError(ctx context.Context, err error, wireValidated bool) error {
	...
	if errors.As(err, &statusError) {
		return classifyChatStatus(statusError.statusCode, statusError.diagnostic)
	}
	var readError *einoChatReadError
	if errors.As(err, &readError) {
		return classifyChatTransportErrorWithStage(ctx, readError, ConnectionStageResponseRead)
	}
