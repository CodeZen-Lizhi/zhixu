...
func TestNewRAGSmokeChatRelayRequiresHTTPSAndProductionTransport(t *testing.T) {
	...
	transport, ok := relay.client.client.Transport.(*http.Transport)
	if !ok || transport.Proxy != nil || transport.TLSClientConfig == nil || transport.TLSClientConfig.MinVersion < tls.VersionTLS12 {
		t.Fatalf("transport=%#v", relay.client.client.Transport)
	}
	if err := relay.client.client.CheckRedirect(nil, nil); err != http.ErrUseLastResponse {
...
func TestRAGSmokeRelayTestTransportHasTLSMinimum(t *testing.T) {
	...
	relay, err := newRAGSmokeChatRelay(server.URL, client)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRAGSmokeRelayCloseLeavesExternalTransportOpen(t *testing.T) {
	t.Parallel()

	external := &countingIdleTransport{}
	relay, err := newRAGSmokeChatRelay("https://chat.example.test", &http.Client{Transport: external})
	if err != nil {
		t.Fatal(err)
	}
	if err := relay.Close(); err != nil {
