...
Trigger: Codex SubagentStart; PreToolUse before Task dispatch on other platforms
...
def _resolve_repo_path(base_path: str, file_path: object) -> tuple[Path, str] | None:
    """Resolve a repo-relative path without allowing traversal or symlink escape."""
    ...
    try:
        repo_root = Path(base_path).resolve()
...
def _read_file_bytes(base_path: str, file_path: str) -> bytes | None:
    """Read a repo-contained file's raw bytes, or return None when unavailable."""
    resolved_path = _resolve_repo_path(base_path, file_path)
    if resolved_path is None:
        return None

    full_path, _ = resolved_path
...
def _materialize_directory(
    ...
    resolved_path = _resolve_repo_path(base_path, dir_path)
    if resolved_path is None:
        return []
...
def read_jsonl_entries(base_path: str, jsonl_path: str) -> list[dict]:
    ...
    """
    resolved_path = _resolve_repo_path(base_path, jsonl_path)
    if resolved_path is None:
        print(
            f"[inject-subagent-context] WARN: rejected unsafe context path: {jsonl_path}",
            file=sys.stderr,
        )
        return []
            ...
            for line in f:
                ...
                try:
                    ...
                    if not isinstance(item, dict):
                        print(
                            f"[inject-subagent-context] WARN: ignored non-object entry in {jsonl_path}",
                            file=sys.stderr,
                        )
                        continue
                    ...
                    if "file" not in item:
                        if "path" in item:
                            print(
                                f"[inject-subagent-context] WARN: ignored unsupported path alias in {jsonl_path}; use file",
                                file=sys.stderr,
                            )
                    ...
                    if not isinstance(file_path, str) or not file_path:
                        print(
                            f"[inject-subagent-context] WARN: ignored entry without a valid file in {jsonl_path}",
                            file=sys.stderr,
                        )
                        continue
                    ...
                    if not isinstance(entry_type, str) or entry_type not in {"file", "directory"}:
                        print(
                            f"[inject-subagent-context] WARN: ignored entry with invalid type in {jsonl_path}: {entry_type!r}",
                            file=sys.stderr,
                        )
                        continue

                    resolved_entry = _resolve_repo_path(base_path, file_path)
                    if resolved_entry is None:
                        print(
                            f"[inject-subagent-context] WARN: ignored unsafe context entry in {jsonl_path}: {file_path}",
                            file=sys.stderr,
                        )
                        continue

                    _, relative_path = resolved_entry
                    saw_real_entry = True
                    entries.append(
                        {
                            "file": relative_path,
