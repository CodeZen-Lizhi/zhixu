...
DO $$
BEGIN
    LOCK TABLE retrieval.index_activation,
    ...
    IF EXISTS (SELECT 1 FROM retrieval.index_activation)
       OR EXISTS (SELECT 1 FROM retrieval.chunk_projection)
       OR EXISTS (SELECT 1 FROM retrieval.index_manifest_chunk)
       OR EXISTS (SELECT 1 FROM retrieval.index_version)
       OR EXISTS (SELECT 1 FROM retrieval.embedding_version) THEN
        RAISE EXCEPTION 'cannot downgrade Retrieval Index Foundation while retrieval business data exists'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
DELETE FROM core.schema_meta WHERE key = 'retrieval_index_foundation';

DROP TRIGGER IF EXISTS retrieval_index_activation_reject_mutation ON retrieval.index_activation;
DROP TRIGGER IF EXISTS retrieval_index_activation_verify_commit ON retrieval.index_activation;
DROP TRIGGER IF EXISTS retrieval_index_activation_validate_insert ON retrieval.index_activation;
DROP FUNCTION IF EXISTS retrieval.verify_index_activation_commit();
DROP FUNCTION IF EXISTS retrieval.validate_index_activation_insert();
DROP TABLE IF EXISTS retrieval.index_activation;

DROP TRIGGER IF EXISTS retrieval_index_version_reject_delete ON retrieval.index_version;
DROP TRIGGER IF EXISTS retrieval_index_version_validate_write ON retrieval.index_version;
DROP FUNCTION IF EXISTS retrieval.validate_index_version_write();

DROP TRIGGER IF EXISTS retrieval_projection_reject_delete ON retrieval.chunk_projection;
DROP TRIGGER IF EXISTS retrieval_projection_validate_write ON retrieval.chunk_projection;
DROP FUNCTION IF EXISTS retrieval.validate_projection_write();
DROP TABLE IF EXISTS retrieval.chunk_projection;

DROP TRIGGER IF EXISTS retrieval_manifest_reject_mutation ON retrieval.index_manifest_chunk;
DROP TRIGGER IF EXISTS retrieval_manifest_validate_insert ON retrieval.index_manifest_chunk;
DROP FUNCTION IF EXISTS retrieval.validate_manifest_chunk_insert();
DROP TABLE IF EXISTS retrieval.index_manifest_chunk;

DROP TABLE IF EXISTS retrieval.index_version;

DROP TRIGGER IF EXISTS retrieval_embedding_version_reject_mutation ON retrieval.embedding_version;
DROP TABLE IF EXISTS retrieval.embedding_version;
DROP FUNCTION IF EXISTS retrieval.reject_immutable_mutation();

DROP INDEX IF EXISTS ingestion.idx_ingestion_canonical_chunk_content_trgm;
ALTER TABLE ingestion.canonical_chunk
    DROP CONSTRAINT IF EXISTS uq_canonical_chunk_id_workspace;
