...
DO $$
BEGIN
    LOCK TABLE retrieval.embedding_cache,
               retrieval.reindex_delivery,
               retrieval.index_version,
               retrieval.embedding_version
        IN ACCESS EXCLUSIVE MODE;
    IF EXISTS (SELECT 1 FROM retrieval.embedding_cache)
       OR EXISTS (
            SELECT 1 FROM retrieval.reindex_delivery
             WHERE regression_code = 'SNAPSHOT_STRUCTURE_V2'
       )
       OR EXISTS (
            SELECT 1 FROM retrieval.index_version
             WHERE embedding_version_id IS NOT NULL
       ) THEN
        RAISE EXCEPTION 'cannot downgrade Embedding Hybrid Search while business data exists' USING ERRCODE = '55000';
    END IF;
END;
$$;
-- +goose StatementEnd

DELETE FROM core.schema_meta WHERE key = 'embedding_hybrid_search';
...
ALTER TABLE retrieval.reindex_delivery
    DROP CONSTRAINT IF EXISTS reindex_delivery_regression_code_check,
    ADD CONSTRAINT reindex_delivery_regression_code_check CHECK (
        regression_code IS NULL OR regression_code = 'SNAPSHOT_STRUCTURE_V1'
    );

DROP TRIGGER IF EXISTS retrieval_embedding_cache_reject_mutation ON retrieval.embedding_cache;
DROP TRIGGER IF EXISTS retrieval_embedding_cache_validate_insert ON retrieval.embedding_cache;
DROP FUNCTION IF EXISTS retrieval.validate_embedding_cache_statement();
DROP TABLE IF EXISTS retrieval.embedding_cache;
