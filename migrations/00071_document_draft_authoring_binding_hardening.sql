...
BEGIN
    IF EXISTS (SELECT 1 FROM authoring.document_publication_binding LIMIT 1)
       ...
       OR EXISTS (SELECT 1 FROM authoring.working_draft LIMIT 1) THEN
        RAISE EXCEPTION 'authoring facts exist; cannot weaken publication binding checks'
            USING ERRCODE='55000';
    END IF;
END
$$;
...
-- Restore the exact 00069 Article Revision mutation contract.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION authoring.validate_article_revision_mutation()
RETURNS trigger
LANGUAGE plpgsql
...
-- Restore the exact 00069 Document publication guard. The 00071 variant also
-- compares proposal_commit.target_mode and must not survive this downgrade.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION authoring.guard_document_publication_path()
RETURNS trigger
LANGUAGE plpgsql
...
-- Restore the exact 00069 Publication binding contract.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION authoring.validate_publication_binding_write()
RETURNS trigger
LANGUAGE plpgsql
