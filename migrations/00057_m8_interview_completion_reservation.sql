...
LOCK TABLE learning.interview_completion_reservation IN ACCESS EXCLUSIVE MODE;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'interview completion reservations exist; rollback would discard recovery state'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF EXISTS (
        SELECT 1
         ...
         WHERE disposition <> 'ACTIVE'
            OR attempt_digest IS NOT NULL
         LIMIT 1
    ) THEN
        RAISE EXCEPTION 'interview artifact holds contain orphan recovery state; rollback is unsafe'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF EXISTS (
        SELECT 1
          FROM learning.artifact_visibility_hold
         GROUP BY workspace_id, owner_type, owner_id, owner_role
        HAVING count(*) > 1
    ) THEN
        RAISE EXCEPTION 'interview artifact holds cannot restore owner-role uniqueness'
            USING ERRCODE = '55000';
    END IF;
END
$$;
...
DROP INDEX learning.idx_learning_artifact_visibility_hold_orphan_recovery;
DROP INDEX learning.uq_learning_artifact_visibility_hold_active_owner_role;

DROP TRIGGER IF EXISTS trg_learning_interview_artifact_visibility_hold_guard
    ON learning.artifact_visibility_hold;
DROP FUNCTION IF EXISTS learning.guard_interview_artifact_visibility_hold();

ALTER TABLE learning.artifact_visibility_hold
    ADD CONSTRAINT uq_learning_artifact_visibility_hold_owner_role
        UNIQUE (workspace_id, owner_type, owner_id, owner_role),
    DROP CONSTRAINT learning_artifact_visibility_hold_active_digest_check,
    DROP CONSTRAINT learning_artifact_visibility_hold_disposition_check,
    DROP CONSTRAINT learning_artifact_visibility_hold_digest_check,
    DROP COLUMN disposition,
    DROP COLUMN attempt_digest;

DROP TABLE learning.interview_completion_reservation;

ALTER TABLE learning.interview_learning_path
    DROP CONSTRAINT uq_learning_interview_path_completion_binding;

ALTER TABLE learning.interview_report
    DROP CONSTRAINT uq_learning_interview_report_completion_binding;
