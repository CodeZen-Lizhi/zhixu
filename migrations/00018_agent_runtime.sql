...
DO $$
BEGIN
    LOCK TABLE agent.model_call, agent.model_run IN ACCESS EXCLUSIVE MODE;
    IF EXISTS (SELECT 1 FROM agent.model_call) OR EXISTS (SELECT 1 FROM agent.model_run) THEN
        RAISE EXCEPTION 'cannot downgrade Agent Runtime while model history exists' USING ERRCODE = '55000';
    END IF;
END;
$$;
-- +goose StatementEnd

DELETE FROM core.schema_meta WHERE key = 'agent_runtime';

DROP TRIGGER IF EXISTS agent_model_call_guard_mutation ON agent.model_call;
DROP FUNCTION IF EXISTS agent.guard_model_call_mutation();
DROP TABLE IF EXISTS agent.model_call;

DROP TRIGGER IF EXISTS agent_model_run_guard_mutation ON agent.model_run;
DROP FUNCTION IF EXISTS agent.guard_model_run_mutation();
DROP TABLE IF EXISTS agent.model_run;
