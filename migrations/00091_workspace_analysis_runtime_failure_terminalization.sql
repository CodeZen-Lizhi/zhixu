    ...
    IF EXISTS (
        SELECT 1
         ...
         WHERE runtime_terminal_at IS NOT NULL
           AND reason='WORKSPACE_ANALYSIS_RUNTIME_FAILED'
         LIMIT 1
    ) THEN
        RAISE EXCEPTION 'runtime workspace analysis failure facts exist; rollback is unsafe'
            USING ERRCODE='55000';
    END IF;
END;
$$;
...
DROP TRIGGER workspace_analysis_runtime_failure_answer_guard_update ON agent.answer;
DROP TRIGGER agent_answer_guard_mutation_update ON agent.answer;
DROP TRIGGER agent_answer_guard_mutation_insert_delete ON agent.answer;
CREATE TRIGGER agent_answer_guard_mutation
    BEFORE INSERT OR UPDATE OR DELETE ON agent.answer
    FOR EACH ROW EXECUTE FUNCTION agent.guard_answer_mutation();
DROP FUNCTION agent.guard_workspace_analysis_runtime_failure_answer();

DROP TRIGGER IF EXISTS workspace_analysis_runtime_failure_guard_insert
    ON agent.workspace_analysis_termination_proof;
DROP FUNCTION IF EXISTS agent.guard_workspace_analysis_runtime_failure_insert();
...
WHERE key='workspace_analysis_runtime_failure_terminalization';
