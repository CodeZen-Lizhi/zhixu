...
-- 00041 can represent only PENDING/COMPLETED and requires one historical row
-- per source-section. Refuse downgrade whenever restoring that contract would
-- discard a terminal outcome or collapse multiple receipts.
-- +goose StatementBegin
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
         ...
         WHERE status NOT IN ('PENDING', 'COMPLETED')
            ...
            OR (status = 'COMPLETED' AND terminal_at IS DISTINCT FROM completed_at)
         LIMIT 1
    ) OR EXISTS (
        SELECT 1
          FROM learning.artifact_section_generation
         GROUP BY workspace_id, artifact_id, source_revision_id, section_key
        HAVING count(*) > 1
    ) THEN
        RAISE EXCEPTION 'cannot restore artifact generation source uniqueness with recovery history'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
DROP INDEX IF EXISTS learning.uq_learning_artifact_section_generation_active_section;

ALTER TABLE learning.artifact_section_generation
    DROP CONSTRAINT IF EXISTS learning_artifact_section_generation_terminal_check,
    DROP CONSTRAINT IF EXISTS learning_artifact_section_generation_time_check,
    DROP CONSTRAINT IF EXISTS learning_artifact_section_generation_error_summary_check,
    DROP CONSTRAINT IF EXISTS learning_artifact_section_generation_error_code_check,
    DROP CONSTRAINT IF EXISTS learning_artifact_section_generation_failure_class_check,
    DROP CONSTRAINT IF EXISTS artifact_section_generation_status_check;
...
ALTER TABLE learning.artifact_section_generation
    ...
    ADD CONSTRAINT uq_learning_artifact_section_generation_source
