...
-- +goose Down
DROP TRIGGER IF EXISTS workflow_definition_reject_update_delete ON workflow.definition;
DROP FUNCTION IF EXISTS workflow.reject_definition_mutation();
DROP TABLE IF EXISTS workflow.outbox_event;
DROP TABLE IF EXISTS workflow.human_task;
DROP TABLE IF EXISTS workflow.node_run;
DROP TABLE IF EXISTS workflow.run;
DROP TABLE IF EXISTS workflow.definition;
DROP SCHEMA IF EXISTS workflow;
DELETE FROM core.schema_meta WHERE key = 'workflow';
