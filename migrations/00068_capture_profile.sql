...
LOCK TABLE core.capture,
           core.capture_command,
           ops.capture_outbox,
           ops.capture_attempt,
           learning.document_knowledge_profile,
           learning.document_knowledge_profile_revision,
           learning.document_knowledge_profile_attempt,
           learning.document_knowledge_profile_evidence,
           ops.model_settings_state,
           ingestion.source_span,
           agent.model_run,
           core.source_version,
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM core.capture LIMIT 1)
       OR EXISTS (SELECT 1 FROM core.capture_command LIMIT 1)
       OR EXISTS (SELECT 1 FROM ops.capture_outbox LIMIT 1)
       OR EXISTS (SELECT 1 FROM ops.capture_attempt LIMIT 1)
       OR EXISTS (SELECT 1 FROM learning.document_knowledge_profile LIMIT 1)
       OR EXISTS (SELECT 1 FROM learning.document_knowledge_profile_revision LIMIT 1)
       OR EXISTS (SELECT 1 FROM learning.document_knowledge_profile_attempt LIMIT 1)
       OR EXISTS (SELECT 1 FROM learning.document_knowledge_profile_evidence LIMIT 1)
       OR EXISTS (
            SELECT 1 FROM ingestion.source_span
             WHERE evidence_kind<>'raw_bytes' OR derived_excerpt<>''
            LIMIT 1
       )
       OR EXISTS (
            SELECT 1 FROM agent.model_run
             WHERE final_result_type='document_knowledge_profile'
            LIMIT 1
       ) THEN
        RAISE EXCEPTION 'capture/profile facts exist; rollback is unsafe' USING ERRCODE='55000';
    END IF;
END
$$;
-- +goose StatementEnd

DELETE FROM core.schema_meta WHERE key='capture_profile';

DROP TRIGGER IF EXISTS document_profile_model_settings_stale ON ops.model_settings_state;
DROP FUNCTION IF EXISTS learning.mark_document_profiles_stale_on_model_settings_activation();
DROP TRIGGER IF EXISTS document_profile_index_activation_stale ON retrieval.index_activation;
DROP FUNCTION IF EXISTS learning.mark_document_profiles_stale_on_index_activation();
DROP TRIGGER IF EXISTS document_profile_evidence_validate ON learning.document_knowledge_profile_evidence;
DROP FUNCTION IF EXISTS learning.validate_document_profile_evidence();
DROP TRIGGER IF EXISTS document_profile_evidence_reject_truncate ON learning.document_knowledge_profile_evidence;
DROP TRIGGER IF EXISTS document_profile_evidence_append_only ON learning.document_knowledge_profile_evidence;
DROP TRIGGER IF EXISTS document_profile_revision_reject_truncate ON learning.document_knowledge_profile_revision;
DROP TRIGGER IF EXISTS document_profile_revision_append_only ON learning.document_knowledge_profile_revision;
DROP TRIGGER IF EXISTS capture_command_reject_truncate ON core.capture_command;
DROP TRIGGER IF EXISTS capture_command_append_only ON core.capture_command;
DROP FUNCTION IF EXISTS learning.reject_capture_immutable_mutation();
DROP TRIGGER IF EXISTS document_profile_validate_write ON learning.document_knowledge_profile;
DROP FUNCTION IF EXISTS learning.validate_document_profile_write();
DROP TRIGGER IF EXISTS capture_validate_write ON core.capture;
DROP FUNCTION IF EXISTS core.validate_capture_write();

DROP TABLE IF EXISTS learning.document_knowledge_profile_evidence;
DROP TABLE IF EXISTS learning.document_knowledge_profile_attempt;
ALTER TABLE learning.document_knowledge_profile
    DROP CONSTRAINT IF EXISTS fk_document_profile_current_revision;
DROP TABLE IF EXISTS learning.document_knowledge_profile_revision;
DROP TABLE IF EXISTS learning.document_knowledge_profile;
DROP TABLE IF EXISTS ops.capture_attempt;
DROP TABLE IF EXISTS ops.capture_outbox;
DROP TABLE IF EXISTS core.capture_command;
DROP TABLE IF EXISTS core.capture;

ALTER TABLE core.source_version
    DROP CONSTRAINT IF EXISTS uq_source_version_id_workspace;

ALTER TABLE ingestion.source_span
    DROP CONSTRAINT IF EXISTS source_span_evidence_shape,
    DROP COLUMN IF EXISTS derived_excerpt,
