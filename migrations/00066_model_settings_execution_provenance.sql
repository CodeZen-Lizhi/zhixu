...
LOCK TABLE retrieval.embedding_version,
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM retrieval.embedding_version
        ...
        RAISE EXCEPTION 'model settings execution provenance must be archived before rollback'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
DELETE FROM core.schema_meta WHERE key = 'model_settings_execution_provenance';

DROP TRIGGER IF EXISTS agent_model_run_model_settings_revision_guard ON agent.model_run;
DROP FUNCTION IF EXISTS agent.enforce_model_run_model_settings_revision();
DROP TRIGGER IF EXISTS retrieval_index_model_settings_revision_guard ON retrieval.index_version;
DROP FUNCTION IF EXISTS retrieval.enforce_index_model_settings_revision();
DROP TRIGGER IF EXISTS retrieval_embedding_model_settings_revision_guard ON retrieval.embedding_version;
DROP FUNCTION IF EXISTS retrieval.enforce_embedding_model_settings_revision();

ALTER TABLE agent.model_run
    DROP CONSTRAINT IF EXISTS agent_model_run_model_settings_revision,
    DROP COLUMN IF EXISTS model_settings_revision;
...
ALTER TABLE retrieval.embedding_version
    ...
    DROP COLUMN IF EXISTS model_settings_revision,
    ADD CONSTRAINT uq_retrieval_embedding_version_contract
        UNIQUE (provider, model, dimensions, config_hash);
