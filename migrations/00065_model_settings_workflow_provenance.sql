...
LOCK TABLE workflow.node_attempt IN ACCESS EXCLUSIVE MODE;
...
BEGIN
    IF EXISTS (
        ...
        WHERE model_settings_revision IS NOT NULL
           OR model_runtime_instance_id IS NOT NULL
        LIMIT 1
    ) THEN
        RAISE EXCEPTION 'workflow model runtime provenance must be archived before rollback'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
DROP TRIGGER IF EXISTS workflow_node_attempt_model_runtime_binding_guard ON workflow.node_attempt;
DROP FUNCTION IF EXISTS workflow.enforce_node_attempt_model_runtime_binding();

ALTER TABLE workflow.node_attempt
    DROP CONSTRAINT IF EXISTS workflow_node_attempt_model_runtime_binding,
    DROP COLUMN IF EXISTS model_runtime_instance_id,
