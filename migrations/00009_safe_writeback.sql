...
DROP TRIGGER IF EXISTS writeback_reindex_guard_update ON workflow.outbox_event;
DROP TRIGGER IF EXISTS writeback_reindex_validate_insert ON workflow.outbox_event;
DROP FUNCTION IF EXISTS change_control.guard_writeback_reindex_mutation();
DROP FUNCTION IF EXISTS change_control.validate_writeback_reindex_insert();
DROP TRIGGER IF EXISTS proposal_commit_reject_update_delete ON change_control.proposal_commit;
DROP TRIGGER IF EXISTS proposal_commit_verify_binding ON change_control.proposal_commit;
DROP FUNCTION IF EXISTS change_control.verify_proposal_commit_binding();
DROP TABLE IF EXISTS change_control.proposal_commit;
DROP TRIGGER IF EXISTS writeback_execution_reject_delete ON change_control.writeback_execution;
DROP TRIGGER IF EXISTS writeback_execution_validate_transition ON change_control.writeback_execution;
DROP TRIGGER IF EXISTS writeback_execution_verify_binding ON change_control.writeback_execution;
DROP FUNCTION IF EXISTS change_control.reject_writeback_execution_delete();
DROP FUNCTION IF EXISTS change_control.validate_writeback_execution_transition();
DROP FUNCTION IF EXISTS change_control.verify_writeback_execution_binding();
DROP TABLE IF EXISTS change_control.writeback_execution;
DROP TRIGGER IF EXISTS proposal_validate_transition ON change_control.proposal;
DROP FUNCTION IF EXISTS change_control.validate_proposal_transition();
ALTER TABLE change_control.approval DROP CONSTRAINT IF EXISTS approval_git_head_format;
ALTER TABLE change_control.approval DROP COLUMN IF EXISTS approved_git_head;
ALTER TABLE change_control.proposal DROP CONSTRAINT IF EXISTS proposal_version_positive;
ALTER TABLE change_control.proposal DROP COLUMN IF EXISTS version;
DELETE FROM core.schema_meta WHERE key = 'safe_writeback';
