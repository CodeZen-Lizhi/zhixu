...
-- Removing these guards while protected learning facts exist would let an old
-- writer reinterpret an Interview shell as Review, attach Review answers to an
-- Interview, or mutate frozen provenance. Keep the forward migration applied
-- until those facts are explicitly archived.
LOCK TABLE learning.review_session, learning.interview_session,
    learning.interview_question, learning.review_answer IN ACCESS EXCLUSIVE MODE;
...
BEGIN
    IF EXISTS (SELECT 1 FROM learning.interview_session LIMIT 1)
       ...
       OR EXISTS (SELECT 1 FROM learning.review_answer LIMIT 1) THEN
        RAISE EXCEPTION 'interview provenance or learning session facts exist; rollback is unsafe'
            USING ERRCODE = '55000';
    END IF;
END
$$;
...
DROP TRIGGER IF EXISTS trg_learning_review_session_learning_binding ON learning.review_session;
DROP TRIGGER IF EXISTS trg_learning_interview_session_binding ON learning.interview_session;
DROP FUNCTION IF EXISTS learning.guard_review_answer_session_binding();
DROP FUNCTION IF EXISTS learning.guard_review_session_learning_binding();
DROP FUNCTION IF EXISTS learning.guard_interview_session_shell_binding();
DROP TRIGGER IF EXISTS trg_learning_interview_question_evidence ON learning.interview_question;
DROP FUNCTION IF EXISTS learning.assert_interview_question_evidence();
DROP FUNCTION IF EXISTS learning.try_interview_evidence_uuid(text);
