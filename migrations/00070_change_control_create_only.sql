...
DO $$
BEGIN
    -- Close the guard/DDL race: no CREATE_ONLY writer may commit between this
    -- snapshot and the later column drops in the same migration transaction.
    LOCK TABLE change_control.proposal_revision,
               change_control.writeback_execution,
               change_control.proposal_commit,
               change_control.tool_authorization
        IN ACCESS EXCLUSIVE MODE;
    IF EXISTS (SELECT 1 FROM change_control.proposal_revision WHERE target_mode='CREATE_ONLY')
       OR EXISTS (SELECT 1 FROM change_control.writeback_execution WHERE target_mode='CREATE_ONLY')
       OR EXISTS (SELECT 1 FROM change_control.proposal_commit WHERE target_mode='CREATE_ONLY')
       OR EXISTS (SELECT 1 FROM change_control.tool_authorization WHERE target_mode='CREATE_ONLY') THEN
        RAISE EXCEPTION 'cannot downgrade while create only change control facts exist' USING ERRCODE='55000';
    END IF;
END;
$$;
-- +goose StatementEnd

DROP TRIGGER IF EXISTS writeback_execution_verify_target_mode ON change_control.writeback_execution;
DROP TRIGGER IF EXISTS proposal_revision_validate_create_only_binding ON change_control.proposal_revision;
DROP FUNCTION IF EXISTS change_control.verify_writeback_execution_target_mode();
DROP FUNCTION IF EXISTS change_control.validate_create_only_revision_binding();

-- 回滚时恢复 00069 之前的函数定义；空库 Down 后不得留下对 target_mode 的引用。
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION change_control.verify_tool_authorization_binding()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
    ...
    SELECT p.workspace_id,p.status,r.target_path,r.proposal_id,r.base_hash,r.change_hash,
     ...
     FOR UPDATE OF p,r,a;
    IF proposal_workspace IS NULL OR proposal_workspace<>NEW.workspace_id OR proposal_status<>'approved'
       OR NEW.scope<>'target:' || proposal_target_path OR revision_proposal<>NEW.proposal_id
       OR approval_proposal<>NEW.proposal_id OR approval_revision<>NEW.revision_id
       OR approval_decision<>'approved' OR revision_change_hash<>NEW.approved_change_hash
       OR approval_change_hash<>NEW.approved_change_hash OR revision_base_hash<>NEW.target_version THEN
        RAISE EXCEPTION 'tool authorization approval binding mismatch' USING ERRCODE='23514';
...
BEGIN
    IF NEW.id<>OLD.id OR NEW.workspace_id<>OLD.workspace_id OR NEW.workflow_run_id<>OLD.workflow_run_id
       OR NEW.node_run_id<>OLD.node_run_id OR NEW.proposal_id<>OLD.proposal_id OR NEW.revision_id<>OLD.revision_id
       OR NEW.approval_id<>OLD.approval_id OR NEW.tool_name<>OLD.tool_name OR NEW.capability<>OLD.capability
       OR NEW.scope<>OLD.scope OR NEW.approved_change_hash<>OLD.approved_change_hash
       OR NEW.target_version<>OLD.target_version OR NEW.token_hash<>OLD.token_hash OR NEW.idempotency_key<>OLD.idempotency_key
       OR NEW.issued_at<>OLD.issued_at OR NEW.expires_at<>OLD.expires_at OR NEW.version<>OLD.version+1
       OR OLD.status<>'issued' OR NEW.status NOT IN ('consumed','revoked','expired') THEN
        RAISE EXCEPTION 'tool authorization immutable field or transition violation' USING ERRCODE='23514';
...
BEGIN
    SELECT * INTO execution FROM change_control.writeback_execution WHERE id=NEW.writeback_execution_id FOR UPDATE;
    SELECT p.status,r.proposal_id,r.target_path,r.change_hash,a.proposal_id,a.revision_id,a.change_hash,a.decision,a.approved_git_head
    ...
    IF execution.id IS NULL OR execution.status NOT IN ('git_committed','publish_recovery_required','verifying')
       ...
       OR NEW.revision_id<>execution.revision_id OR NEW.approval_id<>execution.approval_id
       OR NEW.git_commit<>execution.git_commit OR NEW.parent_git_commit<>execution.parent_git_commit
       OR NEW.target_path<>execution.target_path OR NEW.diff_hash<>execution.diff_hash OR NEW.result_hash<>execution.result_hash
       OR proposal_status NOT IN ('applied','verifying') OR revision_proposal<>execution.proposal_id
       OR revision_target<>execution.target_path OR revision_change_hash<>execution.approved_change_hash
       OR approval_proposal<>execution.proposal_id OR approval_revision<>execution.revision_id
       OR approval_change_hash<>execution.approved_change_hash OR approval_decision<>'approved'
       OR approval_git_head IS NULL OR approval_git_head<>execution.approved_git_head THEN
        RAISE EXCEPTION 'proposal commit binding mismatch' USING ERRCODE='23514';
...
BEGIN
    ...
    IF NOT (
        (OLD.status='verifying' AND NEW.status='verifying'
         ...
         AND NEW.diff_hash IS NOT DISTINCT FROM OLD.diff_hash
         AND NEW.completed_at IS NOT DISTINCT FROM OLD.completed_at)
        OR (OLD.status='prepared' AND (NEW.status IN ('file_prepared','needs_revision','apply_failed')
            OR (NEW.status='file_applied' AND OLD.temporary_ref IS NOT NULL AND OLD.backup_ref IS NOT NULL)))
        OR (OLD.status='file_prepared' AND NEW.status IN ('file_applied','needs_revision','apply_failed','manual_recovery_required'))
        OR (OLD.status='file_applied' AND (NEW.status IN ('git_prepared','compensating_file','manual_recovery_required')
    ...
    IF OLD.git_commit IS NOT NULL AND (NEW.git_commit IS DISTINCT FROM OLD.git_commit OR NEW.parent_git_commit IS DISTINCT FROM OLD.parent_git_commit OR NEW.diff_hash IS DISTINCT FROM OLD.diff_hash) THEN
        RAISE EXCEPTION 'writeback git result is immutable' USING ERRCODE='23514';
    END IF;
    IF OLD.base_blob_id IS NOT NULL AND (NEW.base_blob_id IS DISTINCT FROM OLD.base_blob_id OR NEW.result_blob_id IS DISTINCT FROM OLD.result_blob_id OR NEW.base_mode IS DISTINCT FROM OLD.base_mode OR NEW.diff_hash IS DISTINCT FROM OLD.diff_hash) THEN
        RAISE EXCEPTION 'writeback git intent is immutable' USING ERRCODE='23514';
    END IF;
    IF OLD.file_lock_token IS NOT NULL AND (NEW.temporary_ref IS DISTINCT FROM OLD.temporary_ref OR NEW.backup_ref IS DISTINCT FROM OLD.backup_ref OR NEW.file_byte_size IS DISTINCT FROM OLD.file_byte_size OR NEW.file_mode IS DISTINCT FROM OLD.file_mode OR NEW.file_lock_token IS DISTINCT FROM OLD.file_lock_token OR NEW.file_result_lock_token IS DISTINCT FROM OLD.file_result_lock_token OR NEW.file_backup_lock_token IS DISTINCT FROM OLD.file_backup_lock_token) THEN
        RAISE EXCEPTION 'writeback file lock binding is immutable' USING ERRCODE='23514';
    ...
    IF OLD.completed_at IS NOT NULL AND NEW.completed_at IS DISTINCT FROM OLD.completed_at THEN
        RAISE EXCEPTION 'writeback completion time is immutable' USING ERRCODE='23514';
    END IF;
    IF NEW.status='file_prepared' AND (NEW.temporary_ref IS NULL OR NEW.backup_ref IS NULL OR NEW.file_byte_size IS NULL
        OR NEW.file_mode IS NULL OR NEW.file_lock_token IS NULL OR NEW.file_result_lock_token IS NULL OR NEW.file_backup_lock_token IS NULL) THEN
        RAISE EXCEPTION 'file_prepared requires durable file intent' USING ERRCODE='23514';
    END IF;
    IF NEW.status='git_prepared' AND (NEW.base_blob_id IS NULL OR NEW.result_blob_id IS NULL OR NEW.base_mode IS NULL OR NEW.diff_hash IS NULL) THEN
        RAISE EXCEPTION 'git_prepared requires durable git intent' USING ERRCODE='23514';
    END IF;
    RETURN NEW;
END;
...
ALTER TABLE change_control.proposal_commit DROP CONSTRAINT IF EXISTS proposal_commit_target_mode_check;
ALTER TABLE change_control.proposal_commit DROP COLUMN IF EXISTS target_mode;
ALTER TABLE change_control.tool_authorization DROP CONSTRAINT IF EXISTS tool_authorization_target_mode_check;
ALTER TABLE change_control.tool_authorization DROP COLUMN IF EXISTS target_mode;
ALTER TABLE change_control.writeback_execution
	DROP CONSTRAINT IF EXISTS writeback_git_intent_status_valid,
	ADD CONSTRAINT writeback_git_intent_status_valid CHECK (
    ...
    DROP CONSTRAINT IF EXISTS writeback_file_prepared_fields,
    ADD CONSTRAINT writeback_file_prepared_fields CHECK (
        status<>'file_prepared' OR (temporary_ref IS NOT NULL AND backup_ref IS NOT NULL AND file_byte_size IS NOT NULL
            AND file_mode IS NOT NULL AND file_lock_token IS NOT NULL AND file_result_lock_token IS NOT NULL AND file_backup_lock_token IS NOT NULL)
    ),
    DROP CONSTRAINT IF EXISTS writeback_file_managed_lock_tokens_valid,
    ADD CONSTRAINT writeback_file_managed_lock_tokens_valid CHECK (
        (file_result_lock_token IS NULL AND file_backup_lock_token IS NULL)
    ...
    DROP CONSTRAINT IF EXISTS writeback_execution_create_only_git_intent,
    DROP CONSTRAINT IF EXISTS writeback_execution_create_only_lock_fields,
    DROP CONSTRAINT IF EXISTS writeback_execution_create_only_file_fields,
    DROP CONSTRAINT IF EXISTS writeback_execution_base_version_check,
    DROP CONSTRAINT IF EXISTS writeback_execution_target_mode_check,
    ADD CONSTRAINT writeback_execution_base_hash_check CHECK (base_hash ~ '^[0-9a-f]{64}$'),
    DROP COLUMN IF EXISTS target_mode;
ALTER TABLE change_control.proposal_revision
    DROP CONSTRAINT IF EXISTS proposal_revision_base_version_check,
    DROP CONSTRAINT IF EXISTS proposal_revision_target_mode_check,
    ADD CONSTRAINT proposal_revision_base_hash_check CHECK (base_hash ~ '^[0-9a-f]{64}$'),
    DROP COLUMN IF EXISTS target_mode;
