...
BEGIN
    IF EXISTS (SELECT 1 FROM authoring.document_publication_binding LIMIT 1)
       OR EXISTS (SELECT 1 FROM authoring.document_publication_reservation LIMIT 1)
       OR EXISTS (SELECT 1 FROM authoring.working_draft_command LIMIT 1)
       OR EXISTS (SELECT 1 FROM authoring.working_draft LIMIT 1) THEN
        RAISE EXCEPTION 'authoring draft, revision, or publication history exists; archive before rollback'
            USING ERRCODE='55000';
    END IF;
END
$$;
...
DROP TRIGGER IF EXISTS authoring_publication_reject_truncate ON authoring.document_publication_binding;
DROP TRIGGER IF EXISTS authoring_publication_validate_write ON authoring.document_publication_binding;
DROP TRIGGER IF EXISTS authoring_publication_reservation_reject_truncate ON authoring.document_publication_reservation;
DROP TRIGGER IF EXISTS authoring_publication_reservation_validate_write ON authoring.document_publication_reservation;
DROP TRIGGER IF EXISTS authoring_document_guard_publication_path ON core.document;
DROP TRIGGER IF EXISTS authoring_command_reject_truncate ON authoring.working_draft_command;
DROP TRIGGER IF EXISTS authoring_command_append_only ON authoring.working_draft_command;
DROP TRIGGER IF EXISTS authoring_working_draft_validate_write ON authoring.working_draft;
DROP TRIGGER IF EXISTS authoring_article_revision_reject_truncate ON core.article_revision;
DROP TRIGGER IF EXISTS authoring_article_revision_validate_mutation ON core.article_revision;
DROP FUNCTION IF EXISTS authoring.validate_publication_binding_write();
DROP FUNCTION IF EXISTS authoring.validate_publication_reservation_write();
DROP FUNCTION IF EXISTS authoring.guard_document_publication_path();
DROP FUNCTION IF EXISTS authoring.validate_article_revision_mutation();
DROP FUNCTION IF EXISTS authoring.reject_immutable_mutation();
DROP FUNCTION IF EXISTS authoring.validate_working_draft_write();
DROP TABLE IF EXISTS authoring.document_publication_binding;
DROP TABLE IF EXISTS authoring.document_publication_reservation;
DROP TABLE IF EXISTS authoring.working_draft_command;
DROP INDEX IF EXISTS authoring.uq_authoring_working_draft_document;
DROP TABLE IF EXISTS authoring.working_draft;
ALTER TABLE core.article_revision
    DROP CONSTRAINT IF EXISTS fk_core_article_revision_parent_authoring_binding;
ALTER TABLE core.document
    DROP CONSTRAINT IF EXISTS fk_core_document_current_revision_authoring_binding;
ALTER TABLE core.article_revision
    DROP CONSTRAINT IF EXISTS uq_core_article_revision_authoring_binding;
DROP SCHEMA IF EXISTS authoring;
