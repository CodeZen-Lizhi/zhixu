...
BEGIN
    IF EXISTS (
        SELECT 1
         ...
         WHERE reason='WORKSPACE_ANALYSIS_DEADLINE_EXCEEDED'
           AND operation_id IS NULL
         LIMIT 1
    ) THEN
        RAISE EXCEPTION 'pre-operation workspace analysis deadline facts exist; rollback is unsafe'
            USING ERRCODE='55000';
    END IF;
END;
$$;
...
DROP TRIGGER IF EXISTS workspace_analysis_preoperation_deadline_guard_insert
    ON agent.workspace_analysis_termination_proof;
...
DROP FUNCTION agent.guard_workspace_analysis_preoperation_deadline_insert();
DROP FUNCTION agent.workspace_analysis_deadline_next_slot(uuid);

ALTER TABLE agent.workspace_analysis_termination_proof
    DROP CONSTRAINT IF EXISTS workspace_analysis_termination_proof_deadline_slot,
    DROP COLUMN deadline_operation_kind,
    DROP COLUMN deadline_operation_ordinal;

DELETE FROM core.schema_meta
WHERE key='workspace_analysis_deadline_terminalization';
