...
LOCK TABLE core.workspace, ops.workspace_root_binding_history IN ACCESS EXCLUSIVE MODE;

-- +goose StatementBegin
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM ops.workspace_root_binding_history LIMIT 1) THEN
        RAISE EXCEPTION 'workspace root binding history exists; forward fix required' USING ERRCODE='55000';
    END IF;
END
$$;
-- +goose StatementEnd
...
DROP TRIGGER IF EXISTS workspace_root_binding_history_reject_truncate ON ops.workspace_root_binding_history;
DROP TRIGGER IF EXISTS workspace_root_binding_history_guard ON ops.workspace_root_binding_history;
DROP TRIGGER IF EXISTS workspace_root_binding_history_commit_guard ON ops.workspace_root_binding_history;
DROP FUNCTION IF EXISTS ops.enforce_workspace_root_binding_history_commit();
DROP FUNCTION IF EXISTS ops.enforce_workspace_root_binding_history();
DROP TABLE IF EXISTS ops.workspace_root_binding_history;
ALTER TABLE ops.audit_event DROP COLUMN transaction_id;
