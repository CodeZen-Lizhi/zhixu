...
LOCK TABLE ops.model_settings_state, ops.model_settings_revisions, ops.model_settings_runtime
    IN ACCESS EXCLUSIVE MODE;

-- +goose StatementBegin
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM ops.model_settings_revisions LIMIT 1)
       OR EXISTS (SELECT 1 FROM ops.model_settings_runtime LIMIT 1)
       OR EXISTS (
            SELECT 1 FROM ops.model_settings_state
            WHERE desired_revision<>0 OR active_revision<>0 OR phase<>'idle' OR version<>1
            LIMIT 1
       ) THEN
        RAISE EXCEPTION 'model settings history must be archived before rollback' USING ERRCODE='55000';
    END IF;
END
$$;
-- +goose StatementEnd

DROP TRIGGER IF EXISTS model_settings_runtime_reject_truncate ON ops.model_settings_runtime;
DROP TRIGGER IF EXISTS model_settings_runtime_guard ON ops.model_settings_runtime;
DROP FUNCTION IF EXISTS ops.enforce_model_settings_runtime();
DROP TABLE IF EXISTS ops.model_settings_runtime;
DROP TRIGGER IF EXISTS model_settings_state_reject_truncate ON ops.model_settings_state;
DROP TRIGGER IF EXISTS model_settings_state_guard ON ops.model_settings_state;
DROP FUNCTION IF EXISTS ops.enforce_model_settings_state();
DROP TABLE IF EXISTS ops.model_settings_state;
DROP FUNCTION IF EXISTS ops.model_settings_revision_exists(bigint);
DROP TRIGGER IF EXISTS model_settings_revisions_reject_truncate ON ops.model_settings_revisions;
DROP TRIGGER IF EXISTS model_settings_revisions_append_only ON ops.model_settings_revisions;
DROP FUNCTION IF EXISTS ops.reject_model_settings_revision_mutation();
DROP TABLE IF EXISTS ops.model_settings_revisions;
DROP SEQUENCE IF EXISTS ops.model_settings_revision_seq;
