...
DROP TRIGGER IF EXISTS review_card_project_invalidation_health_update ON learning.review_card;
DROP TRIGGER IF EXISTS review_card_project_invalidation_health_insert ON learning.review_card;

DROP FUNCTION IF EXISTS learning.project_review_card_invalidation_health_update_statement();
DROP FUNCTION IF EXISTS learning.project_review_card_invalidation_health_insert_statement();
DROP FUNCTION IF EXISTS learning.sync_review_invalidation_health_batch(uuid[],uuid[],timestamptz[]);

-- Restore the original row-level projection owned by 00049. Rolling back this
-- optimization changes performance only; the lifecycle and projection facts
-- remain identical.
DROP TRIGGER IF EXISTS review_card_project_invalidation_health ON learning.review_card;
...
DROP TRIGGER IF EXISTS review_card_sync_evidence_selector ON learning.review_card;
DROP FUNCTION IF EXISTS learning.sync_review_card_evidence_selector();
DROP TABLE IF EXISTS learning.review_card_evidence_selector;
DROP INDEX IF EXISTS learning.idx_learning_review_card_invalidated_claim;
DROP INDEX IF EXISTS learning.idx_learning_review_card_approved_claim;
