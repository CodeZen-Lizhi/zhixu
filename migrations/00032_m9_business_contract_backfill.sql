...
-- The preceding UPDATE is intentionally in the same Goose transaction so no
...
-- Refuse the intermediate downgrade before Goose advances its version marker;
-- otherwise a later 00030 Down could silently discard the backfilled facts.
-- +goose StatementBegin
DO $$
BEGIN
