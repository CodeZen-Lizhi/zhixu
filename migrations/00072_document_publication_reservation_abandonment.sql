...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM authoring.document_publication_reservation LIMIT 1) THEN
        RAISE EXCEPTION 'publication reservation facts exist; cannot remove abandonment state'
            USING ERRCODE='55000';
    END IF;
END
$$;
...
-- Restore the exact pre-00072 reservation trigger contract before dropping
-- the abandonment columns.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION authoring.validate_publication_reservation_write()
RETURNS trigger
LANGUAGE plpgsql
...
ALTER TABLE authoring.document_publication_reservation
    DROP CONSTRAINT authoring_reservation_state_shape,
    DROP CONSTRAINT authoring_reservation_status_allowed,
    DROP COLUMN abandoned_at,
    DROP COLUMN error_code,
    ADD CONSTRAINT document_publication_reservation_status_check CHECK (status IN ('PENDING','CLOSED')),
    ADD CONSTRAINT authoring_reservation_state_shape CHECK (
        updated_at>=created_at
