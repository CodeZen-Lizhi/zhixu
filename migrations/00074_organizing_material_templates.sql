...
LOCK TABLE learning.artifact_revision_document_source,
           learning.artifact_revision,
           agent.model_run,
           organizing.command_receipt,
           organizing.generation,
           organizing.run_result,
           organizing.workflow_start_outbox,
           organizing.run_binding,
           organizing.workflow_input_material,
           organizing.draft_material,
           organizing.workflow_input_snapshot,
           organizing.draft_version,
           organizing.template_revision,
           organizing.template,
           organizing.draft
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM learning.artifact_revision_document_source LIMIT 1)
       OR EXISTS (
           SELECT 1 FROM learning.artifact_revision
           WHERE domain_schema_version='artifact-revision/v2'
       )
       OR EXISTS (
           SELECT 1 FROM agent.model_run
           WHERE retrieval_index_version_id IS NULL
             AND output_schema_id IN ('organizing.outline-generation','organizing.document-generation')
       )
       OR EXISTS (SELECT 1 FROM organizing.command_receipt LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.generation LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.run_result LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.workflow_start_outbox LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.run_binding LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.workflow_input_material LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.draft_material LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.workflow_input_snapshot LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.draft_version LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.template_revision LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.template LIMIT 1)
       OR EXISTS (SELECT 1 FROM organizing.draft LIMIT 1) THEN
        RAISE EXCEPTION 'organizing facts exist; rollback is unsafe' USING ERRCODE='55000';
    END IF;
END
$$;
-- +goose StatementEnd

DROP TRIGGER IF EXISTS artifact_revision_project_document_sources ON learning.artifact_revision;
DROP TRIGGER IF EXISTS artifact_revision_document_source_reject_truncate
    ON learning.artifact_revision_document_source;
DROP TRIGGER IF EXISTS artifact_revision_document_source_append_only
    ON learning.artifact_revision_document_source;
DROP FUNCTION IF EXISTS learning.project_artifact_revision_document_sources();
DROP FUNCTION IF EXISTS learning.reject_artifact_revision_document_source_mutation();
DROP INDEX IF EXISTS learning.idx_learning_artifact_document_source_document;
DROP TABLE IF EXISTS learning.artifact_revision_document_source;
...
-- Restore the 00061 RAG-memory Model Run guard before removing organizing,
-- because the v74 definition references organizing.generation.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION agent.guard_model_run_mutation()
RETURNS trigger
LANGUAGE plpgsql
...
DROP TRIGGER IF EXISTS organizing_receipt_reject_truncate ON organizing.command_receipt;
DROP TRIGGER IF EXISTS organizing_receipt_append_only ON organizing.command_receipt;
DROP TRIGGER IF EXISTS organizing_run_result_reject_truncate ON organizing.run_result;
DROP TRIGGER IF EXISTS organizing_run_result_append_only ON organizing.run_result;
DROP TRIGGER IF EXISTS organizing_run_result_validate_insert ON organizing.run_result;
DROP TRIGGER IF EXISTS organizing_generation_reject_truncate ON organizing.generation;
DROP TRIGGER IF EXISTS organizing_generation_validate_write ON organizing.generation;
DROP TRIGGER IF EXISTS organizing_run_binding_reject_truncate ON organizing.run_binding;
DROP TRIGGER IF EXISTS organizing_run_binding_append_only ON organizing.run_binding;
DROP TRIGGER IF EXISTS organizing_run_binding_completion_validate ON organizing.run_binding;
DROP TRIGGER IF EXISTS organizing_run_binding_validate_insert ON organizing.run_binding;
DROP TRIGGER IF EXISTS organizing_snapshot_material_origin_validate ON organizing.workflow_input_material;
DROP TRIGGER IF EXISTS organizing_snapshot_material_reject_truncate ON organizing.workflow_input_material;
DROP TRIGGER IF EXISTS organizing_snapshot_material_append_only ON organizing.workflow_input_material;
DROP TRIGGER IF EXISTS organizing_snapshot_reject_truncate ON organizing.workflow_input_snapshot;
DROP TRIGGER IF EXISTS organizing_snapshot_append_only ON organizing.workflow_input_snapshot;
DROP TRIGGER IF EXISTS organizing_draft_material_reject_truncate ON organizing.draft_material;
DROP TRIGGER IF EXISTS organizing_draft_material_append_only ON organizing.draft_material;
DROP TRIGGER IF EXISTS organizing_draft_version_reject_truncate ON organizing.draft_version;
DROP TRIGGER IF EXISTS organizing_draft_version_append_only ON organizing.draft_version;
DROP TRIGGER IF EXISTS organizing_template_revision_reject_truncate ON organizing.template_revision;
DROP TRIGGER IF EXISTS organizing_template_revision_append_only ON organizing.template_revision;
DROP TRIGGER IF EXISTS organizing_start_outbox_validate_write ON organizing.workflow_start_outbox;
DROP TRIGGER IF EXISTS organizing_snapshot_validate_insert ON organizing.workflow_input_snapshot;
DROP TRIGGER IF EXISTS organizing_template_current_revision_validate ON organizing.template;
DROP TRIGGER IF EXISTS organizing_template_revision_validate_insert ON organizing.template_revision;
DROP TRIGGER IF EXISTS organizing_template_validate_write ON organizing.template;
DROP TRIGGER IF EXISTS organizing_draft_current_version_validate ON organizing.draft;
DROP TRIGGER IF EXISTS organizing_draft_version_validate_insert ON organizing.draft_version;
DROP TRIGGER IF EXISTS organizing_draft_validate_write ON organizing.draft;

ALTER TABLE organizing.draft DROP CONSTRAINT IF EXISTS fk_organizing_draft_confirmed_snapshot;
ALTER TABLE organizing.draft DROP CONSTRAINT IF EXISTS fk_organizing_draft_current_version;
ALTER TABLE organizing.draft DROP CONSTRAINT IF EXISTS fk_organizing_draft_template_revision;
ALTER TABLE organizing.draft_version DROP CONSTRAINT IF EXISTS fk_organizing_draft_version_confirmed_snapshot;
ALTER TABLE organizing.draft_version DROP CONSTRAINT IF EXISTS fk_organizing_draft_version_template_revision;
ALTER TABLE organizing.template DROP CONSTRAINT IF EXISTS fk_organizing_template_current_revision;

DROP TABLE IF EXISTS organizing.command_receipt;
DROP TABLE IF EXISTS organizing.generation;
DROP TABLE IF EXISTS organizing.run_result;
DROP TABLE IF EXISTS organizing.workflow_start_outbox;
DROP TABLE IF EXISTS organizing.run_binding;
DROP TABLE IF EXISTS organizing.workflow_input_material;
DROP TABLE IF EXISTS organizing.draft_material;
DROP TABLE IF EXISTS organizing.workflow_input_snapshot;
DROP TABLE IF EXISTS organizing.draft_version;
DROP TABLE IF EXISTS organizing.template_revision;
DROP TABLE IF EXISTS organizing.template;
DROP TABLE IF EXISTS organizing.draft;

DROP FUNCTION IF EXISTS organizing.validate_start_outbox_write();
DROP FUNCTION IF EXISTS organizing.validate_generation_write();
DROP FUNCTION IF EXISTS organizing.validate_run_result_insert();
DROP FUNCTION IF EXISTS organizing.validate_run_binding_completion();
DROP FUNCTION IF EXISTS organizing.validate_run_binding_insert();
DROP FUNCTION IF EXISTS organizing.validate_snapshot_material_origin();
DROP FUNCTION IF EXISTS organizing.validate_snapshot_insert();
DROP FUNCTION IF EXISTS organizing.validate_template_current_revision();
DROP FUNCTION IF EXISTS organizing.validate_template_revision_insert();
DROP FUNCTION IF EXISTS organizing.validate_template_write();
DROP FUNCTION IF EXISTS organizing.validate_draft_current_version();
DROP FUNCTION IF EXISTS organizing.validate_draft_version_insert();
DROP FUNCTION IF EXISTS organizing.validate_draft_write();
DROP FUNCTION IF EXISTS organizing.reject_immutable_mutation();
DROP FUNCTION IF EXISTS organizing.valid_reason_codes(text[]);
DROP FUNCTION IF EXISTS organizing.valid_material_shape(text,uuid,uuid,uuid,uuid,uuid,uuid,uuid,bigint,text,text,text,jsonb,boolean);
DROP FUNCTION IF EXISTS organizing.valid_evidence(jsonb);
DROP SCHEMA IF EXISTS organizing;

DELETE FROM core.schema_meta WHERE key='organizing_material_templates';
