...
DO $$
BEGIN
    LOCK TABLE retrieval.reindex_delivery_attempt,
               retrieval.reindex_delivery,
               retrieval.index_manifest_source,
               retrieval.index_version
        IN ACCESS EXCLUSIVE MODE;
    IF EXISTS (SELECT 1 FROM retrieval.reindex_delivery_attempt)
       OR EXISTS (SELECT 1 FROM retrieval.reindex_delivery)
       OR EXISTS (SELECT 1 FROM retrieval.index_manifest_source)
       OR EXISTS (
            SELECT 1 FROM retrieval.index_version
             WHERE source_manifest_hash IS NOT NULL OR expected_source_count IS NOT NULL
                OR source_parser_id IS NOT NULL OR source_parser_version IS NOT NULL
                OR source_parser_config_hash IS NOT NULL OR source_chunk_strategy_version IS NOT NULL
                OR source_schema_version IS NOT NULL
       ) THEN
        RAISE EXCEPTION 'cannot downgrade Reindex Consumer while business data exists' USING ERRCODE = '55000';
    END IF;
END;
$$;
-- +goose StatementEnd

DELETE FROM core.schema_meta WHERE key = 'reindex_consumer';

DROP INDEX IF EXISTS workflow.idx_workflow_outbox_reindex_unpublished;

DROP TRIGGER IF EXISTS proposal_verify_reindex_completion ON change_control.proposal;
DROP TRIGGER IF EXISTS writeback_execution_verify_reindex_completion ON change_control.writeback_execution;
DROP TRIGGER IF EXISTS reindex_delivery_verify_completion ON retrieval.reindex_delivery;
DROP FUNCTION IF EXISTS retrieval.verify_reindex_completion_trigger();
DROP FUNCTION IF EXISTS retrieval.assert_reindex_completion(uuid);

DROP TRIGGER IF EXISTS reindex_delivery_attempt_reject_delete ON retrieval.reindex_delivery_attempt;
DROP TRIGGER IF EXISTS reindex_delivery_attempt_validate_write ON retrieval.reindex_delivery_attempt;
DROP FUNCTION IF EXISTS retrieval.validate_reindex_attempt_write();
ALTER TABLE retrieval.reindex_delivery DROP CONSTRAINT IF EXISTS fk_reindex_delivery_current_attempt;
DROP TABLE IF EXISTS retrieval.reindex_delivery_attempt;

DROP TRIGGER IF EXISTS reindex_delivery_validate_write ON retrieval.reindex_delivery;
DROP FUNCTION IF EXISTS retrieval.validate_reindex_delivery_write();
DROP TABLE IF EXISTS retrieval.reindex_delivery;

DROP TRIGGER IF EXISTS retrieval_source_manifest_reject_mutation ON retrieval.index_manifest_source;
DROP TRIGGER IF EXISTS retrieval_source_manifest_validate_insert ON retrieval.index_manifest_source;
DROP FUNCTION IF EXISTS retrieval.validate_manifest_source_statement();
DROP TABLE IF EXISTS retrieval.index_manifest_source;

DROP TRIGGER IF EXISTS retrieval_manifest_validate_insert ON retrieval.index_manifest_chunk;
DROP FUNCTION IF EXISTS retrieval.validate_manifest_chunk_statement();

-- Restore the M6-A row trigger before removing the exact composite FK.
CREATE TRIGGER retrieval_manifest_validate_insert
...
DROP INDEX IF EXISTS ingestion.idx_ingestion_attempt_reindex_selection;

-- Restore the M6-A function before dropping source-bound columns.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION retrieval.validate_index_version_write()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
    ...
    IF NEW.id <> OLD.id OR NEW.workspace_id <> OLD.workspace_id
       ...
       OR NEW.tokenizer_config_hash <> OLD.tokenizer_config_hash OR NEW.fusion_config <> OLD.fusion_config
       OR NEW.source_snapshot_ref <> OLD.source_snapshot_ref OR NEW.manifest_hash <> OLD.manifest_hash
       OR NEW.expected_chunk_count <> OLD.expected_chunk_count OR NEW.idempotency_key <> OLD.idempotency_key
...
ALTER TABLE retrieval.index_version
    DROP CONSTRAINT IF EXISTS retrieval_index_source_manifest_pair,
    DROP COLUMN IF EXISTS source_schema_version,
    DROP COLUMN IF EXISTS source_chunk_strategy_version,
    DROP COLUMN IF EXISTS source_parser_config_hash,
    DROP COLUMN IF EXISTS source_parser_version,
    DROP COLUMN IF EXISTS source_parser_id,
    DROP COLUMN IF EXISTS expected_source_count,
    DROP COLUMN IF EXISTS source_manifest_hash;

ALTER TABLE change_control.writeback_execution DROP CONSTRAINT IF EXISTS uq_writeback_execution_id_workspace;
ALTER TABLE workflow.outbox_event DROP CONSTRAINT IF EXISTS uq_workflow_outbox_id_workspace;
ALTER TABLE ingestion.source_version_projection DROP CONSTRAINT IF EXISTS uq_source_version_projection_workspace;
ALTER TABLE ingestion.parse_projection DROP CONSTRAINT IF EXISTS uq_parse_projection_id_workspace;
ALTER TABLE core.source_version DROP CONSTRAINT IF EXISTS uq_source_version_id_source;
ALTER TABLE core.source DROP CONSTRAINT IF EXISTS uq_source_id_workspace;
