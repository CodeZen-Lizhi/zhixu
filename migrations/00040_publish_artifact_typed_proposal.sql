...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM change_control.proposal WHERE proposal_type = 'publish_artifact') THEN
        RAISE EXCEPTION 'publish artifact proposals exist; cannot downgrade' USING ERRCODE = '55000';
    END IF;
END;
$$;
-- +goose StatementEnd
...
DROP TRIGGER IF EXISTS approval_validate_typed_proposal ON change_control.approval;
DROP FUNCTION IF EXISTS change_control.validate_typed_proposal_approval();
...
CREATE TRIGGER proposal_revision_validate_payload
    BEFORE INSERT ON change_control.proposal_revision
    FOR EACH ROW EXECUTE FUNCTION change_control.validate_proposal_revision_payload();
-- Restore the prior M9 transition contract; it cannot represent this type,
-- but the guard above proved no publish_artifact row exists before downgrade.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION change_control.validate_proposal_transition()
RETURNS trigger
LANGUAGE plpgsql
...
ALTER TABLE change_control.proposal_revision
    DROP CONSTRAINT IF EXISTS ck_proposal_revision_artifact_source_coverage,
    DROP CONSTRAINT IF EXISTS ck_proposal_revision_artifact_content_hash,
    DROP CONSTRAINT IF EXISTS ck_proposal_revision_artifact_version,
    DROP CONSTRAINT IF EXISTS ck_proposal_revision_artifact_revision_no,
    DROP COLUMN IF EXISTS artifact_source_coverage,
    DROP COLUMN IF EXISTS artifact_content_hash,
    DROP COLUMN IF EXISTS artifact_version,
    DROP COLUMN IF EXISTS artifact_revision_no,
    DROP COLUMN IF EXISTS artifact_revision_id,
