...
-- Git capture checkpoints and tombstones are immutable audit facts. Dropping
-- either would make a later replay invent a different current-source view.
-- +goose StatementBegin
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM core.workspace_git_capture_checkpoint)
       OR EXISTS (SELECT 1 FROM core.source WHERE removed_at IS NOT NULL) THEN
        RAISE EXCEPTION 'workspace Git capture facts exist; 00077 down is unsafe' USING ERRCODE='55000';
    END IF;
END;
$$;
-- +goose StatementEnd
...
DROP TRIGGER IF EXISTS workspace_git_capture_checkpoint_update_guard ON core.workspace_git_capture_checkpoint;
DROP FUNCTION IF EXISTS core.validate_workspace_git_capture_checkpoint_update();
DROP TABLE IF EXISTS core.workspace_git_capture_checkpoint;
DROP INDEX IF EXISTS core.idx_source_workspace_active_id;
ALTER TABLE core.source DROP COLUMN IF EXISTS removed_at;
