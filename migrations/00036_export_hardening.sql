...
-- Export request/result/download facts cannot be represented by the base
-- 00034 shape. Require an explicit archive/delete step before rollback.
-- +goose StatementBegin
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM ops.export_job LIMIT 1) THEN
        RAISE EXCEPTION 'export history is non-empty; archive before rollback'
            USING ERRCODE = '55000';
    END IF;
END
$$;
...
DROP INDEX IF EXISTS ops.idx_ops_export_collection;
DROP INDEX IF EXISTS ops.idx_ops_export_pending_recovery;
...
ALTER TABLE ops.export_job
    DROP COLUMN IF EXISTS file_deleted_at,
