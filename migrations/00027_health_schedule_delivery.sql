...
BEGIN
    IF EXISTS (
        SELECT 1 FROM ops.health_schedule
        WHERE pending_due_at IS NOT NULL OR dispatch_lease_until IS NOT NULL
    ) THEN
        RAISE EXCEPTION 'cannot remove health schedule delivery state while a due is pending' USING ERRCODE = '55000';
    END IF;
    IF EXISTS (
        SELECT 1 FROM ops.health_schedule
        WHERE last_run_at IS NOT NULL AND last_run_at < created_at
    ) THEN
        RAISE EXCEPTION 'cannot restore legacy health schedule time ordering with missed-run history' USING ERRCODE = '55000';
    END IF;
END;
$$;
-- +goose StatementEnd

-- Restore the v25 schedule trigger before removing the additive delivery columns.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION ops.validate_health_schedule_write()
RETURNS trigger
LANGUAGE plpgsql
...
DROP INDEX IF EXISTS idx_ops_health_schedule_pending_lease;
ALTER TABLE ops.health_schedule
    DROP CONSTRAINT IF EXISTS ops_health_schedule_dispatch_binding,
    DROP COLUMN IF EXISTS dispatch_lease_until,
    DROP COLUMN IF EXISTS pending_due_at;
...
DELETE FROM core.schema_meta WHERE key='health_schedule_delivery';
