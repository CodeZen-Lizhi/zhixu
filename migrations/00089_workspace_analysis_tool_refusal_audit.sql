...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM agent.workspace_analysis_tool_refusal LIMIT 1) THEN
        RAISE EXCEPTION 'workspace analysis tool refusal facts exist; rollback is unsafe'
            USING ERRCODE='55000';
    END IF;
END;
$$;
...
DROP TRIGGER IF EXISTS workspace_analysis_tool_refusal_reject_mutation
    ON agent.workspace_analysis_tool_refusal;
DROP TRIGGER IF EXISTS workspace_analysis_tool_refusal_reject_truncate
    ON agent.workspace_analysis_tool_refusal;
DROP TRIGGER IF EXISTS workspace_analysis_tool_refusal_guard_insert
    ON agent.workspace_analysis_tool_refusal;
DROP FUNCTION IF EXISTS agent.reject_workspace_analysis_tool_refusal_mutation();
DROP FUNCTION IF EXISTS agent.guard_workspace_analysis_tool_refusal_insert();
DROP INDEX IF EXISTS agent.idx_workspace_analysis_tool_refusal_run;
DROP TABLE agent.workspace_analysis_tool_refusal;
DELETE FROM core.schema_meta WHERE key='workspace_analysis_tool_refusal_audit';
