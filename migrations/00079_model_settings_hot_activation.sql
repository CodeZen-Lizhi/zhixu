...
LOCK TABLE ops.model_settings_state, ops.model_settings_runtime,
    ops.model_settings_rollout_participant, ops.runtime_mutation_gate,
    workflow.node_attempt IN ACCESS EXCLUSIVE MODE;

-- +goose StatementBegin
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM ops.model_settings_rollout_participant LIMIT 1)
       OR EXISTS (
            SELECT 1 FROM ops.model_settings_state
            WHERE phase IN ('preparing','arming','activating') LIMIT 1
       ) OR EXISTS (
            SELECT 1 FROM ops.runtime_mutation_gate
            WHERE owner_kind='model_settings' OR owner_id IS NOT NULL AND owner_kind IS NULL
            LIMIT 1
       ) THEN
        RAISE EXCEPTION 'hot activation history must be archived before rollback' USING ERRCODE='55000';
    END IF;
END
$$;
-- +goose StatementEnd

DROP TRIGGER IF EXISTS model_settings_participant_reject_truncate ON ops.model_settings_rollout_participant;
DROP TRIGGER IF EXISTS model_settings_participant_guard ON ops.model_settings_rollout_participant;
DROP FUNCTION IF EXISTS ops.enforce_model_settings_participant();
DROP TABLE ops.model_settings_rollout_participant;
...
ALTER TABLE ops.model_settings_state
    ...
    DROP CONSTRAINT ops_model_settings_state_rollout_shape,
    ADD CONSTRAINT ops_model_settings_state_phase
        CHECK (phase IN ('idle','validating','draining','applying','verifying','failed')),
    ADD CONSTRAINT ops_model_settings_state_rollout_shape CHECK ((
        (phase='idle' AND rollout_id IS NULL AND target_revision IS NULL
            AND previous_active_revision IS NULL AND lease_expires_at IS NULL AND last_error_code IS NULL)
...
BEGIN
    ...
    IF NEW.desired_revision<>OLD.desired_revision AND NEW.desired_revision<=OLD.desired_revision THEN
        RAISE EXCEPTION 'model settings desired revision cannot move backwards' USING ERRCODE='55000';
    END IF;
    IF OLD.phase IN ('validating','draining','applying','verifying') AND NEW.desired_revision<>OLD.desired_revision THEN
    ...
    IF NOT (
        ...
        OR (OLD.phase IN ('validating','draining','applying','verifying') AND NEW.phase='failed'
            AND NEW.rollout_id=OLD.rollout_id AND NEW.target_revision=OLD.target_revision
            AND NEW.previous_active_revision=OLD.previous_active_revision)
        OR (OLD.phase='verifying' AND NEW.phase='idle' AND NEW.active_revision=OLD.target_revision)
    ) THEN
...
BEGIN
    ...
    IF NOT ops.model_settings_revision_exists(NEW.applied_revision) THEN
        RAISE EXCEPTION 'model settings runtime references an unknown revision' USING ERRCODE='23503';
    END IF;
    IF TG_OP='INSERT' OR NEW.instance_id<>OLD.instance_id THEN RETURN NEW; END IF;
    IF NEW.applied_revision<>OLD.applied_revision OR NEW.applied_at<>OLD.applied_at OR NEW.heartbeat_at<OLD.heartbeat_at THEN
...
BEGIN
    IF NEW.phase IN ('validating','draining','applying','verifying') THEN
        UPDATE ops.runtime_mutation_gate
        SET owner_kind='model_settings',owner_id=NEW.rollout_id,lease_expires_at=NEW.lease_expires_at,
            version=version+1,updated_at=NEW.updated_at
        WHERE singleton=true AND (owner_id IS NULL OR (owner_kind='model_settings' AND owner_id=NEW.rollout_id));
        GET DIAGNOSTICS changed_rows=ROW_COUNT;
        IF changed_rows<>1 THEN RAISE EXCEPTION 'runtime mutation gate is held by another operation' USING ERRCODE='55P03'; END IF;
    ELSIF OLD.phase IN ('validating','draining','applying','verifying') AND NEW.phase IN ('idle','failed') THEN
        UPDATE ops.runtime_mutation_gate
        SET owner_kind=NULL,owner_id=NULL,lease_expires_at=NULL,version=version+1,updated_at=NEW.updated_at
        WHERE singleton=true AND owner_kind='model_settings' AND owner_id=OLD.rollout_id;
        GET DIAGNOSTICS changed_rows=ROW_COUNT;
        IF changed_rows<>1 THEN RAISE EXCEPTION 'model settings mutation gate ownership is corrupt' USING ERRCODE='55000'; END IF;
    END IF;
    RETURN NEW;
END
$$;
...
BEGIN
    IF TG_OP='UPDATE' THEN
        ...
        RETURN NEW;
    END IF;
    IF NEW.model_settings_revision IS NULL THEN RETURN NEW; END IF;
    IF NOT ops.model_settings_revision_exists(NEW.model_settings_revision) THEN
        RAISE EXCEPTION 'workflow node attempt references an unknown model settings revision' USING ERRCODE='23503';
    END IF;
    SELECT runtime.instance_id,runtime.applied_revision,runtime.rollout_id,runtime.phase
    ...
    IF NEW.model_runtime_instance_id IS DISTINCT FROM worker_instance_id
       ...
       OR worker_rollout_id IS NOT NULL OR worker_phase<>'active' THEN
        RAISE EXCEPTION 'workflow node attempt model runtime binding does not match the active worker runtime' USING ERRCODE='23514';
    END IF;
    RETURN NEW;
END
$$;
