...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM ops.git_remote_config_revision)
       OR EXISTS (SELECT 1 FROM ops.git_sync_run)
       OR EXISTS (SELECT 1 FROM ops.git_sync_outbox) THEN
        RAISE EXCEPTION 'Git remote sync facts exist; rollback is unsafe' USING ERRCODE='55000';
    END IF;
END;
$$;
-- +goose StatementEnd
...
DROP TRIGGER IF EXISTS git_sync_attempt_validate_update ON ops.git_sync_attempt;
DROP FUNCTION IF EXISTS ops.validate_git_sync_attempt_update();
DROP INDEX IF EXISTS change_control.idx_writeback_execution_auto_sync_candidates;
DROP TRIGGER IF EXISTS git_sync_run_publish_server_event ON ops.git_sync_run;
DROP FUNCTION IF EXISTS ops.publish_git_sync_run_server_event();
DROP TRIGGER IF EXISTS git_sync_run_validate_update ON ops.git_sync_run;
DROP FUNCTION IF EXISTS ops.validate_git_sync_run_update();
DROP TABLE IF EXISTS ops.git_sync_index_retry_receipt;
DROP TABLE IF EXISTS ops.git_sync_outbox;
DROP TABLE IF EXISTS ops.git_sync_attempt;
DROP TABLE IF EXISTS ops.git_sync_run;
DROP FUNCTION IF EXISTS ops.valid_git_sync_changed_files(jsonb);
DROP FUNCTION IF EXISTS ops.valid_git_sync_path(text);
DROP TABLE IF EXISTS ops.git_remote_command_receipt;
DROP TRIGGER IF EXISTS git_remote_credential_validate_binding ON ops.git_remote_credential;
DROP TRIGGER IF EXISTS git_remote_config_validate_credential ON ops.git_remote_config;
DROP FUNCTION IF EXISTS ops.validate_git_remote_credential_binding();
DROP TABLE IF EXISTS ops.git_remote_credential;
DROP TRIGGER IF EXISTS git_remote_publish_server_event ON ops.git_remote_config;
DROP FUNCTION IF EXISTS ops.publish_git_remote_server_event();
DROP TRIGGER IF EXISTS git_remote_config_validate_projection ON ops.git_remote_config;
DROP FUNCTION IF EXISTS ops.validate_git_remote_config_projection();
DROP TRIGGER IF EXISTS git_remote_config_revision_append_only ON ops.git_remote_config_revision;
DROP FUNCTION IF EXISTS ops.reject_git_remote_config_revision_mutation();
DROP TABLE IF EXISTS ops.git_remote_config_revision;
DROP TABLE IF EXISTS ops.git_remote_config;
DELETE FROM core.schema_meta WHERE key='git_remote_sync';
