...
-- A downgrade cannot silently discard retained Memory, receipts, or audit facts.
LOCK TABLE learning.memory_audit, learning.memory_command, learning.memory IN ACCESS EXCLUSIVE MODE;

-- +goose StatementBegin
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM learning.memory_audit LIMIT 1)
       OR EXISTS (SELECT 1 FROM learning.memory_command LIMIT 1)
       OR EXISTS (SELECT 1 FROM learning.memory LIMIT 1) THEN
        RAISE EXCEPTION 'memory history is non-empty; archive before rollback'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
DROP TRIGGER IF EXISTS trg_learning_memory_audit_append_only ON learning.memory_audit;
DROP FUNCTION IF EXISTS learning.reject_memory_audit_mutation();
DROP TRIGGER IF EXISTS trg_learning_memory_lifecycle ON learning.memory;
DROP FUNCTION IF EXISTS learning.enforce_memory_lifecycle();
DROP INDEX IF EXISTS learning.idx_learning_memory_audit_memory;
DROP TABLE IF EXISTS learning.memory_audit;
DROP INDEX IF EXISTS learning.idx_learning_memory_command_memory;
DROP TABLE IF EXISTS learning.memory_command;
DROP INDEX IF EXISTS learning.idx_learning_memory_expiry_due;
DROP INDEX IF EXISTS learning.idx_learning_memory_effective_context;
DROP INDEX IF EXISTS learning.idx_learning_memory_owner_updated;

ALTER TABLE learning.memory
    DROP CONSTRAINT IF EXISTS learning_memory_episodic_expiry_check;
ALTER TABLE learning.memory
    DROP CONSTRAINT IF EXISTS learning_memory_expiry_order_check;
ALTER TABLE learning.memory
    DROP CONSTRAINT IF EXISTS learning_memory_effective_confirmation_check;
ALTER TABLE learning.memory
    DROP CONSTRAINT IF EXISTS learning_memory_confirmation_pair_check;
ALTER TABLE learning.memory
    DROP CONSTRAINT IF EXISTS learning_memory_confirmation_owner_check;
ALTER TABLE learning.memory
    DROP CONSTRAINT IF EXISTS learning_memory_source_ref_check;
ALTER TABLE learning.memory
    DROP CONSTRAINT IF EXISTS learning_memory_source_type_check;
ALTER TABLE learning.memory
...
ALTER TABLE learning.memory
    DROP CONSTRAINT IF EXISTS memory_status_check;
ALTER TABLE learning.memory
    DROP COLUMN IF EXISTS confirmed_by_principal_id;
ALTER TABLE learning.memory
    DROP COLUMN IF EXISTS confirmed_by_principal_kind;
ALTER TABLE learning.memory
    DROP COLUMN IF EXISTS confirmed_at;
ALTER TABLE learning.memory
    DROP COLUMN IF EXISTS task_scope_id;
ALTER TABLE learning.memory
...
ALTER TABLE learning.memory
    ...
    CHECK (status IN ('CANDIDATE', 'CONFIRMED', 'REVOKED', 'EXPIRED'));
