...
- Platform settings/config files decide which hooks, agents, skills, commands, prompts, and workflows actually run. On Codex, native Trellis sub-agent context is injected by the `SubagentStart` hook; other hook-driven platforms use their documented pre-tool dispatch hooks. Reasonix has no settings file — behavior is encoded inside skill frontmatter.
