...
grep -Fq 'source "${SCRIPT_DIR}/compose-workspace-analysis-v2-smoke-functions.sh"' "${SMOKE_SCRIPT}" || fail 'post-fact rollback does not load the dynamic analysis assertions'
grep -Fq 'assert_workspace_analysis_v2_completed "${ROLLBACK_ANSWER_ID}" default 5 5 1 true' "${SMOKE_SCRIPT}" || fail 'post-fact fixture does not prove the default dynamic analysis sequence and budget'
grep -Fq 'assert_workspace_analysis_v2_timeline_replay "${ROLLBACK_ANSWER_ID}" default' "${SMOKE_SCRIPT}" || fail 'post-fact rollback does not reread the unchanged historical timeline'
