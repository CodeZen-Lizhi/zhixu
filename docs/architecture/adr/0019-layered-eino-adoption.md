...
- `direct|eino` 是进程级实现选择，不是 Provider/Model 身份。选择器不得复用 `ChatAdapterVersion`，API 与 Worker 必须从相同冻结配置构造唯一 `ModelRuntime`。在真实 Provider smoke 通过前默认保持 `direct`；`eino` 显式启用，回滚只切回 `direct`。
...
- 故障测试必须覆盖 request/response 上限、缺 usage、model mismatch、finish reason、tool call、429/5xx/401、取消、deadline、redirect 和敏感错误体。
...
- 在真实 Provider smoke 完成前，生产默认行为不变；显式选择 `eino` 可运行离线 fixture、受控环境和后续灰度。
