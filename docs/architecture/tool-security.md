...
M4-C 已把 Approved Proposal 原子接入正式 Workflow/River：持久 Job Args 只含 schema version、Node Run ID 与 dispatch no；Bootstrap Node input 只含 Proposal、Revision 和 approved Change Hash。Credential 仅在 Worker 栈帧中存在，Begin 后立即清除引用，任何持久层只允许 Authorization token hash。正文、相对路径与 locator/identity token 只保留在拥有恢复事实的 Change Control Proposal/Execution/Reindex 契约中，不得复制进 River Args、Safe Writeback Node input、Workflow Attempt/dispatch Outbox 或错误摘要。M4-D 的安全 Handler 对日志做 fail-closed 脱敏，Metrics 拒绝高基数/敏感 labels，项目只写校验后的 `traceparent` metadata；River 保留 `river:*` recovery 字段可共存但不会进入 Application，Worker health 只返回稳定 `status/code/version`。完整 Approval→Run binding 重放不重新读取文件/Git，历史未绑定 Approval 必须重新通过当前安全门。M6-B Reindex Job Args 仅含 schema version、Delivery ID 与 dispatch no；正文、路径、Workspace、Commit 和 Credential 继续只从严格绑定的 PostgreSQL/Commit 事实加载。只有结构回归、Activation 与跨域 Completion 事务全部通过才将 `index_pending` 推进为 completed。
