...
- 动态分析可按问题省略 Git、再次检索或追加来源阅读，并由模型明确决定结束；调用顺序和次数不能由服务端固定阶段冒充。每步决策、工具结果和预算均可恢复，已经完成的调用重放不得重复付费或产生第二份结果。
...
- 资料完成可用解析后自动触发同 Workspace 的合成任务，不依赖浏览器停留或反复手动选择材料；合成失败时保留原资料，并提供明确处理状态与受控重试。
...
- 首次生成与增量更新均先形成待审 Proposal，批准后才写入正式文件、Git 与索引；待审版本与已发布版本明确区分，陈旧候选不得覆盖用户后续修改。
- 用户可从已发布合成笔记发起 AI 面试；题目与追问冻结该版本的事实、冲突、缺口和原始来源。后续候选不改变已开始的面试，回答后可回看当时的来源。
...
- 提供停写备份与受控恢复的最小操作；完整备份恢复、数据库/文件/Git 一致性和索引切换演练按实际数据风险选择，不作为每次开发交付前置。

### 性能与容量

- 保留覆盖 50 万 Chunk、关系投影和有界图查询的容量工具与设计预算；完整目标环境采样按实际容量问题选择，不以工具存在声称达到阈值。
...
- 开发收尾只要求当前改动必要的行为、数据安全、兼容性和静态检查；已有实现可以复用可信验收记录，不为状态收尾重跑全仓矩阵或新增形式化测试。
- 长期观察、跨平台/真实公网矩阵、完整容量/FPS、灾备演练和覆盖率趋势不作为开发交付门禁。未执行时明确记录，不能写为测试通过；实际失败与尚未实现的业务能力必须单独处理。
- 完整 E2E、AI Eval 和发布包等 M11 工作留待整体开发完成后单独执行；开发任务完成不代表 M11 已通过或软件已经部署。
