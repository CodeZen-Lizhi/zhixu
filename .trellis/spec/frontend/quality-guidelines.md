...
按用户 2026-09-08 的精简要求，只验证实际变更及其相关风险，复用有效既有证据；低影响文档/状态变化不新增
形式化测试。下文完整类别和历史模块清单按影响面选用，不要求每次收尾重跑全部浏览器、性能、跨平台或覆盖率矩阵。
未执行的验证写明未执行，不记 PASS；真实业务缺口与失败独立处理。M11 最终 E2E/AI Eval/发布包待整体开发完成后执行。
