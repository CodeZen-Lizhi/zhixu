...
- 当前固定的 Eino/eino-ext 版本只实现 `chat_completions`。`responses` 枚举与历史 revision 继续可读，避免重写
  append-only 持久事实，但 factory、结构化 Chat 和 Agent/Stream Runtime 都必须在任何网络请求前以
  `MODEL_CHAT_CONFIG_INVALID` fail closed；不得把 Responses revision 静默发送到 `/v1/chat/completions`。
- `ChatContract` 必须冻结实际 Provider、API style 和 endpoint path。Ollama 必须记录为 `ollama` 与
  `/v1/chat/completions`，不能因复用 Eino OpenAI-compatible component 而伪装成 remote OpenAI-compatible。
...
| `chat_api_style=responses` | `InvalidInput` | `MODEL_CHAT_CONFIG_INVALID` | false |
