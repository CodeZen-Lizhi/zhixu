...
- [ ] Agent child 在 `internal/agent/application/scoped_model_run.go` 提供 `GetModelRunScoped`、`GetModelRunRecordScoped`、`FinalizeModelRunScoped` 及同 transaction GORM 实现；保留 for-update/CAS/exact replay，Capture 不实现或复制 Agent SQL。
- [ ] 新增 `GORMProfileRepository` 构造与 ProfileGenerationRepository/ProfileReader/ProfileBatchReader/ProfileRetryScheduler 静态断言。
- [ ] 实现 LookupReady/LoadSource/Prepare/Bind/Complete/Fail/Get/GetProfiles，保持 advisory/shared/row lock 顺序和 bounded set queries。
- [ ] 实现 Profile retry，保持 Profile -> Capture 锁序、旧 Revision 可读、Outbox/receipt/CAS 同事务。
- [ ] Revision/Evidence/Profile Attempt/Agent Model Run 使用同一 scope 原子终结；Evidence/Profile batch arrays 使用 `pq.Array`。
- [ ] 保持 Profile JSON/content digest、Evidence 闭包、Chunk 500/128 KiB SQL fail-closed、READY/STALE/capability/replay 语义。
...
- [x] `go vet -mod=vendor ./internal/capture/... ./internal/workspace/application ./internal/platform/postgres`
- [x] `go test -mod=vendor -tags=integration -run '^$' ./internal/platform/migration ./cmd/worker -count=1 -timeout 60s`
- [x] `go test -mod=vendor -run '^$' ./cmd/api ./cmd/worker ./internal/workspace/... -count=1 -timeout 60s`
...
- [ ] 原位参数化现有 integration factory；每个子用例用 raw pool 迁移后保存 ConnString并关闭 raw pool，再打开唯一 `platformpostgres.Pool`；cleanup先关平台 Pool再 DROP。
- [ ] legacy 从该 Pool 的 `DB()` 构造；GORM 从同一 Pool 创建 Workspace `GORMRepository`/真实 ScopedSourceWriter，再注入 Capture `GORMRepository`，不得使用 mock证明跨 owner事务。
- [ ] 比较并发 Create、exact replay/conflict、Source/Artifact/Version/Capture/Outbox/receipt 原子 commit/rollback 和 response-loss。
- [ ] 比较 Retry、Outbox 双 worker claim/reclaim、owner/version CAS、Attempt replay、URL materialization 和全部 stage/terminal transition。
- [ ] Agent scoped Port 可用后比较 Profile prepare/bind/complete/fail/retry、旧 revision survival、Evidence closure、Model Run 终结和 replay。
- [ ] 覆盖 Index activation/Profile completion 交错、advisory/shared/row locks、deferred FK/trigger、真实 SQLSTATE 和 deadlock/serialization。
- [ ] 覆盖 UUID/JSONB/array/cast、cancel/cause/deadline、stale scope、commit failure、corrupt row no-partial、连接释放和目标查询计划。
- [ ] Final 阶段复跑 Worker Capture Outbox -> Workflow/River、API/Worker Composition 和发布链门禁；本 child 不提前改 cmd。
- [ ] TODO 9 全部通过后才勾选 PRD AC 并完成 child；Profile前置未满足时即使 Core staged 完成也保持 `in_progress`。
