...
- [x] 保留 SQLSTATE/constraint、not-found/found=false、version/replay conflict、retryability 和 commit unknown。
...
## 11. TODO 9 真实 PostgreSQL 门禁

- [ ] 原位参数化现有 integration fixture；每个 legacy/GORM 子测试独立数据库，migration raw pool 关闭后创建唯一 `platformpostgres.Pool`。
- [ ] 比较 Model Run/Call create/replay/conflict、CAS、caller scope 可见性、按 ID/Attempt 查询、FOR UPDATE、trigger、stable call order 和 UNKNOWN recovery。
- [ ] 验证 Capture Profile Revision/Evidence/Attempt 与 Agent Run 在同一 scope commit/rollback、竞争和 exact replay。
- [ ] 比较 Memory claimant、READY+Run 双向绑定、FAILED、rollback 和 commit-loss。
- [ ] 比较 Workspace Analysis 双授权、cancel 竞态、budget、replacement、success/candidate/review/failure/refusal/unknown、Workflow fence found/error 翻译与 commit recovery。
- [ ] 比较 Capability DB lease、Run scoped insert/replay，以及 Conversation Workflow/Event/Agent owner facts 同事务。
- [ ] 验证 RAG Progress advisory lock + Event append 的 commit/rollback/conflict/commit unknown。
- [ ] 覆盖真实 SQLSTATE、deferred trigger、cancel/cause/deadline、nil/foreign/stale scope、corrupt row、连接释放和目标 EXPLAIN。
- [ ] Final 阶段复跑 API/Worker/Capture/Artifact/Conversation/Organizing/Workflow/River 端到端；本 child 不提前改 cmd。
- [ ] TODO 9 全部通过后才勾选 PRD AC 并完成 child；production 切换和 legacy 删除仍归 Final。
