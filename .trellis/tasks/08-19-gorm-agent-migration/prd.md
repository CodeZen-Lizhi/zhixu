...
- [ ] AC1：Agent staged `GORMRepository` 与 `GORMWorkspaceAnalysisRepository` 覆盖 Model Run/Call、Recovery、Memory Snapshot、Workspace Analysis 与 Candidate/Checkpoint/Capability 现有契约，结果和状态语义与 legacy 一致。
- [ ] AC2：`ScopedModelRunFinalizer` 与独立 `ScopedModelRunAttemptFinder` 不暴露 `any`/GORM/sql/pgx，在 caller-owned scope 中保持锁、稳定 Calls、按 Attempt 查询、CAS 与 exact replay；Capture 只依赖三方法 Finalizer并能与 Profile 原子组合。
