...
2026-08-21 完成 Agent staged 范围的 Application scoped Port、Model Run/Call、Recovery、RAG Memory Snapshot、Workspace Analysis Run/Capability、Tools owner scoped participant/refusal/authority 与 RAG Progress 实现。生产 `cmd/api`、`cmd/worker`、Capture、Conversation、Artifact 和 Organizing 仍使用 legacy pgx 路径；未增加 selector、双写、fallback、migration 或测试文件。

本阶段不能完成 child：Workflow owner 已交付 scoped execution fence，但独立 `GORMWorkspaceAnalysisRepository` 的 Model Operation/Result/Candidate/Checkpoint 阶段尚未实现；`ZHIXU_TEST_DATABASE_URL` 未配置，TODO 9 的真实 PostgreSQL 等价门禁未执行。PRD AC、任务完成状态和归档保持未完成。
...
以下命令在 2026-08-21 对新增 Tools scoped 范围复跑并通过：
...
`integration -run '^$'` 只证明测试可编译，不是数据库行为证据。Trellis validate 仅有既有大型 spec 注入截断 warning，验证本身通过。
...
- Go Review：检查 scoped Port、scope 生命周期、UoW 所有权、context、DTO/domain closure、资源和 production compatibility。未发现 P0/P1；补齐非法/失活 scope 的平台原始 cause chain。唯一 P2 是新增 SQL 路径缺少真实 PostgreSQL 行为覆盖，保留到 TODO 9。
- SQL Review：逐项核对 Participant、Refusal、Authority、legacy Tools 与 `00085/00089/00091`。Reserve 保持 Tool Call 先于 immediate FK，Settle 顺序为 Reservation -> Run budget -> Operation，UNKNOWN 全额结算；未发现 P0/P1/P2。
- Trellis Check：确认 owner 边界、生产未接线、Model Operation 主阶段仍未完成、PRD AC 与 TODO 9 保持未完成。

## 未完成与风险

1. Workflow child 已交付 scoped execution fence，但 `GORMWorkspaceAnalysisRepository` 的 Model Operation、Result、Candidate 和 Checkpoint 主阶段仍未实现；Agent Tools scoped 实现没有复制 `workflow.*` 或 `workflow.tool_call` SQL。
2. 没有真实 PostgreSQL 证据证明 GORM `?` 重写、UUID/interval 参数、trigger/deferred constraint、真实 SQLSTATE、锁竞争和目标执行计划。
3. Memory claimant/READY 双向绑定、Recovery 并发 `SKIP LOCKED`、Capability DB-clock freshness、Tools participant/refusal/authority 的 immediate/deferred FK 与锁竞争、RAG advisory lock + Event append、commit response loss、context cancel/deadline 和连接释放仍需 TODO 9 原位参数化 integration fixture。
4. Foundation `TransactionScope` 没有 Pool affinity identity。当前只能拒绝 nil/foreign type/stale scope，不能识别来自另一个平台 Pool 的 active scope；同池由 Composition 与 TODO 9 fixture 保证，严格 cross-pool rejection 需 Foundation 后续能力。
5. 生产切换、legacy `any` Port 删除和全仓 pgx 收口仍归 Final child；本 child 不具备切换条件。
