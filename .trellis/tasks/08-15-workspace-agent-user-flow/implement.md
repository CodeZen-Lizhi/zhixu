...
> 状态：用户已批准实施；Phase 0–6 的本地实现、真实 PostgreSQL/River、桌面/移动浏览器、隔离 Collector 的 Worker OTLP Metrics 演练以及单入口 post-fact Worker-only 回滚演练已完成。功能开关默认关闭；目标环境 Migration、Canary、Worker OTLP 观察窗口、扩量和实际回滚记录仍需发布授权。
...
### Production execution (requires deployment authority)
...
本地实现、AC1–AC11、Compatibility 和 release rehearsal 已完成。仍保持未完成的是需要外部部署授权的目标环境 Migration、不可变 API/Worker 发布物核验、真实 canary Workspace、Worker OTLP 观察窗口、扩量批准和实际回滚记录；这些证据完成前 Production execution 不勾选，任务继续保持 `in_progress`。
