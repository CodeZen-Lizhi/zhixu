...
- 当前没有真实外部 exporter 和 OpenAI-Compatible Provider 凭据；对应 smoke 记录为 SKIP，不能据此切换默认实现。
...
最终验证包括受影响包 `go test -race`、Eino scheduler `-race -count=20`、相关 `go vet`、vendor API/Worker 构建、`go mod tidy -diff`、`make compose-check`、PoC race/vet、Python 合同编译和 `git diff --check`。生产 Eino live smoke 因未配置 `ZHIXU_EINO_LIVE_*` 凭据仍为 SKIP，因此 direct 保持默认并保留全部回退路径。

最终复核还通过了 `make openapi-check`、integration tag 编译和 `make agent-eval`；离线 fixture 的五类关系 precision/recall/F1、citation precision/coverage、faithfulness、conflict disclosure 与 appropriate refusal 均为 `1`。独立代码 Review 重新检查五个计数包装器、Worker Composition Root 和 PostgreSQL/River 重投断言，未发现 P0/P1/P2 问题。live smoke 测试本身可运行但明确输出 `SKIP`，没有把缺少外部凭据包装成通过。
