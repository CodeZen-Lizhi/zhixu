...
- [ ] staged `GORMRepository` 完整实现现有 `application.Store`，稳定 Port 与 legacy pgx 生产路径保持不变。
- [ ] Review Answer frozen snapshot/evidence、reservation exact replay/reopen、Artifact digest/hold 与 attempt fence 在真实 PostgreSQL 上与 legacy 等价。
- [ ] Path/Step/receipt/final reservation 的原子写入、回滚、历史保护、稳定顺序和唯一约束在真实 PostgreSQL 上等价。
- [ ] Path/Step command 的 advisory lock、expected-version CAS、状态转换、父聚合推进和 exact replay 在真实 PostgreSQL 上等价。
- [ ] maintenance 的数据库时间、有界 `SKIP LOCKED`、ABANDONED/ORPHANED 和迟到请求 fence 在真实 PostgreSQL 上等价。
- [ ] no-row、SQLSTATE、cancel/deadline、commit response-loss、Rows/transaction 生命周期和日志脱敏通过门禁。
- [ ] 局部 test/race/vet/compile、module/vendor、Trellis validate、Go/SQL Review、gofmt 与 `git diff --check` 通过并记录。
- [ ] TODO 9 真实 PostgreSQL 门禁全部通过；在此之前不得完成、归档或声明生产已迁移。TODO 3 仅阻断 Final。
