...
- [ ] `go mod tidy -diff` 已只读执行但非绿；输出为共享工作树预存的 `go.sum`/sqlite 依赖漂移，未应用，待依赖 owner 收敛。
...
- [ ] TODO 9 全部通过后记录 Review 三 child 跨模块门禁所需证据和 Final 构造/legacy 删除清单；本 child 仍不修改父任务或 `cmd/**`。
- [ ] 回滚仅删除本 child staged GORM 文件并还原本 child 内共享 helper/test factory；不回滚 Schema、历史数据、Foundation 或其他会话改动。
