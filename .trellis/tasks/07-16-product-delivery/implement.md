...
| M7-02 | M7 | 实现 Semantic Link Candidate、fingerprint、确认/忽略 Proposal | `internal/graph/**`, `internal/changecontrol/**` | M7-01,M5-03 | 忽略候选不重复；内容变化再评估 | 候选噪声 | 主 Agent + 子 Agent | 已完成：Candidate→typed Proposal→Approval→Relation、持久 Topic scan/River、fault/eval/browser smoke、全量门禁与两轮独立复审通过；目录/Smart Collection scope 仍归后续任务 |
