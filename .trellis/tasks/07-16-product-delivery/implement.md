...
| M7-02 | M7 | 实现 Semantic Link Candidate、fingerprint、确认/忽略 Proposal | `internal/graph/**`, `internal/changecontrol/**` | M7-01,M5-03 | 忽略候选不重复；内容变化再评估 | 候选噪声 | 主 Agent + 子 Agent | 已完成：Candidate/Decision/fingerprint、typed Relation Proposal、Approval→Knowledge Relation、durable Topic scan、严格前端与五项评测通过全量门禁和两轮独立审查；Document/目录/Smart Collection 与 M10 认证/500k 容量仍按后续任务推进 |
