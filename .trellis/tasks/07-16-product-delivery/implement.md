...
| M6-04 | M6 | 实现 RAG API、会话、SSE 流式展示和反馈评测 | `internal/app/**`, `web/src/features/rag/**` | M1-04,M6-02,M6-03 | RAG E2E、SSE reconnect、拒答样本 | 流式草稿被误当完成 | 主 Agent + 子 Agent | 进行中：T01-T06 契约、迁移、领域、Conversation Repository、持久 SSE Handler 与原子 Question→Workflow 派发已完成；T07-T11 RAG 执行/HTTP/生产装配及 T12-T17 前端/E2E/交付门禁待实施 |
