...
| M7-04 | M7 | 实现 Knowledge Event、Timeline、Impact Analysis | `internal/knowledge/**`, `internal/health/**`, `internal/audit/**` | M5-04,M7-03 | Commit/Proposal/Conflict/下游互查；只生成报告/Proposal | 级联误写 | 主 Agent + 子 Agent | 已完成：首轮工作提交 `5cd940b`；本轮补齐 Timeline UI、`impact-analysis/v2` Artifact/Review Card impact、一等 owner event、正式 `downstream_update` Proposal 意图及真实 PostgreSQL/API/Worker/Vite 动态验收。批准仍保持零执行副作用；下游 owner executor、Document/Eval impact 与全局 Audit 继续 deferred。 |
...
| M8-03 | M8 | 实现 Interview Session、知识缺口、共享 Learning Path 和 Memory 生命周期 | `internal/review/**`, `internal/memory/**` | M8-02,M7-04 | 面试报告、只记录明确确认 Memory、可编辑删除 | 隐式长期记忆 | 子 Agent | 已完成：Interview origin Path、证据驱动 Interview、难度/deadline/连续追问、SKIPPED gap、Memory Candidate→Confirm、Interview-scoped effective context、Completion reservation/hidden-hold Worker、Review-derived shared Path 与 24h maintenance 已交付；动态补验通过 Interview/Memory Repository、`00059/00060` 前向迁移、Review Answer→Path 和真实 API/Worker/Vite 浏览器主链路。业务数据 Down guard、Review Path reservation/hold 专门并发与 ABANDONED 重开仍待独立覆盖，通用 Agent/RAG Memory 注入未交付。 |
...
| M9-03 | M9 | 实现异步导出任务、脱敏、权限/过期和结果追踪 | `internal/export/**`, `web/src/features/{collections,settings}/**` | M7-03,M1-04 | Markdown/JSON/附件导出、权限/过期、任务恢复；复用已交付 Collection 三视图 | 擅自引入 XLSX/CSV | 子 Agent | 已完成：`0165783` 交付 Collection `MARKDOWN|METADATA_JSON`；M9 residual 追加 Workspace `ATTACHMENTS_ZIP`、持久 gate、确定性 ZIP/manifest、流式下载、Settings 与真实 PostgreSQL/API/Worker/Vite 浏览器门禁，AC-33 已关闭。`EVALUATION_JSON|AUDIT_JSON` 仍 deferred。 |
