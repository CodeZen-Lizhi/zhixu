...
- [ ] Query Port、Candidate/Confirm、Scan/Planner/Page/Writer/Cancel 的 staged GORM 实现完整，
      legacy 构造和生产 Composition 未改变。
- [ ] Candidate Confirm 只通过 opaque scoped Change Control capability 参与同一 UoW，
      不复制新的跨 owner SQL，不产生嵌套事务或部分 Proposal/Decision。
- [ ] Change Control scoped Proposal能力已在Change Control task中独立实现、审查和验证；
