...
- [x] AC1：Capture 核心 Repository、Retry、Outbox 和 Processing 已通过共享 GORM root/UoW 实现，Create 与 URL materialization 和 Workspace scoped writer 同事务提交/回滚。
- [x] AC2：Agent scoped Model Run Port 已由 Agent owner 提供，Profile/Retry GORM Adapter 在同一 scope 内保持 Revision/Evidence/Profile Attempt/Model Run 原子终结。
- [x] AC3：Capture/Profile 的 Workspace 隔离、keyset、批量读取、严格 scanner、DB time、锁序、CAS、exact replay、错误码和安全日志与 legacy 等价。
- [x] AC4：现有 unit/race/vet、integration compile、真实 SQLSTATE/trigger/并发/回滚/连接释放与目标查询计划等模块级门禁通过；Worker outbox/Workflow、API/Worker Composition、最终发布链和生产组合 deadlock/serialization 按父任务归 Final 复跑。
- [x] AC5：生产 Composition 仍未切换、legacy Adapter 未删除、无 selector/双写/fallback/second pool/AutoMigrate；Final 交接清单完整。
- [x] AC6：TODO 9 模块级真实 PostgreSQL 门禁全部通过并有可复核证据；Final 专属组合门禁不阻断本 child 归档。
