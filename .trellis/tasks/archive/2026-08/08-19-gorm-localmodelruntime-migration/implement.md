...
- [x] 原位参数化 legacy/GORM integration factory，fixture 使用同一完整 platform Pool；通过外部 `migration_test` 包解决 shared `testdb` import cycle，未复制 fixture 生命周期。
- [x] 实测生命周期、lease/clock、CAS、idempotency、probe/hold/operation 回滚、ReadDemand snapshot、scoped commit/rollback、role privileges、SQLSTATE 与 EXPLAIN。
- [x] TODO 9 实库门禁全部通过；Model Settings/Final 仍负责生产切换和 legacy 删除。
...
### 6.2 2026-08-31 实库验收结果

- [x] `TestManagedOllamaMigrationConstraints`、`TestManagedOllamaMigrationRepeatedUp`、`TestManagedOllamaRuntimeRoleLeastPrivilege` 通过 race + 独立 PostgreSQL fixture。
- [x] `TestManagedOllamaPersistentPullBudgetAndDeadline`、`TestManagedOllamaActiveRecoverySeed`、`TestManagedOllamaSupersedesStaleActiveRecovery`、`TestManagedOllamaLifecycleStoreCAS` 通过 legacy/GORM 对照 race 场景。
- [x] `TestManagedOllamaScopedActivationTransaction` 通过 caller-owned scope 的 commit/rollback/visibility/stale-scope 验收。
- [x] `TestManagedOllamaReadDemandCancellationAndConnectionRelease` 通过取消原因与连接归还验收；legacy 保持 `context.Canceled` 基线，GORM 额外保留自定义 cause。
- [x] `TestManagedOllamaGORMStatementCountAndExplain` 通过：`ReadDemand` 为 4 条 GORM statements，operation idempotency 与 hold-operation 查询使用目标索引。
- [x] `CompleteOperation` 占位符与 `ClaimTestProbe` ownership 返回契约的两处 GORM/legacy 差异已修复，并重新通过相关真实场景。
