...
- ADR 编号冲突已解决：ADR-0024 是基线分层采用，ADR-0025 是 Embedding 采用，ADR-0026 是其余
  Runtime 路线的扩展门禁结论。
...
- [x] ADR-0025 接受双 Provider Embedding 的可回滚内部实现，默认仍为 `direct`。
...
- [x] ADR-0026 记录 PoC PASS、生产 No-Go；根模块、数据库、River Worker 和生产 Composition 未接入 checkpoint。
