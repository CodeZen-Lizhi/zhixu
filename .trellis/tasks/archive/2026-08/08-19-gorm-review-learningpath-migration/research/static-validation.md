...
- TODO 9 真实 PostgreSQL 门禁已全部通过：legacy/GORM 行为对照、锁竞态、恢复、SQLSTATE、语句数、连接生命周期和执行计划均有可复核证据。
- Go、SQL 与 Trellis 最终审查未发现剩余 P0-P2 缺陷；PRD AC 已全部完成，本 child 可归档。生产 Composition、legacy 删除和跨 child 收口仍只属于 Final。
...
- `internal/review/learningpath/adapter/postgres/review_learning_path_integration_test.go`
- `internal/review/learningpath/adapter/postgres/review_learning_path_test_helpers_integration_test.go`
- 本任务 PRD、Implement、验证记录与最终交接。
...
| `go mod tidy -diff` | PASS，无输出；未修改依赖文件 |
...
## TODO 9 最终门禁（2026-08-30）

- `Complete wins before maintenance` 已通过 adapter-aware barrier 对 legacy 与 GORM 成对验证；maintenance 的另外两个确定性 winner、ABANDONED reopen、old-attempt fence 和 response-loss 也均为真实 PostgreSQL 对照。
- GORM 已验证 no-row、`context.Canceled`、deadline、receipt unique `23505`、history trigger `23514`、FK `23503`、commit-response-loss、事务锁等待/清理和连接归还。
- 固定计数器证明 snapshot evidence、Path+Steps 读取和 maintenance 分别为 3、2、1 条 GORM statement；读取结束后共享 pgx Pool 无占用连接。
- `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)` 证明 evidence 为单次一行 set projection；5,000 条容量 fixture 下 maintenance 使用 `idx_learning_path_creation_pending_maintenance`。
- 12 个顶层 Testcontainers PostgreSQL 场景均以 `-race -count=1 -timeout=60s` 串行通过，最慢 maintenance lock 场景为 50.979s。
- TODO 9 已闭环。本 child 仍不切生产 Composition、不删除 legacy；这些操作与 Review 三 child 跨模块门禁继续留给 Final。
...
- `go mod tidy -diff` 已恢复为无输出；未应用或修改依赖文件。
- 本轮没有产生需要新增公共 `.trellis/spec/` 的稳定契约；`FOR KEY SHARE` 必须处于写事务的边界已由现有 Design/Begin 流程覆盖。
...
## TODO 9 Final Closure (2026-08-30)

- The 2026-08-28 gaps above are closed by `TestReviewLearningPathPostgreSQLCompleteWinsBeforeMaintenance` and `TestReviewLearningPathPostgreSQLGORMStatementBoundsForeignKeyAndPlans`.
- The final review added a deferred rollback to the maintenance EXPLAIN transaction so assertion failures cannot retain a checked-out connection. The affected normal and race PostgreSQL test passed after that fix.
- All 12 top-level PostgreSQL scenarios were rerun separately with `-race -count=1 -timeout=60s`; all passed without sharing a container concurrently. Unit/race, vet, integration compile, API/Worker compile, `go list`, `go mod tidy -diff`, `go mod verify`, Trellis validation, gofmt and `git diff --check` also passed.
- Go/SQL/Trellis quality gates report no remaining P0-P2 findings. All PRD Acceptance Criteria are complete; production Composition and legacy removal remain Final-only.
