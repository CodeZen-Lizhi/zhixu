...
本任务只建立后续模块共同使用的数据访问基础，不迁移业务 Repository，不修改 `cmd/**` Composition，也不删除旧 pgx 事务路径。TODO9 的统一 Testcontainers/Atlas 工厂现已可用，Foundation 通过该工厂完成自身真实 PostgreSQL/River 门禁后即可作为 staged 基础归档。
...
- `riverdatabasesql.SupportsListener=false`，严禁用它启动 Worker。真实提交/回滚原子性、pgx Worker 消费、轮询和恢复语义由 Foundation 集成测试使用现有 pgx Worker 验证。
...
Foundation 通过带 `integration` build tag 的平台验收文件证明共享 facade Close、GORM commit/rollback、GORM+River 原子写入和 pgx Worker 消费；fixture 使用统一 `testdb.Require`，未配置外部 admin URL 时自动创建隔离容器并执行完整 Atlas/River migration。默认单元测试不连接数据库。

真实网络层 COMMIT 断链、目标环境连接预算和各业务 owner 的错误分类仍属于发布或对应模块门禁；Foundation 使用 commit 后错误注入证明已提交事实与唯一 River Job 的 exact replay，不把该证据冒充网络代理故障演练。生产 Composition 仍只在 Final child 切换。
