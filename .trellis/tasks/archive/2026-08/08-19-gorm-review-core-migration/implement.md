...
- [x] 2026-08-31 完成原筛选缺口：五类 selector、200/has_more、due/invalidation `EXPLAIN`、取消和连接释放均已在真实 PostgreSQL 验证；历史筛选结论保留于 `research/todo9-screening-2026-08-27.md`。
- [x] 仅在现有 `repository_integration_test.go` 原位建立 legacy/GORM implementation factory；每个实现使用独立 migrated disposable database，不新增测试文件。
- [x] 使用一个普通 `platformpostgres.Pool` 同时提供 `DB()` / `GORM()` / `UnitOfWork()`，证明无第二 pool、scope root 错配或关闭泄漏。
- [x] 成对运行 Answer/Schedule/receipt 原子性、same/different-key 并发、CAS rollback、REVIEW/INTERVIEW shell、ABA、Deck schedule race；GORM commit response-loss 用同包 UoW wrapper 在真实 `Within` 成功后返回受控错误，再验证 root exact lookup。
- [x] 成对运行 Approval/quarantine/high-conflict 串行、due UTC/daily limit、malformed Evidence、refuting/superseded Claim 与五类有界失效。
- [x] 验证 trigger 投影、cancel/deadline/sql.ErrTxDone、真实 SQLSTATE、JSONB/array binding、Rows/连接释放。
- [x] 已执行整包命令；因 11 个场景各自迁移独立 Testcontainers 数据库，60 秒包级预算在第三个场景的 Atlas migration 阶段超时。按仓库短时门禁约定拆分同一 `-race -p 1 -timeout 60s` 命令后，全部 11 个场景的 legacy/GORM 变体通过。
- [x] 已记录 due/invalidation 关键索引、EXPLAIN、bounded statement count 和无 N+1 证据。

## 9. Completion And Rollback

- [x] TODO 9 不可用时保持 PRD AC 未勾选、`task.json.status=in_progress`、legacy production wiring 不变，不归档或宣称完成。
- [x] 已记录 Review 三 child 跨模块门禁与 Final 构造/legacy 删除清单；本 child 未修改 `cmd/**`。
- [x] 回滚只删除本 child staged GORM 文件并还原本包内必要共享 helper/test factory；不回滚 Schema、历史数据、Foundation 或其他 child。
