...
- [x] 只在现有 integration 文件建立 legacy/GORM implementation fixture；每个实现使用独立 migrated disposable database，不新增测试文件。
- [x] 一个普通 `platformpostgres.Open` Pool 同时提供 `DB()` / `GORM()` / `UnitOfWork()`，验证无第二 pool/root 错配/关闭泄漏。
- [x] 成对运行 Question Select、Start/Submit、Completion、Path/Step、List 与 maintenance；覆盖 same/different key、并发、rollback、trigger 与坏数据。
- [x] 以 UnitOfWork success-then-error wrapper 确定性验证 GORM commit response-loss，不复用 pgx Begin double、不靠 sleep。
- [x] 验证 REPORT/PATH 两个 hold、Artifact digest/attempt、ABANDONED/ORPHANED、late completion 和 exact release。
- [x] 验证 Interview/Review Path 双向不可见、shell/provenance guard、zero FSRS writes、SQLSTATE/cancel/deadline/Rows/连接释放。
- [x] 已运行字面 package race 命令；因每个 variant 独立启动并迁移 Testcontainers 数据库，60 秒时仅停在后续容器启动阶段。五个成对 integration 场景均在相同 60 秒约束下拆分通过。
- [x] 记录关键 Select/List/Completion/maintenance 查询的 statement count、索引与 EXPLAIN，无 N+1/无界扫描。

## 9. Completion And Rollback

- [x] TODO 9 已可用并完成；归档前 legacy production wiring 保持不变，未修改 `cmd/**`。
- [x] Review 三 child 的跨模块门禁、Final 构造切换与 legacy 删除继续归父任务/Final child；本 child 只记录并保持该边界。
- [x] 回滚只删除本 child staged GORM 文件并还原本包必要共享 helper/test fixture；不回滚 Schema、Foundation、其他 owner 或用户改动。
