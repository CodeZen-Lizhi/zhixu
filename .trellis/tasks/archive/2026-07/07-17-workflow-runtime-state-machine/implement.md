...
2. [x] **M4B-02 冻结 Attempt/Dispatch/Retry 模型**
...
3. [x] **M4B-03 实现 Failure 分类与 Retry 计算**
...
- [x] Domain tests 使用 `-race` 通过。
- [x] Success/Failure 互斥和所有计数不变量有直接测试，不依赖 Repository 偶然行为。

## Phase 2: Schema And DB-time Lease

4. [x] **M4B-04 新增前向 Workflow 状态机迁移**
...
5. [x] **M4B-05 实现 DB-time Claim 与 lease reclaim**
...
6. [x] **M4B-06 实现 Heartbeat 与 Context 取消**
...
- [x] PostgreSQL integration 证明一个 Node 至多一个活动 Attempt。
- [x] 锁序说明与 SQL 实现一致；Claim/Heartbeat 并发重复运行无死锁。

## Phase 3: Result Transitions And DAG

7. [x] **M4B-07 实现 Retry 事务**
...
8. [x] **M4B-08 实现 Fail 与 Manual 事务**
...
- [x] Complete/Retry/Fail 与 Job/Outbox 原子性由事务回滚/响应重放测试证明。
- [x] 多前驱汇合、运行中控制 checkpoint、duplicate/reclaim 的真实 PostgreSQL 集成通过；更高次数压力测试仍可作为后续增强。

## Phase 4: Human And Control Plane

11. [x] **M4B-11 修正 Human Task 等待与提交语义**
...
12. [x] **M4B-12 实现控制命令 Repository/Application**
...
13. [x] **M4B-13 增加 Pause/Resume/Cancel HTTP 与 OpenAPI**
...
- [x] 多前驱、running worker control checkpoint、Human Worker outcome 和 PostgreSQL/River 集成矩阵通过；M4-D 仍负责生产 lifecycle/readiness/OTel。
- [ ] 未引入 Approval/SafeWriteback、部署、readiness、指标或第二队列实现。

## Phase 5: Compatibility And Quality Gate

15. [x] **M4B-15 完成兼容与安全扫描**
