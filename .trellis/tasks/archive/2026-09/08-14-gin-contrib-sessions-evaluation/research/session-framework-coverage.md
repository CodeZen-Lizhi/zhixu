...
2026-09-08 复核：GORM 前置已完成，原子认证/轮换与唯一 Session 表仍由项目拥有，下述强制项不满足的结论不变；正式决策见 ADR-0030。
...
| 唯一 PostgreSQL Session 表、共享连接池和正式迁移入口 | 是 | 10 | 无；PostgreSQL/GORM Store 自带连接和表，自定义 Store 不接管 | 0 | `cmd/api/main.go`、`atlas/migrations/00034_learning_ops_auth.sql` |
...
| 官方 GORM Store | 拒绝 | TODO 10 已完成；`gormstore` 自有表和 blob 生命周期仍不是项目 `auth.session` |
...
1. 候选能复用已交付的统一 GORM transaction boundary，不自建表、迁移或连接池；
