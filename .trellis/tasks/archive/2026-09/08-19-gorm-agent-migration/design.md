...
- GORM no-row 识别 `sql.ErrNoRows` 和 `gorm.ErrRecordNotFound`；当前 pgx `ErrNoRows` 包装标准 sentinel，GORM 无需直接 import pgx；SQLSTATE 通过 `platformpostgres.SQLState` 投影读取；
...
以下为完整风险场景目录；自 2026-09-01 起按父任务精简政策选择主路径与关键事务不变量，其余按直接改动风险触发。2026-09-08 实际验收范围见 `research/static-validation.md`。
...
现有 `internal/platform/testdb` 可通过 Testcontainers 提供真实 PostgreSQL，无外部 DSN 不阻断验证；compile-only/skip 不能视为实库验收。
