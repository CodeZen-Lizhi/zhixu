...
- [x] 统一 context Cause、cancel/deadline、no-row、`sql.ErrTxDone`、SQLSTATE 与 commit unknown 分类；GORM 文件通过平台错误投影读取 SQLSTATE。
...
- [x] staged GORM 文件无 pgx Tx/Row/Rows/pool/protocol、gorm.Open、root Transaction、DDL、
      AutoMigrate/Migrator、Save/Preload/Association 或 pgconn import。
...
## 5. Static Verification And Review（初次 staged 实现记录）
...
## 6. 真实 PostgreSQL 门禁（2026-09-08，按精简政策）

- [x] 复用已有 `internal/platform/testdb` fixture；legacy/GORM variant 独立迁移，GORM/UoW、Tools、Workflow、Agent、Events、Audit 来自同一个 platform Pool。无需外部 DSN。
- [x] 普通 Call/Policy/refusal/start/finalize/unknown/Timeline 主路径与敏感数据边界通过 legacy/GORM 对照。
- [x] Workspace Analysis participant/receipt/authority 闭包通过 legacy/GORM 对照。
- [x] Event 失败时所有 owner 写入回滚，验证 Tool Call、预算、Operation 与 Event 的同 scope 原子性。
- [x] refusal/Audit 原子回放、不写 Server Event 和无预算副作用通过 legacy/GORM 对照。
- [x] SQLSTATE 改用平台投影后，GORM 主路径、两个 owner 的局部 unit/vet 与 Go/SQL 复审通过。
- [x] gofmt、`git diff --check`、Trellis validate 与最新验证记录完成；未新增或修改测试文件，保留已有 fixture 改动。

完整 fault/response-loss/SQLSTATE/并发锁竞争/连接释放/多进程公平性/EXPLAIN 矩阵未在本轮全部执行；按风险或 Final 门禁触发，不把未执行场景记为通过。具体命令与耗时见 `research/static-validation.md`。

## 7. Final Handoff

- [x] `final-handoff.md` 记录普通与 Workspace Analysis constructor、同 Pool 依赖链和关闭顺序。
- [x] 标明 `ExecutionService.calls` 的 Receipt/WA/refusal 动态接口要求；由 Final 提供完整能力的纯委托组合，不能只传普通 GORMRepository。
- [x] 记录 legacy Repository、pgx transaction seam 与共享纯 helper 的删除/保留边界。
- [x] 记录 Final 静态规则：Tools 普通 data access 不允许 pgx；平台/worker 既有 River runtime 另按父任务 allowlist。
- [x] 回填 child AC；Worker Tool execution/Workspace Analysis production composition gate、状态及归档交主会话处理。
