...
当前状态以文末 2026-09-08 实库收口和 [`../final-handoff.md`](../final-handoff.md) 为准。
以下历史日期保留当时的验证范围，不再代表当前环境或 Model Settings scoped fence 的阻断状态。
...
## 2026-09-08 GORM Runtime 实库收口

遵循父任务 `research/lean-test-policy-2026-09-01.md`。本轮发现历史 Runtime/Worker integration
仍构造 legacy Repository，不能作为 GORM Runtime 的验收证据；已在三个既有文件中原位调整代表性
fixture，使用真实 `GORMRuntimeRepository`、同 Pool 的 Model Settings/Audit/Sealer 与 River
database/sql producer。没有新增测试文件、另一套数据库工厂、生产接线或 Schema 修改。

- Workflow unit、vet 通过。
- 第一组真实 PostgreSQL integration 通过（26.233s）：StartScoped、Claim/Heartbeat/terminal
  exact replay，以及 pgx Worker 实际消费 GORM Runtime 创建的 River Job。
- 第二组真实 PostgreSQL integration 通过（29.807s）：同 scope 的 Run/Node/Outbox/Job 可见性与
  调用方 rollback、Model Settings singleton 锁持有/释放、并发唯一启动、Claim/reclaim，以及
  scoped Terminal Hook 失败后的 Delivery/Control 整体回滚。
- 定向 integration `-race` 通过（49.706s）：Repository、Workspace execution fence、Tools
  policy/recovery fence 与并发租约。每条命令均限定 `-timeout=60s`，未跑全仓或全量故障矩阵。
- Retrieval child 报告 `TestDispatcherInsertFailureRollsBackDeliveryAndPublishedAt/gorm` 通过
  （包耗时 11.420s）：`NewGORMReindexOutbox` 配合真实 Change Control verifier、Model Settings
  fence 和 River producer，验证 Delivery/Job/Outbox 三方回滚与成功提交。准确命令见 handoff。

Docker 29.4.0 可用；共享 `testdb.Require(FailWhenUnavailable)` 直接提供隔离数据库，无环境变量
跳过。Go/SQL 人工审查未发现本范围剩余 P0/P1/P2，保留已有 `gorm_reindex_outbox.go`。本轮只修复
验证接线，未修改 Runtime 状态机或公共 API。

尚未重复执行 GORM commit response-loss、SIGKILL、Human/retry/join 全矩阵、完整跨 owner
端到端或容量 EXPLAIN。Foundation scope 的 active foreign-Pool 拒绝仍没有保证，Final 必须统一
Pool Composition。生产切换、legacy helper 迁出与 pgx allowlist 收口由主会话/Final 负责；child
状态保持 `in_progress`，不提交或归档。完整构造和清理清单见 [`../final-handoff.md`](../final-handoff.md)。
