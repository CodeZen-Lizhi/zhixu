...
- [x] Workflow Repository 读写迁移到 GORM，Runtime/Node/Outbox 和租约行为不变。
- [x] `ScopedWorkspaceAnalysisExecutionFence` 在同一 scope 内保持 Run -> Node Run -> Node Attempt 锁序、完整快照、`found=false` 与稳定错误合同；不访问 Agent 表、不泄漏数据库类型且不管理 caller transaction。
- [x] Tool policy snapshot 与 stale recovery fence 保持 legacy `FOR SHARE` / Node -> Attempt `SKIP LOCKED`
      语义，返回原始事实且不访问 Tool Call/Agent owner 表。
- [x] 业务写入与 River job insert 同事务提交/回滚，Worker 可消费且无重复/丢失语义变化。
- [x] 新增 GORM/scoped API 不泄漏数据库类型，legacy pgx allowlist 无新增调用；错误、取消、日志和连接池检查通过。
- [x] git diff --check、受影响包编译及现有相关测试通过。

- [x] TODO 9 工厂可用；本 child 仍不得切换生产实现或删除 legacy，TODO 3 仅阻断 Final。

2026-09-08：以上按父任务精简门禁复核，具体命令、范围和未重复执行的矩阵见 `final-handoff.md`。
勾选表示 Workflow 自身 staged 实现通过，不代表 Final Composition 已切换。
