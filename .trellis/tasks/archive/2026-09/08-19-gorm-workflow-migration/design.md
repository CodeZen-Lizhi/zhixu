...
本设计的 TODO 9 环境与 Model Settings fence 前置已解除。`internal/platform/testdb.Require` 在没有
`ZHIXU_TEST_DATABASE_URL` 时直接创建独立 pgvector 容器；Workflow fixture 固定
`FailWhenUnavailable`，不会静默跳过。`modelsettingspostgres.GORMRepository.CheckEnqueue` 已接受
`foundation.TransactionScope`，并在 Workflow 的 caller-owned scope 内持有 singleton `FOR SHARE`。
构造时仍需同 Pool 的 Audit appender 和 Secret Sealer，不能用 no-op fence 替代。

验收范围以父任务 `research/lean-test-policy-2026-09-01.md` 为准。本 child 已验证 GORM Runtime
主路径、Scoped Start/River/Model Settings 锁的提交回滚、并发启动/租约与 scoped Hook 回滚；旧记录中
使用 legacy Runtime 的测试不计作 GORM Runtime 证据。准确命令与 Final 接线清单见 `final-handoff.md`。
消费 owner 的具体 Start/Hook 与最终 Composition 仍由各自 child 和 Final 负责。
...
公开构造器要求完整平台 Pool、River Options 以及 typed-nil 安全的 scoped enqueue fence。Options 的 legacy `EnqueueFence` 必须为空；factory 在内部依次用 `riveradapter.NewClientWithOptions(pool.DB(), nil, options)` 建立同池 insert-only 配置 Client，再调用 `riveradapter.NewScopedJobInserter(pool, client, scopedFence)`。调用方不能注入另一个 Pool 的 Client/producer，Runtime root 与 River insert driver 因而都从同一个 Pool 派生。仅供同包验证的私有构造 helper 才可接收 `ScopedJobInserter` fake。Worker/listener client 仍由 Final 从同一个 `pool.DB()` 与同一份 queue options 构造。Model Settings GORM Repository 的 `CheckEnqueue(ctx, foundation.TransactionScope)` 是正式 scoped fence；禁止使用 no-op 替代。Foundation scope 的 cross-pool 限制仍适用于外部 caller-owned `StartScoped`，由 Composition 与单一 Pool fixture 保证。
...
实库工厂优先使用隔离 Testcontainers，不要求预先配置 `ZHIXU_TEST_DATABASE_URL`。Docker 或显式外部
admin 环境都不可用时，不得把 compile-only 写成实库 PASS。普通无 Hook GORM 基线不能代表带 Hook
的跨 owner 生产路径已验收；Final 必须使用各消费 child 的实际证据。

### Retrieval Reindex Outbox 参与者

`NewGORMReindexOutbox(pool)` 实现 Retrieval application 定义的 `ScopedReindexOutbox`。它只在
caller-owned scope 中领取/发布 `workflow.outbox_event`，不拥有事务和 Delivery mutation。
first-dispatch 保持 `occurred_at,id` FIFO、`FOR UPDATE OF event SKIP LOCKED`、未发布 predicate 和
已有 Delivery blocker 的只读反连接；Retrieval 在取得 Workspace advisory lock 后再次验证 blocker。
`PublishReindexOutboxScoped` 以 event ID、事件类型与 `published_at IS NULL` 做 CAS，并使用数据库时间。
Delivery、River Job 与 Outbox 发布由 Retrieval 的唯一 UoW 原子提交。此参与者迁移前由 Retrieval
直接执行的错误分类与稳定 code 保持不变，验证归入 Retrieval dispatcher 的既有 fixture。
