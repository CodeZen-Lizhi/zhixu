...
Foundation 与 28 个模块已有核心真实 PostgreSQL 及相关原子性证据；Final 统一完成 GORM/scoped 接线、旧 pgx 清理与可执行边界门禁。最终实际覆盖 30 个 persistence owner（模块之外单列 Approval Dispatch 与 Root Grant），30 个 child 的一对一验收映射见 [最终验收记录](../08-19-gorm-composition-pgx-convergence/research/final-acceptance.md)。

本地开发完成后，用户已授权提交、推送和状态同步；最终代码已推送至 `origin/dev`，目标环境部署未执行。Schema 与已发布迁移完全不变；追加 M9 用例暴露的既有 82 指针回填/trigger 冲突单列为历史升级发布阻断，不能把它记为 GORM 业务测试通过或擅自改写历史迁移。最终状态以收口记录和 task.json 为准，后文初始背景不代表当前仍未引入 GORM。
...
- [x] AC1：父任务维护完整、无重复的 PostgreSQL 模块清单；每个独立模块恰好映射到一个迁移子任务，跨模块依赖和执行波次可追踪。
- [x] AC2：GORM 平台与事务基础子任务完成，并证明显式配置、连接生命周期、错误翻译、事务/Savepoint、pgvector 和 River 原子性方案；不存在第二连接池竞争或事务降级。
- [x] AC3：每个模块子任务均满足 R6，并拥有独立验证证据和回滚点；父任务不以“部分模块已迁移”宣称 TODO 10 完成。
- [x] AC4：API、Worker、Workspace CLI/Probe 和测试 Composition 使用同一批准的数据访问入口；模块迁移期间没有长期双写、双读或运行时 selector。
- [x] AC5：Domain/Application 不导入 GORM 或 pgx；普通业务 Repository 不直接依赖 pgx，残留 pgx 与父任务 allowlist 完全一致且被静态门禁保护。
- [x] AC6：生产、测试和命令入口均不能调用 GORM `AutoMigrate`/`Migrator` 修改 Schema；Persistence Model 与 Atlas 批准结构一致。
- [x] AC7：每个模块均有 R6 规定的核心真实 PostgreSQL 证据；事务/锁/River/跨 owner 模块另有一条代表性原子性或冲突验证，性能与故障专项只在改动风险触发时执行。
- [x] AC8：TODO 9 已提供 Testcontainers 验收门禁、TODO 3 已完成 Atlas 唯一 Schema 事实源后，最终 Composition/pgx 收口子任务完成，TODO 10 才可归档。
