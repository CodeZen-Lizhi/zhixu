...
| T04 | Implement create/list/read/turn/answer repositories | `internal/conversation/application/**`, `adapter/postgres/**` | T02,T03 | real PG tests cover Workspace isolation, stable paging, bounded context, no N+1, exact replay/conflict and terminal Answer read projection | cross-workspace leak | completed: real PostgreSQL proves stable Workspace paging, batched Turn/Answer reads, latest 8/32 KiB context, exact replay/conflict, event-failure rollback, commit-response-loss recovery and replay after event retention cleanup |
| T05 | Implement persistent Server Event store and SSE handler | `internal/events/**`, migration trigger | T02,T03 | handler tests cover headers/framing/heartbeat/cancel; PG tests cover monotonic order, replay, future/invalid/expired cursor and redacted payload | long-lived resource leak | completed: typed/redacted events, caller-owned AppendTx concurrency/replay, bounded retained reads and preflighted SSE framing/heartbeat/cancel pass unit, race and real PostgreSQL gates; production Router/Composition Root registration remains T11 |
| T06 | Implement atomic Question-to-Workflow dispatch | `internal/conversation/adapter/postgres/dispatch*`, Workflow StartTx reuse | T02-T04 | concurrent same-key/different-key tests; any injected failure rolls back Question/Answer/Run/Node/Event/Job; response-loss exact replay | cross-schema half state | completed: real PostgreSQL proves the single UoW, same/different-key concurrency, archived/terminal semantics, injected rollback, response-loss/event-retention replay and replay during a real Worker Claim; full race/vet/tidy/Make gates and two-round independent review pass |
| T07 | Add Query Plan/RAG Workflow contract and context loader | `internal/agent/adapter/workflow/rag_*`, Conversation execution port, Agent catalog | T03,T04,T06 | strict input/output tests; only IDs/hash in persisted input; bounded context/options and binding drift fail closed; PLAN/RAG v2/Clarification schemas and Definition registered | raw content leak | completed: strict six-field output receipt, exact v1/v2/PLAN/Clarification catalog snapshots, narrow two-query execution-context loader, persisted-history hash recomputation, race/real PostgreSQL/vet/tidy gates and independent review pass |
| T08 | Implement retrieval-first Query Plan and RAG executor | Agent workflow, Retrieval/Knowledge related-topic adapters, model catalog | T07 | tests cover clarification, 1..3 rewrites, persistent retrieval summary, no evidence refusal, RAG v2 related topic/follow-up, conflicts, gates, degradation, provider failure and no Tool Loop | hallucination, duplicate model call | completed: strict PLAN、scoped Retrieval、单批 Eligibility/Topic、冲突披露、RAG v2/Refusal、Citation/Faithfulness、真实阶段事件、冻结上下文 Workflow executor 与 terminal-first replay 通过 race/PG/review |
| T09 | Implement atomic Model Run/Answer finalizer and replay | Agent PG Tx seam, Conversation PG finalizer | T08 | real PG concurrency/response-loss tests prove one terminal Answer/Refusal/Clarification, matching terminal Model Run, immutable result/summary and replay without Provider call | split terminal facts | completed: 00022 deferred Retrieval、Model Run/Answer/Conversation/Event 单事务 CAS、三类终态、并发 exact replay、commit response-loss、event rollback/retention 与 split-state 检测通过真实 PostgreSQL 和独立复验 |
| T10 | Implement Conversation/Answer/Feedback HTTP and OpenAPI | `internal/conversation/http/**`, `internal/app/router.go`, `cmd/api/main.go`, `api/openapi/**` | T04,T06,T09 | strict JSON, depth/format defaults and hashing, idempotency, cursor, ETag, 202/status URL, clarification/retrieval summary, Problem/404 anti-enumeration and 405 tests; `make openapi-check` | wire drift | completed: 7 条 REST Handler、Feedback append-only PG、稳定 scoped cursor、Answer+Workflow ETag、strict JSON/media type、可打开 Citation 投影与 8 条 OpenAPI/SSE 契约通过 race/真实 PG/独立复验；生产真实依赖组装按 T11 继续 |
| T11 | Wire production API/Worker composition and readiness | `cmd/api/**`, `cmd/worker/**`, config/readiness tests | T05-T10 | RAG definition/executor registered only with real dependencies; disabled/mismatch states fail closed; existing relation/tool workers unaffected | partial readiness | completed: API共享Event Store组装Conversation/SSE并门禁Question，Worker真实组装RAG全依赖与双Registry；disabled/read-only、enabled fail-closed、status/OpenAPI、临时迁移PG composition及Relation/Tool/SafeWriteback回归通过 |
| T12 | Implement typed frontend Conversation/RAG clients and shared SSE thin layer | `web/src/api/**`, mandatory `web/src/events/**` | T05,T10 | strict decoders reject invalid UUID/time/status/envelope/RAG v2/clarification/summary; command keys replay; sole SSE owner covers parser/reconnect/expired fallback and targeted invalidation | duplicate decoder | completed: strict Conversation/RAG wire owner、四态 Answer、持久 current_stage/ETag 恢复、fetch-stream SSE cursor/recovery/invalidation 与 93 个前端测试通过；真实 PostgreSQL 阶段投影和独立复验无 P0-P2 |
| T13 | Implement minimal real RAG page | `web/src/features/rag/**`, routes, styles, component tests | T12 | create/select Conversation, depth/format, submit/clarify, stage/poll recovery, Answer/Refusal/conflict/Citation/summary/related topics/follow-ups/feedback; keyboard/live region; desktop/mobile stable layout | draft shown as final | completed: `/chat` 双路由、Workspace单一owner、无限分页+latest Turn恢复、SSE/有界轮询、四态Answer/证据/反馈/范围控件与移动Citation drawer完成；108前端测试及真实临时DB/API桌面/移动浏览器烟测通过 |
| T14 | Add RAG integration and Compose smoke gates | `Makefile`, `deploy/compose-rag-smoke.sh`, Go integration tests, deterministic model fixture | T09-T13 | `make rag-integration` and `make compose-rag-smoke` traverse public API→River→Retrieval→Agent→Answer→SSE→Feedback and exact replay | smoke bypasses real seam | completed: public HTTP/River/Retrieval/Knowledge/Model/Answer/SSE/Feedback, exact three Model Calls, replay zero calls, Bearer/schema fixture and disposable Compose gate pass |
| T15 | Sync docs/specs and run full quality gate | `README.md`, architecture/API/testing/runbook/spec docs, task status | T02-T14 | all commands below pass; browser actually opens desktop/mobile; diff/review clean; docs match wire/schema/runtime | undocumented behavior | completed: README/API/RAG/frontend/testing/deployment/checklist and parent plan synchronized; full race/vet/make/frontend/OpenAPI/PG/Compose gates pass; T13 desktop/mobile real browser evidence retained |
| T16 | Independent cross-layer/Go/SQL/frontend review, fix and reverify | full task diff | T15 | reviewer checks requirement completeness, logic, edges, quality, tests and runtime; all verified findings fixed; at most two re-review rounds | review blind spot | completed: 修复损坏本地 SSE cursor 的权威回查/无游标恢复与 nil Context fail-closed，独立复验无 P0-P2 |
| T17 | Commit, archive and record journal | Trellis task/parent/journal, Git | T16 | focused implementation commit plus archive/journal commits; clean worktree except pre-existing unrelated changes; no push | lost task trace | completed: 交付改动已进入提交，任务按 Trellis 流程归档且不 push |
