...
3. [ ] 重构 `cmd/worker` lifecycle：按顺序启动 Registry/Runtime/Health/River，直接调用 v0.40.0 `Start`；正常 Stop+SoftStopTimeout 与紧急 StopAndCancel 两条路径互斥，增加 hard shutdown deadline、非零退出和资源关闭测试。
...
7. [ ] 增加 graceful stop、forced cancel、kill -9、DB 短断、两 Worker 重启恢复 smoke，断言不重复领域副作用。
