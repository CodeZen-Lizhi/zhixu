...
- GORM 文件不使用 pgx Tx/Row/Rows/pool/protocol；`pgconn.PgError` 仅用于 SQLSTATE 分类。
...
本 child 只维护以下场景/fixture 契约并保持 `in_progress`；TODO 9 执行后将结果回填
`research/static-validation.md`，才可勾选对应 AC。
