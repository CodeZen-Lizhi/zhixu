...
- [x] Phase 6 的 Chi 清理、tidy、vendor 解析和分层 import 门禁已完成；Application integration smoke 改经生产 Router，
  不再直接 import Gin。
- [x] Phase 7 已完成全仓 race/vet、受影响普通/race、API/OpenAPI/vendor/Compose 子合同与独立审查；两个既有基线失败、
  launcher fixture 端口竞态及外部 PostgreSQL/浏览器边界已在 `prd.md` 精确记录。
- [x] Phase 8 的路线图/系统设计同步已由 `ab6029c4` 完成；Gin HTTP 长期规范、任务证据、最终提交和归档在本次收口完成。

## Phase 1. Freeze Baseline

- [x] 记录 25 个 Handler、179 个 Handler operation、3 个 app operation（livez/readyz/system status）、154 OpenAPI path/
  182 operation 和可选 `/metrics`。
- [x] 增加 runtime route inventory 测试，规范化 Gin `:param` 为 OpenAPI `{param}`，精确比较 method/path 集合。
- [x] 增加 Engine 默认差异测试：尾斜杠/双斜杠/编码参数不漂移、404/405 使用 Problem、unsupported method、`Allow` Header、
  trusted proxy spoof、静态 fallback、可选 metrics。
- [x] 先运行现有 Router/Auth/SSE/strict JSON/OpenAPI 基线，失败时区分迁移回归与用户并行改动。
...
- [x] 将 `github.com/gin-gonic/gin v1.12.0` 加入 `go.mod`；暂不删除 Chi，待所有调用方迁移后统一 tidy/vendor。
- [x] 在 `internal/httpapi` 增加 Gin Handler bridge：复制 route params 到标准 `Request.PathValue` 后调用官方 `gin.WrapF`。
- [x] 增加 validator helper 和并发/边界测试；只为已锁定通用约束添加 `validate` tag 和原错误映射。
- [x] 验证 bridge 保留 Header、status、`http.Flusher`、request context 和 path value。
...
- [x] 用 `gin.New` 重写 `internal/app/router.go`，显式配置 redirect、raw path、405、proxy、NoRoute/NoMethod 和 static fallback；
  不使用 `gin.Default`、Gin Recovery 或 Gin StaticFS。
- [x] 将 no-store、request ID、trace、request log、panic recovery 改为 Gin Middleware；用 Gin-aware response-start tracker
  替换 Chi `statusWriter`，保留 Flusher/Hijacker/Pusher/Unwrap。
- [x] 保持路由日志 `{param}` 模板格式和敏感信息边界。
- [x] 将 `internal/auth/http` 的 routes 和 Middleware 改为 Gin，认证成功仍通过标准 request context 传 Principal。
- [x] 更新 app/auth/recovery tests，覆盖 Middleware 顺序、Abort、无输出与 200/204/Write/Flush/NoRoute response-started panic、
...
- [x] Workspace、Workflow、Change Control、Collection。
- [x] Health、Ingestion、Retrieval、Graph/Candidate。

### Batch B

- [x] Conversation、Events、Knowledge、Artifact、Authoring。
- [x] Capture、Organizing、Document History、Git Sync。
- [x] Model Settings 由主会话在用户现有未提交改动上增量迁移。

### Batch C

- [x] Export/Attachment。
- [x] Review、Learning Path、Memory、Interview。
- [x] Review no-store group Middleware 改为 Gin，保持所有 route group Header。
...
- [x] `NewRouter` 注册不 panic，runtime Gin routes 与当前 OpenAPI 183 operation 精确相等；只排除不属于 OpenAPI 的可选 `/metrics`，
  `/livez` 和 `/readyz` 必须参与对比。
- [x] Auth/CSRF/Origin/API Token/Capability/Workspace/write authorization 回归通过。
- [x] strict JSON、unknown fields、Unicode、multiple document、body/content-type limit 和 validator error mapping 回归通过。
- [x] SSE Last-Event-ID、heartbeat、flush、replay/reset/end、断连和 response-started panic 回归通过。
- [x] Capture multipart 和 Export/Attachment 下载 Header/字节/stream 回归通过。
- [x] cmd/api composition 与相关真实 PostgreSQL/Router 集成测试可编译；当前无数据库 URL，未伪报运行通过。
...
- [x] 全仓生产/测试 import 和 symbol 删除 Chi；删除临时迁移代码。
- [x] 从 `go.mod`/`go.sum` 删除 Chi，锁定 Gin；执行 tidy 和 vendor 更新。
- [x] 验证 Domain/Application/Repository/Workflow 无 Gin import。
- [x] 检查 vendor license/manifest 一致且 vendor mode 可构建。
...
- [x] 运行 focused 和全仓 Go tests；失败先定位是否依赖外部数据库/Docker，再按项目规则重试一次或记录盲区。
- [x] 运行 race、vet、API build、OpenAPI 和适用 Compose 门禁。
- [x] 使用 `go-review` 和 `code-review-and-quality`，并由独立 reviewer 复核共享 HTTP、安全、SSE、依赖和测试覆盖。
- [x] 修复当前范围内所有已验证缺陷后重新运行受影响门禁。
...
- [x] 更新 `.trellis/spec/backend` 对应 HTTP/错误/质量事实和 `docs/architecture/system-design.md` 当前技术栈。
